function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid >= 2086 and item.itemid <= 2089 and item:getActionId() == 1001 then
		-- Key used on door
		if target and target.getActionId then
			if target:getActionId() == 1001 then
				-- Door is locked, unlock it and open
				target:setActionId(1002) -- Mark as unlocked
			...
			elseif target:getActionId() == 1002 then
				-- Door is unlocked, lock it and close
				target:setActionId(1001) -- Mark as locked
	...
	if item:getActionId() == 1001 then
		-- Door is locked - check for key with action ID 1001
		local hasKey = false
		...
		if backpack and backpack:isContainer() then
			for i = 0, backpack:getSize() - 1 do
				...
				if backpackItem and backpackItem:getActionId() == 1001 then
					hasKey = true
					break
				end
			end
		...
		if hasKey then
			...
			item:setActionId(1002) -- Mark as unlocked
	...
	elseif item:getActionId() == 1002 then
