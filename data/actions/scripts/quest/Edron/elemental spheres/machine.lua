...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if isInArray({7911, 7912}, item.itemid) then
		local gemCount = player:getStorageValue(GlobalStorage.ElementalSphere.MachineGemCount)
		if isInArray({33268, 33269}, toPosition.x) and toPosition.y == 31830 and toPosition.z == 10 and gemCount >= 20 then
			player:setStorageValue(GlobalStorage.ElementalSphere.MachineGemCount, gemCount - 20)
