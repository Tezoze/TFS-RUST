...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 1946 then
		...
		for i = 1, #config.playerPositions do
			...
			if playerTile:getStorageValue(Storage.AnnihilatorDone) == 1 then
				player:sendTextMessage(MESSAGE_STATUS_SMALL, playerTile:getName() .. " has already completed the Annihilator.")
				return true
			end
		...
		for i = 1, #storePlayers do
			...
			players:setStorageValue(Storage.AnnihilatorDone, 1)
		end
