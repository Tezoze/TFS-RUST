local config = {
	...
	[3020] = {
		...
		storage = Storage.TheTravellingTraderQuest.Mission02,
		formerValue = 3,
		newValue = 4
	},
	[3024] = {
	...
	[50128] = {
		...
		storage = Storage.hiddenCityOfBeregar.RoyalRescue,
		formerValue = 4,
		newValue = 4
	},
	[50126] = {
	...
	[45305] = {
		...
		storage = Storage.BloodBrothers.BloodCrystal.Quest,
		newValue = 1
	},
	-- 65203 reservado
}
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if size == 1 then
		...
		if not reward then
			print('[Warning] QuestSystem:', 'Could not create quest reward item')
			return true
		end
	end
	...
	if reward then
		...
		if items[1].actionId and reward.setActionId then
			reward:setActionId(items[1].actionId)
		end

		if items[1].text and reward.setText then
			reward:setText(items[1].text)
		end

		if items[1].decay and reward.decay then
			reward:decay()
		end
		...
		for i = 1, size do
			...
			if reward:addItemEx(tmp) ~= RETURNVALUE_NOERROR then
				...
				if items[i].actionId and tmp.setActionId then
					tmp:setActionId(items[i].actionId)
				end

				if items[i].text and tmp.setText then
					tmp:setText(items[i].text)
				end

				if items[i].decay and tmp.decay then
					tmp:decay()
				end
