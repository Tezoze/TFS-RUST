...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 12320 and target.actionid == 8026 then
		local poisonCount = Game.getStorageValue(Storage.WrathoftheEmperor.Mission03) or 0
		if poisonCount < 5 then
			Game.setStorageValue(Storage.WrathoftheEmperor.Mission03, poisonCount + 1)
