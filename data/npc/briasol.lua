-- Briasol - Converted from XML to Lua NpcType
-- Original XML: data/npc/Briasol.xml
-- Original Script: data/npc/scripts/Briasol.lua

local npcName = "Briasol"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a briasol")
npcType:health(100)
...
npcType:outfit({lookType = 159, lookHead = 3, lookBody = 86, lookLegs = 87, lookFeet = 76})
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "fine vase") then
		if player:getStorageValue(Storage.TheTravellingTraderQuest.Mission04) == 1 then
			npcHandler:say({
	...
	elseif msgcontains(msg, "yes") then
		if npcHandler.topic[cid] == 1 then
			if player:getMoney() + player:getBankBalance() >= 1000 then
				...
				player:setStorageValue(Storage.TheTravellingTraderQuest.Mission04, 2)
