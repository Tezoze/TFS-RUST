-- Yalahari (NpcBuilder — In Service of Yalahar Quest, Bad Side)
local npc = NpcBuilder:new("Yalahari",
	{lookType = 309, lookHead = 38, lookBody = 88, lookLegs = 88, lookFeet = 115})
...
npc:onSay(function(npc, player, message, builder)
	...
	if lower:find("mission", 1, true) then
		if player:getStorageValue(Storage.InServiceofYalahar.Questline) == 17 then
			...
			player:setStorageValue(12273, 1) -- Access to Azerus door
			s.topic = 0
		elseif player:getStorageValue(Storage.InServiceofYalahar.NotesAzerus) == 2 and player:getStorageValue(Storage.InServiceofYalahar.Questline) == 18 then
			builder:sayMultiple(npc, {
		...
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 19 then
			...
			player:setStorageValue(12273, 1) -- Access to Azerus door
			player:setStorageValue(12275, 1) -- Access to alchemist quarter door
			s.topic = 0
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 21 and player:getStorageValue(Storage.InServiceofYalahar.AlchemistFormula) == 1 then
			player:unregisterEvent("ServiceOfYalaharDiseasedTrio")
			builder:say(npc, "So you have killed the plague carriers. Have you also retrieved the research papers? ", player)
			s.topic = 1
		...
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 27 then
			...
			player:setStorageValue(12263, 1) -- Access to Tamerin's door
			player:setStorageValue(12264, 1) -- Access to Morik's door
			s.topic = 0
		...
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 41 and player:getStorageValue(Storage.InServiceofYalahar.QuaraInky) == 1 and player:getStorageValue(Storage.InServiceofYalahar.QuaraSharptooth) == 1 and player:getStorageValue(Storage.InServiceofYalahar.QuaraSplasher) == 1 and player:getStorageValue(Storage.InServiceofYalahar.QuaraState) == 2 then
			player:unregisterEvent("ServiceOfYalaharQuaraLeaders")
			builder:say(npc, "This will teach these fishmen who is the ruler of that area. You have earned yourself a special privilege. But we will talk about that when we speak about your next mission. ", player)
		...
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 46 then
			if player:getStorageValue(Storage.InServiceofYalahar.MatrixState) == 1 then
				builder:say(npc, "Your failure is an outrage! I think we have to talk about the missions you have accomplished so far. ", player)
			...
			elseif player:getStorageValue(Storage.InServiceofYalahar.MatrixState) == 2 then
				builder:say(npc, "Now we will have power we truly deserve!...", player)
				player:setStorageValue(Storage.InServiceofYalahar.BadSide, player:getStorageValue(Storage.InServiceofYalahar.BadSide) >= 0 and player:getStorageValue(Storage.InServiceofYalahar.BadSide) + 1 or 0) -- Side Storage
			end
		...
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 49 or player:getStorageValue(Storage.InServiceofYalahar.Questline) == 48 then
			builder:say(npc, "So do you want to side with me |PLAYERNAME|? ", player)
			s.topic = 2
		...
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 52 and player:getStorageValue(Storage.InServiceofYalahar.SideDecision) == 2 then
			builder:say(npc, "Great work, take this outfit and you are able to open the door to the reward room.", player)
	...
	elseif lower == "yes" then
		if s.topic == 1 then
			if player:removeItem(9733, 1) then
				...
				player:unregisterEvent("ServiceOfYalaharDiseasedTrio") -- Deregister diseased trio events after mission completion
				builder:say(npc, "Impressive indeed! Someone with your skills will quickly raise in our ranks of helpers. You have great potential, and if you are upfor further missions, just ask for them. ", player)
				s.topic = 0
			end
			return true
