-- Grof, The Guard - Converted from XML to Lua NpcType
-- Original XML: data/npc/Grof, The Guard.xml
-- Original Script: data/npc/scripts/Grof, The Guard.lua
...
npcType:nameDescription("a grof, the guard")
npcType:health(100)
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "trouble") and (player:getStorageValue(Storage.TheInquisition.GrofGuard) or 0) < 1 and (player:getStorageValue(Storage.TheInquisition.Mission01) or 0) ~= -1 then
	...
	elseif msgcontains(msg, "authorities") then
		if npcHandler.topic[cid] == 1 then
			...
			if (player:getStorageValue(Storage.TheInquisition.GrofGuard) or 0) < 1 then
