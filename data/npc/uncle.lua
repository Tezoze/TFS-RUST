-- Uncle - Converted from XML to Lua NpcType
-- Original XML: data/npc/Uncle.xml
-- Original Script: data/npc/scripts/Uncle.lua
...
npcType:nameDescription("a uncle")
npcType:health(100)
...
npcType:outfit({lookType = 151, lookHead = 38, lookBody = 19, lookLegs = 20, lookFeet = 95, lookAddons = 1})
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'yes') then
		if npcHandler.topic[cid] == 1 then
			...
			player:setStorageValue(12550, 1) -- Quest log integration
		...
		elseif npcHandler.topic[cid] == 2 then
			...
			player:setStorageValue(12552, 4) -- Quest log: Mission completed
		...
		elseif npcHandler.topic[cid] == 3 then
			if player:removeItem(7705, 1) then
				...
				player:setStorageValue(12555, 2) -- Mission 2 completed
		...
		elseif npcHandler.topic[cid] == 4 then
			...
			player:setStorageValue(12558, 4) -- Mission 3 completed
		...
		elseif npcHandler.topic[cid] == 5 then
			...
			player:setStorageValue(12561, 3) -- Mission 4 completed
			player:unregisterEvent("SecretServiceBlackKnight") -- Unregister Black Knight kill event
		...
		elseif npcHandler.topic[cid] == 6 then
			if player:removeItem(7708, 1) then
				...
				player:setStorageValue(12564, 2) -- Mission 5 completed
		...
		elseif npcHandler.topic[cid] == 7 then
			...
			player:setStorageValue(12567, 3) -- Mission 6 completed
		...
		elseif npcHandler.topic[cid] == 8 then
			if player:removeItem(7699, 1) then
				...
				player:setStorageValue(12569, 2) -- Mission 7 completed
	...
	elseif msgcontains(msg, 'join') then
		if player:getStorageValue(Storage.secretService.Quest) < 1 then
			...
			npcHandler:say('You are already serving another city\'s secret service! You cannot join multiple factions.', cid)
		end
	elseif msgcontains(msg, 'mission') then
		if player:getStorageValue(Storage.secretService.Quest) == 1 then
			player:setStorageValue(Storage.secretService.Quest, 2)
			player:setStorageValue(Storage.secretService.AVINMission01, 1)
			player:setStorageValue(12552, 1) -- Mission 1: For Your Eyes Only
		...
		elseif player:getStorageValue(Storage.secretService.AVINMission01) == 4 and player:getStorageValue(Storage.secretService.Quest) == 3 then
			...
			player:setStorageValue(12555, 1) -- Mission 2: A File Between Friends
			npcHandler:say({
		...
		elseif player:getStorageValue(Storage.secretService.AVINMission02) == 2 and player:getStorageValue(Storage.secretService.Quest) == 5 then
			...
			player:setStorageValue(12558, 1) -- Mission 3: What Men are Made of
			npcHandler:say({
		...
		elseif player:getStorageValue(Storage.secretService.AVINMission03) == 4 and player:getStorageValue(Storage.secretService.Quest) == 7 then
			...
			player:setStorageValue(12561, 1) -- Mission 4: Pawn Captures Knight
			player:registerEvent("SecretServiceBlackKnight") -- Register Black Knight kill event
			npcHandler:say({
		...
		elseif player:getStorageValue(Storage.secretService.AVINMission04) == 3 and player:getStorageValue(Storage.secretService.Quest) == 9 then
			...
			player:setStorageValue(12564, 1) -- Mission 5: A Cryptic Mission
		...
		elseif player:getStorageValue(Storage.secretService.AVINMission05) == 2 and player:getStorageValue(Storage.secretService.Quest) == 11 then
			...
			player:setStorageValue(12567, 1) -- Mission 6: A Little Bribe Won't Hurt
		...
		elseif player:getStorageValue(Storage.secretService.AVINMission06) == 3 and player:getStorageValue(Storage.secretService.Quest) == 13 then
			...
			player:setStorageValue(12569, 1) -- Mission 7: Licence to Kill
			npcHandler:say({
