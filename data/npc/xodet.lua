-- Xodet - Magic Shop NPC (Converted from XML to Lua NpcType)
-- Uses NpcHandler for focus/distance management like XML NPCs

local npcName = "Xodet"
local npcType = Game.createNpcType(npcName)

npcType:name(npcName)
npcType:nameDescription("a magic shopkeeper")
npcType:health(100)
...
-- Use NpcHandler for focus/distance management
local keywordHandler = KeywordHandler:new()
local npcHandler = NpcHandler:new(keywordHandler)

-- First wand/rod items by vocation base ID
local firstMageItems = {
    [1] = 2190,  -- Sorcerer -> Wand of Vortex
    [2] = 2182,  -- Druid -> Snakebite Rod
}

-- Custom message handler for first wand/rod quest
local function creatureSayCallback(cid, type, msg)
    ...
    local itemId = firstMageItems[vocationId]
    
    if msgcontains(msg, 'first rod') or msgcontains(msg, 'first wand') then
        -- Check if player is a mage (sorcerer=1 or druid=2)
        if vocationId == 1 or vocationId == 2 then
...
npcHandler:setMessage(MESSAGE_GREET, "Welcome to my little magic shop, |PLAYERNAME|! Ask me for a {trade} if you want to see my wares.")

keywordHandler:addKeyword({'job'}, StdModule.say, {npcHandler = npcHandler, text = "I am selling magical equipment."})
keywordHandler:addKeyword({'name'}, StdModule.say, {npcHandler = npcHandler, text = "My name is Xodet."})
keywordHandler:addKeyword({'magic'}, StdModule.say, {npcHandler = npcHandler, text = "I sell wands, rods, runes, potions and spellbooks. Just ask me for a {trade}."})
keywordHandler:addKeyword({'equipment'}, StdModule.say, {npcHandler = npcHandler, text = "I sell wands, rods, runes, potions and spellbooks. Just ask me for a {trade}."})
