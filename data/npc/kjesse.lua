-- Kjesse - Converted from XML to Lua NpcType
-- Original XML: data/npc/Kjesse.xml
-- Original Script: data/npc/scripts/Kjesse.lua

local npcName = "Kjesse"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a kjesse")
npcType:health(100)
...
local function addHealKeyword(text, condition, effect)
	keywordHandler:addKeyword({'heal'}, StdModule.say, {npcHandler = npcHandler, text = text},
		function(player) return player:hasCondition(condition) end,
		function(player)
