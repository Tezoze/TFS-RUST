-- Dorian - Converted from XML to Lua NpcType
-- Original XML: data/npc/Dorian.xml
-- Original Script: data/npc/scripts/Dorian.lua

local npcName = "Dorian"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a dorian")
npcType:health(100)
...
npcType:outfit({lookType = 151, lookHead = 22, lookBody = 58, lookLegs = 77, lookFeet = 21, lookAddons = 2})
...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "yes") then
		...
		elseif npcHandler.topic[cid] == 7 then
			if player:removeItem(8764, 1) then
...
local shopItems = {
    {id = 8717, buy = 50, sell = 0, subType = 0, name = "lock pick"},
}
