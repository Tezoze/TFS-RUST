-- Prezil - Converted from XML to Lua NpcType
-- Original XML: data/npc/Prezil.xml
-- Original Script: data/npc/scripts/Prezil.lua

local npcName = "Prezil"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a prezil")
npcType:health(100)
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "adventurer") and msgcontains(msg, "stone") then
		...
		elseif player:getStorageValue(Storage.AdventurersGuild.FreeStone.Prezil) ~= 1 then
	...
	if npcHandler.topic[cid] == 2 and msgcontains(msg, "yes") then
		...
		player:setStorageValue(Storage.AdventurersGuild.FreeStone.Prezil, 1)
		npcHandler.topic[cid] = 0
		return true
