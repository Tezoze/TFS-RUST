-- Asrak - Converted from XML to Lua NpcType
-- Original XML: data/npc/Asrak.xml
-- Original Script: data/npc/scripts/Asrak.lua
...
npcType:nameDescription("a asrak")
npcType:health(100)
...
npcType:outfit({lookType = 29, lookHead = 10, lookBody = 20, lookLegs = 30, lookFeet = 40})
...
local spells = {
    -- HEALING SPELLS (Shared + Paladin Specific)
    ...
    {name = 'Destroy Field',        price = 700,   level = 17,  vocations = {3, 7},       type = 'support'},
...
-- Helper function to check spell vocation requirement
...
-- Helper to check if player is Paladin OR Knight (for conversation flow)
...
local function creatureSayCallback(cid, type, msg)
    ...
    -- GREETING INFO
    if m == 'spells' then
        if not isValidVocation(player) then
    ...
    if m:find('healing') or m:find('support') or m:find('conjure') then
        ...
        if m:find('support') then category = 'support'
...
npcHandler:setMessage(MESSAGE_GREET, "Welcome to our shelter, |PLAYERNAME|.")
