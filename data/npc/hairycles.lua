-- Hairycles - Converted from XML to Lua NpcType
-- Original XML: data/npc/Hairycles.xml
-- Original Script: data/npc/scripts/Hairycles.lua

local npcName = "Hairycles"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a hairycles")
npcType:health(100)
...
local function creatureSayCallback(cid, type, msg)
	...
	elseif npcHandler.topic[cid] == 2 then
		if msgcontains(msg, 'yes') then
			...
			player:setStorageValue(12128, 1)
		elseif msgcontains(msg, 'no') then
	...
	elseif npcHandler.topic[cid] == 6 then
		if msgcontains(msg, 'yes') then
			...
			player:setStorageValue(12119, 1)
		elseif msgcontains(msg, 'no') then
			npcHandler:say('Me sad. Me expected better from you!', cid)
			addEvent(releasePlayer, 1000, cid)
		end
...
local shopItems = {
    {id = 5086, buy = 65, sell = 0, subType = 0, name = "monkey statue 'see' kit"},
    {id = 5087, buy = 65, sell = 0, subType = 0, name = "monkey statue 'hear' kit"},
    {id = 5088, buy = 65, sell = 0, subType = 0, name = "monkey statue 'speak' kit"},
}
...
local function openTradeWindow(cid, message, keywords, parameters, node)
    ...
    -- Quest check: must have completed The Ape City quest (Questline >= 18)
