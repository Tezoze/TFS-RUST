-- Baa'leal - Converted from XML to Lua NpcType
-- Original XML: data/npc/Baa'leal.xml
-- Original Script: data/npc/scripts/Baa_leal.lua

local npcName = "Baa'Leal"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a baa'leal")
npcType:health(100)
...
local function greetCallback(cid, message)
	...
	-- Check if player used the correct Efreet greeting
	if message and type(message) == "string" and not msgcontains(message, 'djanni\'hah') and player:getStorageValue(Storage.DjinnWar.Faction.Efreet) ~= 1 then
		-- Wrong greeting and not Efreet faction - apply fire damage
	...
	local missionProgress = player:getStorageValue(Storage.DjinnWar.EfreetFaction.Mission01) or -1
