-- Pemaret - Converted from XML to Lua NpcType
-- Original XML: data/npc/Pemaret.xml
-- Original Script: data/npc/scripts/Pemaret.lua

local npcName = "Pemaret"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a pemaret")
npcType:health(100)
...
npcType:outfit({lookType = 128, lookHead = 79, lookBody = 10, lookLegs = 126, lookFeet = 126})
...
local function creatureSayCallback(cid, type, msg)
	...
	if m:find('marlin') then
		if player:getItemCount(7963) > 0 then
			...
			npcHandler:say("A marlin is a magnificent fish! If you catch one, bring it to me and I can make a decoration for you.", cid)
		end
		return true
	end
...
local travelDestinations = {
	{keyword = 'edron', cost = 20, destination = Position(33176, 31764, 6), 
	 text = 'Do you want to get to Edron for |TRAVELCOST|?'},
	{keyword = 'eremo', cost = 0, destination = Position(33314, 31883, 7), 
	 text = 'Oh, you know the good old sage Eremo. I can bring you to his little island free of charge. Do you want me to do that?'}
}

for _, travel in ipairs(travelDestinations) do
	if travel.cost > 0 then
		local travelKeyword = keywordHandler:addKeyword({travel.keyword}, StdModule.say, {
...
-- BASIC KEYWORDS
keywordHandler:addKeyword({'job', 'captain'}, StdModule.say, {
	npcHandler = npcHandler, 
...
local shopItems = {
    {id = 7159, buy = 0, sell = 100, subType = 0, name = "green perch"},
    {id = 7963, buy = 0, sell = 800, subType = 0, name = "marlin"},
    {id = 2669, buy = 0, sell = 100, subType = 0, name = "northern pike"},
    {id = 7158, buy = 0, sell = 100, subType = 0, name = "rainbow trout"},
    {id = 2667, buy = 5, sell = 0, subType = 0, name = "fish"},
}
