-- Palimuth (NpcBuilder — In Service of Yalahar Quest, Good Side)
local npc = NpcBuilder:new("Palimuth",
	{lookType = 325, lookHead = 97, lookLegs = 79, lookFeet = 98})
npc:greetMessage("Greetings.")
npc:farewellMessage("Good bye, |PLAYERNAME|.")

-- Simple keyword responses
npc:addKeyword("blood crystal", "What would you need a blood crystal for? Maybe a magician knows where to find one.")
...
npc:onSay(function(npc, player, message, builder)
	...
	-- Auto-initialize questline
	if questline < 1 then
		player:setStorageValue(Storage.InServiceofYalahar.Questline, 3)
		questline = 3
	end

	if lower:find("job", 1, true) then
		if player:getStorageValue(Storage.InServiceofYalahar.Questline) == 54 then
			builder:say(npc, "Did you bring me the vampiric crest?", player)
			s.topic = 6
		else
			builder:say(npc, "I'm an Augur of the city of Yalahar. My special duty consists of coordinating the efforts to keep the city and its services running.", player)
		end
		return true

	elseif lower:find("vampiric crest", 1, true) and player:getStorageValue(Storage.InServiceofYalahar.Questline) ~= 54 then
		builder:say(npc, "I'm an Augur of the city of Yalahar. My special duty consists of coordinating the efforts to keep the city and its services running.", player)
		return true

	elseif lower:find("mission", 1, true) then
		...
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 5 then
			builder:say(npc, "So are you done with your work?", player)
			s.topic = 2
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 6 then
			...
			s.topic = 3
		elseif (player:getStorageValue(Storage.InServiceofYalahar.Questline) or 0) >= 7 and (player:getStorageValue(Storage.InServiceofYalahar.Questline) or 0) <= 14 then
			builder:say(npc, "Did you get all the reports my superiors asked for? ", player)
			s.topic = 4
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 15 then
			...
			player:setStorageValue(12272, 1) -- Access to research notes door
			s.topic = 0
		elseif (player:getStorageValue(Storage.InServiceofYalahar.NotesPalimuth) or 0) == 1 and player:getStorageValue(Storage.InServiceofYalahar.Questline) == 16 then
			builder:sayMultiple(npc, {
			...
			player:setStorageValue(12243, 1) -- Access to door to Azerus
			s.topic = 0
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 20 then
			...
			player:registerEvent("ServiceOfYalaharDiseasedTrio") -- Dynamic registration for diseased trio events
			s.topic = 0
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 23 then
			...
			s.topic = 0
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 25 and (player:getStorageValue(Storage.InServiceofYalahar.MrWestStatus) or 0) == 1 then
			builder:sayMultiple(npc, {
			...
			player:setStorageValue(Storage.InServiceofYalahar.GoodSide, (player:getStorageValue(Storage.InServiceofYalahar.GoodSide) or 0) + 1)
		...
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 28 then
			...
			s.topic = 0
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 32 and (player:getStorageValue(Storage.InServiceofYalahar.TamerinStatus) or 0) == 1 then
			builder:say(npc, "These are great news indeed. The people of Yalahar will be grateful. The Yalahari probably not, so take care of yourself. ", player)
			player:setStorageValue(Storage.InServiceofYalahar.GoodSide, (player:getStorageValue(Storage.InServiceofYalahar.GoodSide) or 0) + 1) -- Side Storage
		...
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 40 then
			...
			player:setStorageValue(12278, 1) -- Access to quara leaders door
			...
			player:registerEvent("ServiceOfYalaharQuaraLeaders") -- Dynamic registration for quara leaders events
			s.topic = 0
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 42 and (player:getStorageValue(Storage.InServiceofYalahar.QuaraState) or 0) == 1 then
			player:unregisterEvent("ServiceOfYalaharQuaraLeaders") -- Unregister in case it was registered by Yalahari
			builder:say(npc, "Oh no! So that's the reason for the quara attacks! I will do my best to close these sewage pipes. We will have to use other drains. ", player)
		...
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 44 then
			...
			player:setStorageValue(12277, 1) -- Access to food/weapon crystal door
			player:setStorageValue(12271, 1) -- Access to matrix door
		...
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 49 or player:getStorageValue(Storage.InServiceofYalahar.Questline) == 48 then
			builder:say(npc, "So do you want to side with me? ", player)
			s.topic = 5
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 50 and (player:getStorageValue(Storage.InServiceofYalahar.SideDecision) or 0) == 1 then
			builder:sayMultiple(npc, {
			...
			player:setStorageValue(12280, 1) -- Access to last fight door
			player:setStorageValue(Storage.InServiceofYalahar.Mission10, 2) -- StorageValue for Questlog "Mission 10: The Final Battle"
			player:registerEvent("ServiceOfYalaharAzerus") -- Dynamic registration for Azerus events
			s.topic = 0
		elseif player:getStorageValue(Storage.InServiceofYalahar.Questline) == 52 then
			...
			player:setStorageValue(12279, 1) -- Access to reward door
	...
	elseif lower == "yes" then
		...
		elseif s.topic == 2 then
			if player:getStorageValue(Storage.InServiceofYalahar.SewerPipe01) == 1 and player:getStorageValue(Storage.InServiceofYalahar.SewerPipe02) == 1 and player:getStorageValue(Storage.InServiceofYalahar.SewerPipe03) == 1 and player:getStorageValue(Storage.InServiceofYalahar.SewerPipe04) == 1 then
				...
				builder:say(npc, "Thank you very much. You have no idea how hard it was to find someone volunteering for that job. If you feel ready for further {missions}, just tell me.", player)
				s.topic = 0
			end
			return true
		...
		elseif s.topic == 4 then
			if player:getStorageValue(Storage.InServiceofYalahar.Questline) == 14 then
				...
				builder:say(npc, "Excellent! My superiors will be pleased to get these reports. I will for sure emphasise your efforts in this mission. Please come back soon to see if there are any more {missions} available for you. ", player)
				s.topic = 0
			else
				builder:say(npc, "Come back when you do.", player)
				s.topic = 0
			end
			return true
		elseif s.topic == 5 then
			...
			builder:say(npc, "I knew that you were smart enough to make the right decision! Your next mission will be a special one! ", player)
			s.topic = 0
			return true
		elseif s.topic == 6 then
			if player:getItemCount(9955) > 0 and (player:getStorageValue(Storage.InServiceofYalahar.Questline) or 0) >= 50 then
				player:removeItem(9955, 1) -- Consume the vampiric crest
				player:setStorageValue(Storage.InServiceofYalahar.Questline, 55)
				builder:say(npc, "Great! Here, take this yalaharian addon in a return.", player)
