-- A Sleeping Dragon - Converted from XML to Lua NpcType
-- Original XML: data/npc/A Sleeping Dragon.xml
-- Original Script: data/npc/scripts/A Sleeping Dragon.lua

local npcName = "A Sleeping Dragon"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a a sleeping dragon")
npcType:health(100)
...
npcType:outfit({lookTypeEx = 12358})
...
local function creatureSayCallback(cid, type, msg)
	...
	if player:getStorageValue(Storage.WrathoftheEmperor.Questline) == 27 then
		if(msg:upper() == "SOLOSARASATIQUARIUM") and player:getStorageValue(Storage.WrathoftheEmperor.InterdimensionalPotion) == 1 then
			npcHandler:say({
