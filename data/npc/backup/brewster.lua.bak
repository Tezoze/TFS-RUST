-- Brewster - Converted from XML to Lua NpcType
-- Original XML: data/npc/Brewster.xml
-- Original Script: data/npc/scripts/Brewster.lua

local npcName = "Brewster"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a brewster")
npcType:health(100)
...
npcType:outfit({lookType = 128, lookHead = 57, lookBody = 131, lookLegs = 132, lookFeet = 114})
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "adventurer") and msgcontains(msg, "stone") then
		...
		elseif player:getStorageValue(Storage.AdventurersGuild.FreeStone.Brewster) ~= 1 then
	...
	elseif msgcontains(msg, "stake") then
		...
		if stakeStorage == 9 and player:getItemCount(5941) == 0 then
			npcHandler:say('I think you have forgotten to bring your stake, pilgrim.', cid)
		elseif stakeStorage == 9 then
			npcHandler:say('Yes, I have been informed and haven\'t yet drunk enough to forget, so you can count yourself lucky. Are you prepared to <hicks> receive my line of the prayer?', cid)
			npcHandler.topic[cid] = 4
		elseif stakeStorage == 10 then
			npcHandler:say('You should visit Tyrias in Liberty Bay now.', cid)
		elseif stakeStorage > 10 then
			npcHandler:say('You have already received my line of the prayer. Don\'t make me do more work than necessary!', cid)
		else
			npcHandler:say('A blessed stake? That\'s a <hicks> strange request. Maybe Quentin knows more, he is one of the oldest monks after all.', cid)
		end
		return true
	...
	elseif npcHandler.topic[cid] == 4 and msgcontains(msg, "yes") then
		npcHandler:say('So receive my prayer: \'Your hand shall be guided - your feet shall walk in <hicks> harmony\'. Now, take your stake to Tyrias in <hicks> Liberty Bay for the next line of the prayer. I shall let him know what he is to do.', cid)
	...
	elseif npcHandler.topic[cid] == 4 and msgcontains(msg, "no") then
		npcHandler:say('I\'ll wait for you.', cid)
		npcHandler.topic[cid] = 0
		return true
	end
