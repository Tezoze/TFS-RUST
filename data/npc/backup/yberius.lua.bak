-- Yberius - Converted from XML to Lua NpcType
-- Original XML: data/npc/Yberius.xml
-- Original Script: data/npc/scripts/Yberius.lua

local npcName = "Yberius"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a yberius")
npcType:health(100)
...
npcType:outfit({lookType = 225, lookHead = 20, lookBody = 30, lookLegs = 40, lookFeet = 50})
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "adventurer") and msgcontains(msg, "stone") then
		...
		elseif player:getStorageValue(Storage.AdventurersGuild.FreeStone.Yberius) ~= 1 then
	...
	elseif msgcontains(msg, "stake") then
		...
		if stakeStorage == 4 and player:getItemCount(5941) == 0 then
		...
		elseif stakeStorage == 5 then
			npcHandler:say('You should visit Isimov in the dwarven settlement now.', cid)
		elseif stakeStorage > 5 then
	...
	if npcHandler.topic[cid] == 2 and msgcontains(msg, "yes") then
		...
		player:setStorageValue(Storage.AdventurersGuild.FreeStone.Yberius, 1)
		npcHandler.topic[cid] = 0
		return true
