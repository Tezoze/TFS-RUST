-- Lorietta - Converted from XML to Lua NpcType
-- Original XML: data/npc/Lorietta.xml
-- Original Script: data/npc/scripts/Lorietta.lua
...
npcType:nameDescription("a lorietta")
npcType:health(100)
...
npcType:outfit({lookType = 270, lookHead = 78, lookBody = 79, lookLegs = 80, lookFeet = 77, lookAddons = 1})
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "adventurer") and msgcontains(msg, "stone") then
		...
		elseif player:getStorageValue(Storage.AdventurersGuild.FreeStone.Lorietta) ~= 1 then
	...
	-- Blood crystal dialogue for Blood Brothers quest
	elseif msgcontains(msg, "blood crystal") then
		npcHandler:say('Ehehehe! A blood crystal? You must be a fool looking for one. I like fools. Why don\'t you check the cultist hideout in the magician quarter?', cid)
		return true
	...
	-- Handle yes/no responses for adventurer stone
