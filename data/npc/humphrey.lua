-- Humphrey - Converted from XML to Lua NpcType
-- Original XML: data/npc/Humphrey.xml
-- Original Script: data/npc/scripts/Humphrey.lua

local npcName = "Humphrey"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a humphrey")
npcType:health(100)
...
npcType:outfit({lookType = 133, lookBody = 115, lookLegs = 102, lookFeet = 95})
...
-- Embrace of Tibia
local blessKeyword = keywordHandler:addKeyword({'embrace'}, function(cid, message, keywords, parameters, node)
...
local missionKeyword = keywordHandler:addKeyword({'mission'}, function(cid, message, keywords, parameters, node)
	...
	if player:getStorageValue(Storage.PilgrimageOfAshes.Mission02) >= 2 then
		npcHandler:say("You have already received the embrace of Tibia. You should continue your pilgrimage to the next sacred place.", cid)
		return true
	end


	local blessCost = StdModule.calculateRegularBlessingCost(player:getLevel()) - 1000
	npcHandler:say('Child of nature on a blessed pilgrimage, would you like to receive the embrace of Tibia for ' .. blessCost .. ' gold coins?', cid)
	...
	missionKeyword:addChildKeyword({'yes'}, function(cid, message, keywords, parameters, node)
		...
		elseif not player:removeTotalMoney(blessCost) then
			...
			player:addBlessing(2)
			player:setStorageValue(Storage.PilgrimageOfAshes.Mission02, 2)
			player:setStorageValue(Storage.PilgrimageOfAshes.Questline, 3)
			player:setStorageValue(Storage.PilgrimageOfAshes.Mission03, 1)
			player:addMapMark(Position(32697, 31718, 7), MAPMARK_GREENSOUTH, "Suntower")
			npcHandler:say({
				"So receive the embrace of Tibia, pilgrim. This is the second of five available blessings. Let me tell you where you will find the third sacred place: ...",
				"Southeast of the Ab'Dendriel gates stands an old tree. This tree holds the suntower, where the mystic Edala will provide you with the blessing of the two suns. Crunor bless you on your pilgrimage."
			}, cid)
			player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
		end
		npcHandler.topic[cid] = 0
