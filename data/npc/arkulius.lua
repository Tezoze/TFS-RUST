-- Arkulius - Converted from XML to Lua NpcType
-- Original XML: data/npc/Arkulius.xml
-- Original Script: data/npc/scripts/Arkulius.lua

local npcName = "Arkulius"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a arkulius")
npcType:health(100)
...
npcType:outfit({lookType = 130, lookBody = 79, lookLegs = 90, lookFeet = 117, lookAddons = 3})
...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "mission") or msgcontains(msg, "quest") then
		local value = player:getStorageValue(GlobalStorage.ElementalSphere.QuestLine)
		if value < 1 then
		...
		elseif value == 1 then
			if player:getItemCount(player:isSorcerer() and 8304 or player:isDruid() and 8305 or player:isPaladin() and 8300 or player:isKnight() and 8306) > 0 then
				player:setStorageValue(GlobalStorage.ElementalSphere.QuestLine, 2)
				npcHandler:say({
			...
			npcHandler.topic[cid] = 0
		elseif value == 2 then
			if player:removeItem(8310, 1) and player:getStorageValue(GlobalStorage.ElementalSphere.QuestLine) < 3 then
				...
				player:setStorageValue(GlobalStorage.ElementalSphere.QuestLine, 3)
			end
		end
	elseif npcHandler.topic[cid] == 1 and msgcontains(msg, "yes") then
		player:setStorageValue(GlobalStorage.ElementalSphere.QuestLine, 1)
...
local shopItems = {
    {id = 8303, buy = 0, sell = 2000, subType = 0, name = "energy soil"},
    {id = 8304, buy = 0, sell = 5000, subType = 0, name = "eternal flames"},
    {id = 8300, buy = 0, sell = 5000, subType = 0, name = "flawless ice crystal"},
    {id = 8299, buy = 0, sell = 2000, subType = 0, name = "glimmering soil"},
    {id = 8305, buy = 0, sell = 5000, subType = 0, name = "mother soil"},
    {id = 8302, buy = 0, sell = 2000, subType = 0, name = "iced soil"},
    {id = 8298, buy = 0, sell = 2000, subType = 0, name = "natural soil"},
    {id = 8310, buy = 0, sell = 5000, subType = 0, name = "neutral matter"},
    {id = 8306, buy = 0, sell = 5000, subType = 0, name = "pure energy"},
}
