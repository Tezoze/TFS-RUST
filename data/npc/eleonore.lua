-- Eleonore - Converted from XML to Lua NpcType
-- Original XML: data/npc/Eleonore.xml
-- Original Script: data/npc/scripts/Eleonore.lua

local npcName = "Eleonore"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a eleonore")
npcType:health(100)
...
npcType:outfit({lookType = 140, lookHead = 114, lookBody = 66, lookLegs = 34, lookFeet = 53})
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "ring") or msgcontains(msg, "mission") then
		if player:getStorageValue(Storage.TheShatteredIsles.TheGovernorDaughter) < 1 then
			...
			player:setStorageValue(Storage.TheShatteredIsles.Questline, 1)
		...
		elseif player:getStorageValue(Storage.TheShatteredIsles.TheGovernorDaughter) == 1 then
			npcHandler:say("Have you found my ring yet? It was stolen by a parrot and hidden somewhere in the nearby mountains. You might need a rake to reach it.", cid)
			npcHandler.topic[cid] = 0
	...
	elseif msgcontains(msg, "here") then
		npcHandler:say("I live here in the governor's residence. My father is the governor of this island.", cid)
