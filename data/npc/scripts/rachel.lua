-- Generated by import-npcs (NPC-2). Do not edit by hand.
-- Source: rachel.npc

local npc = NpcType("Rachel")
npc:appearance({ lookType = 136, lookHead = 58, lookBody = 84, lookLegs = 86, lookFeet = 114 })
npc:movement({ radius = 1, speed = 5, goStrength = 5 })
npc:health(100)
npc:sex(0)
npc:race(1)

npc:dialogue(NpcDialogue({
	policy = "queued_single_focus",
	rules = {
		{
			line = 13,
			when = {
				{ situation = "address" },
				{ words = { "hello$" } },
				{ property = "sorcerer" },
				{ property = "male" },
				{ select = true },
			},
			actions = {
				{ say = "Welcome back, brother %N!" },
				{ set = { var = "topic", value = 1 } },
			},
		},
		{
			line = 14,
			when = {
				{ situation = "address" },
				{ words = { "hello$" } },
				{ property = "sorcerer" },
				{ property = "female" },
				{ select = true },
			},
			actions = {
				{ say = "Welcome back, sister %N! Isn't your name %N?" },
				{ set = { var = "topic", value = 1 } },
			},
		},
		{
			line = 15,
			when = {
				{ situation = "address" },
				{ words = { "hi$" } },
				{ property = "sorcerer" },
				{ property = "male" },
				{ select = true },
			},
			actions = {
				{ say = "Welcome back, brother %N! Wasn't your name %N?" },
				{ set = { var = "topic", value = 1 } },
			},
		},
		{
			line = 16,
			when = {
				{ situation = "address" },
				{ words = { "hi$" } },
				{ property = "sorcerer" },
				{ property = "female" },
				{ select = true },
			},
			actions = {
				{ say = "Welcome back, sister %N! Wasn't your name %N?" },
				{ set = { var = "topic", value = 1 } },
			},
		},
		{
			line = 17,
			when = {
				{ situation = "address" },
				{ words = { "hi$" } },
				{ select = true },
			},
			actions = {
				{ say = "Welcome %N! Whats your need?" },
			},
		},
		{
			line = 18,
			when = {
				{ situation = "address" },
				{ words = { "hello$" } },
				{ select = true },
			},
			actions = {
				{ say = "Welcome %N! Whats your need?" },
			},
		},
		{
			line = 19,
			when = {
				{ situation = "address" },
				{ select = true },
			},
			actions = {
				{ idle = true },
			},
		},
		{
			line = 20,
			when = {
				{ situation = "busy" },
				{ words = { "hello$" } },
				{ select = true },
			},
			actions = {
				{ say = "Wait, %N! One after the other." },
				{ queue = true },
			},
		},
		{
			line = 21,
			when = {
				{ situation = "busy" },
				{ words = { "hi$" } },
				{ select = true },
			},
			actions = {
				{ repeatPrevious = true },
			},
		},
		{
			line = 22,
			when = {
				{ situation = "busy" },
				{ select = true },
			},
			actions = {
				{ nop = true },
			},
		},
		{
			line = 23,
			when = {
				{ situation = "vanish" },
				{ select = true },
			},
			actions = {
				{ say = "These impatient young brats!" },
			},
		},
		{
			line = 25,
			when = {
				{ words = { "bye" } },
			},
			actions = {
				{ say = "Good bye, %N" },
				{ idle = true },
			},
		},
		{
			line = 26,
			when = {
				{ words = { "farewell" } },
			},
			actions = {
				{ repeatPrevious = true },
			},
		},
		{
			line = 27,
			when = {
				{ words = { "job" } },
			},
			actions = {
				{ say = "I am the head alchemist of Carlin. I keep the secret recipies of our ancestors. Besides, I am selling mana and life fluids, spellbooks, wands, rods and runes." },
			},
		},
		{
			line = 28,
			when = {
				{ words = { "name" } },
			},
			actions = {
				{ say = "I am the illusterous Rachel, of course." },
			},
		},
		{
			line = 29,
			when = {
				{ words = { "time" } },
			},
			actions = {
				{ say = "Time is of no meaning to us sorcerers." },
			},
		},
		{
			line = 30,
			when = {
				{ words = { "wisdom" } },
			},
			actions = {
				{ say = "Wisdom arises from patience." },
			},
		},
		{
			line = 31,
			when = {
				{ words = { "patience" } },
			},
			actions = {
				{ say = "You have to free yourself from unpatience to learn the deeper secrets of magic." },
			},
		},
		{
			line = 32,
			when = {
				{ words = { "ancestor" } },
			},
			actions = {
				{ say = "We are a guild of old traditions and even older secrets." },
			},
		},
		{
			line = 33,
			when = {
				{ words = { "sorcerer" } },
			},
			actions = {
				{ say = "Spells are the minor parts that make a sorcerer. To be one is a state of mind, not of a full spellbook." },
			},
		},
		{
			line = 34,
			when = {
				{ words = { "power" } },
			},
			actions = {
				{ say = "Power is important, but it is just the way, not the ultimate goal." },
			},
		},
		{
			line = 35,
			when = {
				{ words = { "goal" } },
			},
			actions = {
				{ say = "This secrect will be taught you by life, not by me." },
			},
		},
		{
			line = 36,
			when = {
				{ words = { "vocation" } },
			},
			actions = {
				{ say = "Your vocation is your profession. There are four vocations in Tibia: Sorcerers, paladins, knights, and druids." },
			},
		},
		{
			line = 37,
			when = {
				{ words = { "spell$" } },
			},
			actions = {
				{ say = "I am too busy to teach you, ask in your guild about that." },
			},
		},
		{
			line = 38,
			when = {
				{ words = { "spells" } },
			},
			actions = {
				{ repeatPrevious = true },
			},
		},
		{
			line = 40,
			when = {
				{ words = { "rune" } },
			},
			actions = {
				{ say = "I sell blank runes and spell runes." },
			},
		},
		{
			line = 41,
			when = {
				{ words = { "spellbook" } },
			},
			actions = {
				{ set = { var = "type", value = 2175 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 150 } },
				{ say = "A spellbook is a nice tool for beginners. Do you want to buy one for %P gold?" },
				{ set = { var = "topic", value = 2 } },
			},
		},
		{
			line = 42,
			when = {
				{ words = { "blank" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2260 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 10 } },
				{ say = "Do you want to buy one for %P gold?" },
				{ set = { var = "topic", value = 2 } },
			},
		},
		{
			line = 43,
			when = {
				{ words = { "mana" } },
				{ words = { "fluid" } },
			},
			actions = {
				{ set = { var = "type", value = 2006 } },
				{ set = { var = "data", value = 10 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 55 } },
				{ say = "Do you want to buy mana fluid for %P gold?" },
				{ set = { var = "topic", value = 3 } },
			},
		},
		{
			line = 44,
			when = {
				{ words = { "life" } },
				{ words = { "fluid" } },
			},
			actions = {
				{ set = { var = "type", value = 2006 } },
				{ set = { var = "data", value = 11 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 60 } },
				{ say = "Do you want to buy life fluid for %P gold?" },
				{ set = { var = "topic", value = 3 } },
			},
		},
		{
			line = 46,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "spellbook" } },
			},
			actions = {
				{ set = { var = "type", value = 2175 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 150, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A spellbooks for %P gold?" },
				{ set = { var = "topic", value = 2 } },
			},
		},
		{
			line = 47,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "blank" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2260 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 10, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A runes for %P gold?" },
				{ set = { var = "topic", value = 2 } },
			},
		},
		{
			line = 48,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "mana" } },
				{ words = { "fluid" } },
			},
			actions = {
				{ set = { var = "type", value = 2006 } },
				{ set = { var = "data", value = 10 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 55, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A potions of mana fluid for %P gold?" },
				{ set = { var = "topic", value = 3 } },
			},
		},
		{
			line = 49,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "life" } },
				{ words = { "fluid" } },
			},
			actions = {
				{ set = { var = "type", value = 2006 } },
				{ set = { var = "data", value = 11 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 60, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A potions of life fluid for %P gold?" },
				{ set = { var = "topic", value = 3 } },
			},
		},
		{
			line = 51,
			when = {
				{ words = { "sell" } },
				{ words = { "talon" } },
			},
			actions = {
				{ set = { var = "type", value = 2151 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 320 } },
				{ say = "Do you want to sell one of the magic gems called talon for %P gold?" },
				{ set = { var = "topic", value = 6 } },
			},
		},
		{
			line = 52,
			when = {
				{ words = { "sell" } },
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "talon" } },
			},
			actions = {
				{ set = { var = "type", value = 2151 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 320, rhs = { capture = 1 } } } } },
				{ say = "Do you want to sell %A magic gems called talon for %P gold?" },
				{ set = { var = "topic", value = 6 } },
			},
		},
		{
			line = 54,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 1 },
				{ words = { "yes" } },
			},
			actions = {
				{ say = "I thought so, what do you want?" },
			},
		},
		{
			line = 55,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 1 },
				{ words = { "no" } },
			},
			actions = {
				{ say = "First lesson: DON'T LIE TO RACHEL!" },
				{ burning = { cycles = 10, param = 4 } },
				{ effectMe = 15 },
				{ effectOpp = 16 },
			},
		},
		{
			line = 56,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 1 },
				{ property = "sorcerer" },
			},
			actions = {
				{ say = "I thought only intelligent persons are allowed to become sorcerers." },
			},
		},
		{
			line = 57,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 1 },
			},
			actions = {
				{ say = "I am glad that only intelligent persons are allowed to become sorcerers." },
			},
		},
		{
			line = 59,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 2 },
				{ words = { "yes" } },
				{ expr = { countMoney = true }, op = ">=", rhs = { session = "price" } },
			},
			actions = {
				{ say = "Here you are." },
				{ deleteMoney = true },
				{ create = { item = { session = "type" }, count = 1 } },
			},
		},
		{
			line = 60,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 2 },
				{ words = { "yes" } },
			},
			actions = {
				{ say = "Come back when you have enough money." },
			},
		},
		{
			line = 61,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 2 },
			},
			actions = {
				{ say = "Hmm, maybe next time." },
			},
		},
		{
			line = 63,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 3 },
				{ words = { "yes" } },
				{ expr = { countMoney = true }, op = ">=", rhs = { session = "price" } },
			},
			actions = {
				{ say = "Here you are. There is a deposit of 5 gold for an empty vial." },
				{ deleteMoney = true },
				{ create = { item = { session = "type" }, count = 1 } },
			},
		},
		{
			line = 64,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 3 },
				{ words = { "yes" } },
			},
			actions = {
				{ say = "Come back, when you have enough money." },
			},
		},
		{
			line = 65,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 3 },
			},
			actions = {
				{ say = "Hmm, but next time." },
			},
		},
		{
			line = 67,
			when = {
				{ words = { "deposit" } },
			},
			actions = {
				{ say = "I will pay you 5 gold for every empty vial. Ok?" },
				{ set = { var = "data", value = 0 } },
				{ set = { var = "topic", value = 5 } },
			},
		},
		{
			line = 68,
			when = {
				{ words = { "vial" } },
			},
			actions = {
				{ repeatPrevious = true },
			},
		},
		{
			line = 69,
			when = {
				{ words = { "flask" } },
			},
			actions = {
				{ repeatPrevious = true },
			},
		},
		{
			line = 70,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 5 },
				{ words = { "yes" } },
				{ expr = { count = 2006 }, op = ">", rhs = 0 },
			},
			actions = {
				{ set = { var = "amount", value = { count = 2006 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = { session = "amount" }, rhs = 5 } } } },
				{ say = "Here you are ... %P gold." },
				{ delete = { item = 2006, count = 1 } },
				{ createMoney = true },
			},
		},
		{
			line = 71,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 5 },
				{ words = { "yes" } },
			},
			actions = {
				{ say = "You don't have any empty vials." },
			},
		},
		{
			line = 72,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 5 },
			},
			actions = {
				{ say = "Hmm, but please keep Tibia litter free." },
			},
		},
		{
			line = 74,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 6 },
				{ words = { "yes" } },
				{ expr = { count = { session = "type" } }, op = ">=", rhs = { session = "amount" } },
			},
			actions = {
				{ say = "Ok. Here is your money." },
				{ delete = { item = { session = "type" }, count = 1 } },
				{ createMoney = true },
			},
		},
		{
			line = 75,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 6 },
				{ words = { "yes" } },
			},
			actions = {
				{ say = "Sorry, you do not have one." },
			},
		},
		{
			line = 76,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 6 },
				{ words = { "yes" } },
				{ expr = { session = "amount" }, op = ">", rhs = 1 },
			},
			actions = {
				{ say = "Sorry, you do not have so many." },
			},
		},
		{
			line = 77,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 6 },
			},
			actions = {
				{ say = "Maybe next time." },
			},
		},
		{
			line = 5,
			when = {
				{ words = { "spell" } },
				{ words = { "rune" } },
			},
			actions = {
				{ say = "I sell missile runes, explosive runes, field runes, wall runes, bomb runes, healing runes, convince creature runes and chameleon runes." },
			},
		},
		{
			line = 6,
			when = {
				{ words = { "missile" } },
				{ words = { "rune" } },
			},
			actions = {
				{ say = "I can offer you light magic missile runes, heavy magic missile runes and sudden death runes." },
			},
		},
		{
			line = 7,
			when = {
				{ words = { "explosive" } },
				{ words = { "rune" } },
			},
			actions = {
				{ say = "I can offer you fireball runes, great fireball runes and explosion runes." },
			},
		},
		{
			line = 8,
			when = {
				{ words = { "field" } },
				{ words = { "rune" } },
			},
			actions = {
				{ say = "I can offer you fire field runes, energy field runes, poison field runes and destroy field runes." },
			},
		},
		{
			line = 9,
			when = {
				{ words = { "wall" } },
				{ words = { "rune" } },
			},
			actions = {
				{ say = "I can offer you fire wall runes, energy wall runes and poison wall runes." },
			},
		},
		{
			line = 10,
			when = {
				{ words = { "bomb" } },
				{ words = { "rune" } },
			},
			actions = {
				{ say = "I can offer you firebomb runes." },
			},
		},
		{
			line = 11,
			when = {
				{ words = { "healing" } },
				{ words = { "rune" } },
			},
			actions = {
				{ say = "I can offer you antidote runes, intense healing runes and ultimate healing runes." },
			},
		},
		{
			line = 13,
			when = {
				{ words = { "light" } },
				{ words = { "magic" } },
				{ words = { "missile" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2287 } },
				{ set = { var = "data", value = 5 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 40 } },
				{ say = "Do you want to buy a light magic missile rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 14,
			when = {
				{ words = { "poison" } },
				{ words = { "field" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2285 } },
				{ set = { var = "data", value = 3 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 65 } },
				{ say = "Do you want to buy a poison field rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 15,
			when = {
				{ words = { "antidote" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2266 } },
				{ set = { var = "data", value = 1 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 65 } },
				{ say = "Do you want to buy an antidote rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 16,
			when = {
				{ words = { "fire" } },
				{ words = { "field" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2301 } },
				{ set = { var = "data", value = 3 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 85 } },
				{ say = "Do you want to buy a fire field rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 17,
			when = {
				{ words = { "intense" } },
				{ words = { "healing" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2265 } },
				{ set = { var = "data", value = 1 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 95 } },
				{ say = "Do you want to buy an intense healing rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 18,
			when = {
				{ words = { "fireball" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2302 } },
				{ set = { var = "data", value = 2 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 95 } },
				{ say = "Do you want to buy a fireball rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 19,
			when = {
				{ words = { "destroy" } },
				{ words = { "field" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2261 } },
				{ set = { var = "data", value = 3 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 45 } },
				{ say = "Do you want to buy a destroy field rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 20,
			when = {
				{ words = { "heavy" } },
				{ words = { "magic" } },
				{ words = { "missile" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2311 } },
				{ set = { var = "data", value = 5 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 125 } },
				{ say = "Do you want to buy a heavy magic missile rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 21,
			when = {
				{ words = { "energy" } },
				{ words = { "field" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2277 } },
				{ set = { var = "data", value = 3 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 115 } },
				{ say = "Do you want to buy an energy field rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 22,
			when = {
				{ words = { "ultimate" } },
				{ words = { "healing" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2273 } },
				{ set = { var = "data", value = 1 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 175 } },
				{ say = "Do you want to buy an ultimate healing rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 23,
			when = {
				{ words = { "convince" } },
				{ words = { "creature" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2290 } },
				{ set = { var = "data", value = 1 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 80 } },
				{ say = "Do you want to buy a convince creature rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 24,
			when = {
				{ words = { "great" } },
				{ words = { "fireball" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2304 } },
				{ set = { var = "data", value = 2 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 180 } },
				{ say = "Do you want to buy a great fireball rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 25,
			when = {
				{ words = { "chameleon" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2291 } },
				{ set = { var = "data", value = 1 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 210 } },
				{ say = "Do you want to buy a chameleon rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 26,
			when = {
				{ words = { "fire" } },
				{ words = { "bomb" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2305 } },
				{ set = { var = "data", value = 2 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 235 } },
				{ say = "Do you want to buy a firebomb rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 27,
			when = {
				{ words = { "poison" } },
				{ words = { "wall" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2289 } },
				{ set = { var = "data", value = 4 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 210 } },
				{ say = "Do you want to buy a poison wall rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 28,
			when = {
				{ words = { "explosion" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2313 } },
				{ set = { var = "data", value = 3 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 250 } },
				{ say = "Do you want to buy an explosion rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 29,
			when = {
				{ words = { "fire" } },
				{ words = { "wall" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2303 } },
				{ set = { var = "data", value = 4 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 245 } },
				{ say = "Do you want to buy a fire wall rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 30,
			when = {
				{ words = { "sudden" } },
				{ words = { "death" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2268 } },
				{ set = { var = "data", value = 1 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 325 } },
				{ say = "Do you want to buy a sudden death rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 31,
			when = {
				{ words = { "energy" } },
				{ words = { "wall" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2279 } },
				{ set = { var = "data", value = 4 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 340 } },
				{ say = "Do you want to buy an energy wall rune for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 33,
			when = {
				{ words = { "envenom" } },
				{ words = { "rune" } },
			},
			actions = {
				{ say = "Sorry, but runes of this type can't be purchased here." },
			},
		},
		{
			line = 34,
			when = {
				{ words = { "desintegrate" } },
				{ words = { "rune" } },
			},
			actions = {
				{ repeatPrevious = true },
			},
		},
		{
			line = 35,
			when = {
				{ words = { "poison" } },
				{ words = { "bomb" } },
				{ words = { "rune" } },
			},
			actions = {
				{ repeatPrevious = true },
			},
		},
		{
			line = 36,
			when = {
				{ words = { "soulfire" } },
				{ words = { "rune" } },
			},
			actions = {
				{ repeatPrevious = true },
			},
		},
		{
			line = 37,
			when = {
				{ words = { "energy" } },
				{ words = { "bomb" } },
				{ words = { "rune" } },
			},
			actions = {
				{ repeatPrevious = true },
			},
		},
		{
			line = 38,
			when = {
				{ words = { "magic" } },
				{ words = { "wall" } },
				{ words = { "rune" } },
			},
			actions = {
				{ repeatPrevious = true },
			},
		},
		{
			line = 39,
			when = {
				{ words = { "animate" } },
				{ words = { "dead" } },
				{ words = { "rune" } },
			},
			actions = {
				{ repeatPrevious = true },
			},
		},
		{
			line = 40,
			when = {
				{ words = { "paralyze" } },
				{ words = { "rune" } },
			},
			actions = {
				{ repeatPrevious = true },
			},
		},
		{
			line = 42,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "light" } },
				{ words = { "magic" } },
				{ words = { "missile" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2287 } },
				{ set = { var = "data", value = 5 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 40, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A light magic missile runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 43,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "poison" } },
				{ words = { "field" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2285 } },
				{ set = { var = "data", value = 3 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 65, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A poison field runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 44,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "antidote" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2266 } },
				{ set = { var = "data", value = 1 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 65, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A antidote runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 45,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "fire" } },
				{ words = { "field" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2301 } },
				{ set = { var = "data", value = 3 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 85, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A fire field runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 46,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "intense" } },
				{ words = { "healing" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2265 } },
				{ set = { var = "data", value = 1 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 95, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A intense healing runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 47,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "fireball" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2302 } },
				{ set = { var = "data", value = 2 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 95, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A fireball runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 48,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "destroy" } },
				{ words = { "field" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2261 } },
				{ set = { var = "data", value = 3 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 45, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A destroy field runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 49,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "heavy" } },
				{ words = { "magic" } },
				{ words = { "missile" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2311 } },
				{ set = { var = "data", value = 5 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 125, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A heavy magic missile runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 50,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "energy" } },
				{ words = { "field" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2277 } },
				{ set = { var = "data", value = 3 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 115, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A energy field runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 51,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "ultimate" } },
				{ words = { "healing" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2273 } },
				{ set = { var = "data", value = 1 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 175, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A ultimate healing runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 52,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "convince" } },
				{ words = { "creature" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2290 } },
				{ set = { var = "data", value = 1 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 80, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A convince creature runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 53,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "great" } },
				{ words = { "fireball" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2304 } },
				{ set = { var = "data", value = 2 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 180, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A great fireball runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 54,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "chameleon" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2291 } },
				{ set = { var = "data", value = 1 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 210, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A chameleon runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 55,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "fire" } },
				{ words = { "bomb" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2305 } },
				{ set = { var = "data", value = 2 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 235, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A firebomb runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 56,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "poison" } },
				{ words = { "wall" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2289 } },
				{ set = { var = "data", value = 4 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 210, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A poison wall runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 57,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "explosion" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2313 } },
				{ set = { var = "data", value = 3 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 250, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A explosion runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 58,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "fire" } },
				{ words = { "wall" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2303 } },
				{ set = { var = "data", value = 4 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 245, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A fire wall runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 59,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "sudden" } },
				{ words = { "death" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2268 } },
				{ set = { var = "data", value = 1 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 325, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A sudden death runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 60,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "energy" } },
				{ words = { "wall" } },
				{ words = { "rune" } },
			},
			actions = {
				{ set = { var = "type", value = 2279 } },
				{ set = { var = "data", value = 4 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 340, rhs = { capture = 1 } } } } },
				{ say = "Do you want to buy %A energy wall runes for %P gold?" },
				{ set = { var = "topic", value = 99 } },
			},
		},
		{
			line = 63,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 99 },
				{ words = { "yes" } },
				{ expr = { countMoney = true }, op = ">=", rhs = { session = "price" } },
			},
			actions = {
				{ say = "Here you are." },
				{ deleteMoney = true },
				{ create = { item = { session = "type" }, count = 1 } },
			},
		},
		{
			line = 64,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 99 },
				{ words = { "yes" } },
			},
			actions = {
				{ say = "Sorry, you don't have enough gold." },
			},
		},
		{
			line = 65,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 99 },
			},
			actions = {
				{ say = "As you wish." },
			},
		},
		{
			line = 5,
			when = {
				{ words = { "wand" } },
			},
			actions = {
				{ say = "Wands can be wielded by sorcerers only and have a certain level requirement. There are five different wands, would you like to hear about them?" },
				{ set = { var = "topic", value = 90 } },
			},
		},
		{
			line = 6,
			when = {
				{ words = { "rod" } },
			},
			actions = {
				{ say = "Rods can be wielded by druids only and have a certain level requirement. There are five different rods, would you like to hear about them?" },
				{ set = { var = "topic", value = 91 } },
			},
		},
		{
			line = 8,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 90 },
				{ words = { "yes" } },
			},
			actions = {
				{ say = "The names of the wands are 'Wand of Vortex', 'Wand of Dragonbreath', 'Wand of Plague', 'Wand of Cosmic Energy' and 'Wand of Inferno'. Which one would you like to buy?" },
			},
		},
		{
			line = 9,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 90 },
				{ words = { "no" } },
			},
			actions = {
				{ say = "Maybe another time." },
			},
		},
		{
			line = 10,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 90 },
			},
			actions = {
				{ say = "Maybe another time." },
			},
		},
		{
			line = 12,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 91 },
				{ words = { "yes" } },
			},
			actions = {
				{ say = "The names of the rods are 'Snakebite Rod', 'Moonlight Rod', 'Volcanic Rod', 'Quagmire Rod', and 'Tempest Rod'. Which one would you like to buy?" },
			},
		},
		{
			line = 13,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 91 },
				{ words = { "no" } },
			},
			actions = {
				{ say = "Maybe another time." },
			},
		},
		{
			line = 14,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 91 },
			},
			actions = {
				{ say = "Maybe another time." },
			},
		},
		{
			line = 16,
			when = {
				{ property = "sorcerer" },
				{ words = { "wand" } },
				{ words = { "of" } },
				{ words = { "vortex" } },
				{ expr = { questValue = 333 }, op = "<", rhs = 1 },
			},
			actions = {
				{ say = "Oh, is this your first wand of vortex? Take this little present from me as a free sample!" },
				{ setQuestValue = { id = 333, value = 1 } },
				{ set = { var = "type", value = 2190 } },
				{ set = { var = "amount", value = 1 } },
				{ create = { item = { session = "type" }, count = 1 } },
			},
		},
		{
			line = 17,
			when = {
				{ property = "druid" },
				{ words = { "snakebite" } },
				{ words = { "rod" } },
				{ expr = { questValue = 333 }, op = "<", rhs = 1 },
			},
			actions = {
				{ say = "Oh, is this your first snakebite rod? Take this little present from me as a free sample!" },
				{ setQuestValue = { id = 333, value = 1 } },
				{ set = { var = "type", value = 2182 } },
				{ set = { var = "amount", value = 1 } },
				{ create = { item = { session = "type" }, count = 1 } },
			},
		},
		{
			line = 19,
			when = {
				{ words = { "wand" } },
				{ words = { "of" } },
				{ words = { "vortex" } },
			},
			actions = {
				{ set = { var = "type", value = 2190 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 500 } },
				{ say = "This wand is only for sorcerers of level 7 and above. Would you like to buy a wand of vortex for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 20,
			when = {
				{ words = { "wand" } },
				{ words = { "of" } },
				{ words = { "dragonbreath" } },
			},
			actions = {
				{ set = { var = "type", value = 2191 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 1000 } },
				{ say = "This wand is only for sorcerers of level 13 and above. Would you like to buy a wand of dragonbreath for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 21,
			when = {
				{ words = { "wand" } },
				{ words = { "of" } },
				{ words = { "plague" } },
			},
			actions = {
				{ set = { var = "type", value = 2188 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 5000 } },
				{ say = "This wand is only for sorcerers of level 19 and above. Would you like to buy a wand of plague for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 22,
			when = {
				{ words = { "wand" } },
				{ words = { "of" } },
				{ words = { "cosmic" } },
				{ words = { "energy" } },
			},
			actions = {
				{ set = { var = "type", value = 2189 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 10000 } },
				{ say = "This wand is only for sorcerers of level 26 and above. Would you like to buy a wand of cosmic energy for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 23,
			when = {
				{ words = { "wand" } },
				{ words = { "of" } },
				{ words = { "inferno" } },
			},
			actions = {
				{ say = "Sorry, this wand contains magic far too powerful and we are afraid to store it here. I heard they have a few of these at the Edron academy though." },
			},
		},
		{
			line = 25,
			when = {
				{ words = { "snakebite" } },
				{ words = { "rod" } },
			},
			actions = {
				{ set = { var = "type", value = 2182 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 500 } },
				{ say = "This rod is only for druids of level 7 and above. Would you like to buy a snakebite rod for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 26,
			when = {
				{ words = { "moonlight" } },
				{ words = { "rod" } },
			},
			actions = {
				{ set = { var = "type", value = 2186 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 1000 } },
				{ say = "This rod is only for druids of level 13 and above. Would you like to buy a moonlight rod for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 27,
			when = {
				{ words = { "volcanic" } },
				{ words = { "rod" } },
			},
			actions = {
				{ set = { var = "type", value = 2185 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 5000 } },
				{ say = "This rod is only for druids of level 19 and above. Would you like to buy a volcanic rod for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 28,
			when = {
				{ words = { "quagmire" } },
				{ words = { "rod" } },
			},
			actions = {
				{ set = { var = "type", value = 2181 } },
				{ set = { var = "amount", value = 1 } },
				{ set = { var = "price", value = 10000 } },
				{ say = "This rod is only for druids of level 26 and above. Would you like to buy a quagmire rod for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 29,
			when = {
				{ words = { "tempest" } },
				{ words = { "rod" } },
			},
			actions = {
				{ say = "Sorry, this rod contains magic far too powerful and we are afraid to store it here. I heard they have a few of these at the Edron academy though." },
			},
		},
		{
			line = 31,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "wand" } },
				{ words = { "of" } },
				{ words = { "vortex" } },
			},
			actions = {
				{ set = { var = "type", value = 2190 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 500, rhs = { capture = 1 } } } } },
				{ say = "This wand is only for sorcerers of level 7 and above. Would you like to buy %A wands of vortex for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 32,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "wand" } },
				{ words = { "of" } },
				{ words = { "dragonbreath" } },
			},
			actions = {
				{ set = { var = "type", value = 2191 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 1000, rhs = { capture = 1 } } } } },
				{ say = "This wand is only for sorcerers of level 13 and above. Would you like to buy %A wands of dragonbreath for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 33,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "wand" } },
				{ words = { "of" } },
				{ words = { "plague" } },
			},
			actions = {
				{ set = { var = "type", value = 2188 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 5000, rhs = { capture = 1 } } } } },
				{ say = "This wand is only for sorcerers of level 19 and above. Would you like to buy %A wands of plague for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 34,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "wand" } },
				{ words = { "of" } },
				{ words = { "cosmic" } },
				{ words = { "energy" } },
			},
			actions = {
				{ set = { var = "type", value = 2189 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 10000, rhs = { capture = 1 } } } } },
				{ say = "This wand is only for sorcerers of level 26 and above. Would you like to buy %A wands of cosmic energy for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 35,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "wand" } },
				{ words = { "of" } },
				{ words = { "inferno" } },
			},
			actions = {
				{ say = "Sorry, this wand contains far too powerful magic and we are afraid to store it here. I heard they have a few of these at the Edron academy though." },
			},
		},
		{
			line = 38,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "snakebite" } },
				{ words = { "rod" } },
			},
			actions = {
				{ set = { var = "type", value = 2182 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 500, rhs = { capture = 1 } } } } },
				{ say = "This rod is only for druids of level 7 and above. Would you like to buy %A snakebite rods for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 39,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "moonlight" } },
				{ words = { "rod" } },
			},
			actions = {
				{ set = { var = "type", value = 2186 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 1000, rhs = { capture = 1 } } } } },
				{ say = "This rod is only for druids of level 13 and above. Would you like to buy %A moonlight rods for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 40,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "volcanic" } },
				{ words = { "rod" } },
			},
			actions = {
				{ set = { var = "type", value = 2185 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 5000, rhs = { capture = 1 } } } } },
				{ say = "This rod is only for druids of level 19 and above. Would you like to buy %A volcanic rods for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 41,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "quagmire" } },
				{ words = { "rod" } },
			},
			actions = {
				{ set = { var = "type", value = 2181 } },
				{ set = { var = "amount", value = { capture = 1 } } },
				{ set = { var = "price", value = { binary = { op = "*", lhs = 10000, rhs = { capture = 1 } } } } },
				{ say = "This rod is only for druids of level 26 and above. Would you like to buy %A quagmire rods for %P gold?" },
				{ set = { var = "topic", value = 92 } },
			},
		},
		{
			line = 42,
			when = {
				{ capture = 1 },
				{ expr = 1, op = "<", rhs = { capture = 1 } },
				{ words = { "tempest" } },
				{ words = { "rod" } },
			},
			actions = {
				{ say = "Sorry, this rod contains far too powerful magic and we are afraid to store it here. I heard they have a few of these at the Edron academy though." },
			},
		},
		{
			line = 45,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 92 },
				{ words = { "yes" } },
				{ expr = { countMoney = true }, op = ">=", rhs = { session = "price" } },
			},
			actions = {
				{ say = "Here you are." },
				{ deleteMoney = true },
				{ create = { item = { session = "type" }, count = 1 } },
			},
		},
		{
			line = 46,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 92 },
				{ words = { "yes" } },
			},
			actions = {
				{ say = "Sorry, you don't have enough gold." },
			},
		},
		{
			line = 47,
			when = {
				{ expr = { session = "topic" }, op = "=", rhs = 92 },
			},
			actions = {
				{ say = "You don't know what you're missing." },
			},
		},
	},
}))

npc:register()
