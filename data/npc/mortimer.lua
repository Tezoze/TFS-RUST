-- Mortimer - Converted from XML to Lua NpcType
-- Original XML: data/npc/Mortimer.xml
-- Original Script: data/npc/scripts/Mortimer.lua

local npcName = "Mortimer"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a mortimer")
npcType:health(100)
...
local function creatureSayCallback(cid, type, msg)
    ...
    elseif msgcontains(msg, "mission") then
        ...
        -- Logic: If joined (status > 3) and questline is active but below Rank 2 threshold (26)
        if joinStatus > 3 and questLine >= 4 and questLine < 26 then
            local missions = {}
        ...
        -- Rank 4: Explorer (Astral Travel & Ectoplasm)
        ...
        elseif questLine == 49 then
             -- Logic gap filler: If player has dress but not handed in yet, same as 48
             npcHandler:say("Did you bring the dress?", cid)
             npcHandler.topic[cid] = 31

        -- Spectral Stone / Mail Mission
