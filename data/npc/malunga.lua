-- Malunga - Converted from XML to Lua NpcType
-- Original XML: data/npc/Malunga.xml
-- Original Script: data/npc/scripts/Malunga.lua

local npcName = "Malunga"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a malunga")
npcType:health(100)
...
npcType:outfit({lookType = 149, lookHead = 95, lookBody = 78, lookLegs = 19})
...
local function creatureSayCallback(cid, type, msg)
    ...
    if m == 'attack spells' or m == 'attack' or m == 'healing spells' or m == 'healing' or m == 'support spells' or m == 'support' or m == 'conjure spells' or m == 'conjure' then
        ...
        for _, spell in ipairs(spells) do
            -- Filter logic:
            -- 1. Must match the category (attack/healing/support)
...
local shopItems = {
    ...
    {id = 11328, buy = 0, sell = 110, subType = 0, name = "widows mandibles"},
