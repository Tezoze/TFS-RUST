-- Ongulf - Converted from XML to Lua NpcType
-- Original XML: data/npc/Ongulf.xml
-- Original Script: data/npc/scripts/Ongulf.lua

local npcName = "Ongulf"
local npcType = Game.createNpcType(npcName)

-- NPC Properties (from XML)
npcType:name(npcName)
npcType:nameDescription("a ongulf")
npcType:health(100)
...
local function getHandlers(npc)
	...
	if not npcHandlers[key] then
		...
		npcHandlers[key].npcHandler:setMessage(MESSAGE_GREET, "Hello, |PLAYERNAME|. You've come at a good time for our {project}.")
...
creatureSayCallback = function(cid, type, msg)
	...
	elseif msgcontains(msg, "mission") then
		...
		elseif player:getStorageValue(Storage.TheNewFrontier.Questline) == 11 then
			...
			player:registerEvent("NewFrontierShardOfCorruption") -- Dynamic registration for shard of corruption events
			npcHandler.topic[cid] = 0
		elseif player:getStorageValue(Storage.TheNewFrontier.Questline) == 13 then
			...
			player:unregisterEvent("NewFrontierShardOfCorruption")
			npcHandler.topic[cid] = 0
		...
		elseif player:getStorageValue(Storage.TheNewFrontier.Questline) == 27 then
			...
			player:setStorageValue(Storage.TheNewFrontier.Mission01, 3) --Questlog, The New Frontier Quest "Mission 01: New Land" (ensure complete for token users)
