-- Walter, The Guard - Converted from XML to Lua NpcType
-- Original XML: data/npc/Walter, The Guard.xml
-- Original Script: data/npc/scripts/Walter, The Guard.lua
...
npcType:nameDescription("a walter, the guard")
npcType:health(100)
...
npcType:outfit({lookType = 131, lookHead = 19, lookBody = 19, lookLegs = 38, lookFeet = 38})
...
local function creatureSayCallback(cid, type, msg)
	...
	if(msgcontains(msg, "trouble") and (player:getStorageValue(Storage.TheInquisition.WalterGuard) or 0) < 1 and (player:getStorageValue(Storage.TheInquisition.Mission01) or 0) ~= -1) then
	...
	elseif(msgcontains(msg, "gods allow")) then
		if(npcHandler.topic[cid] == 3) then
			...
			if((player:getStorageValue(Storage.TheInquisition.WalterGuard) or 0) < 1) then
