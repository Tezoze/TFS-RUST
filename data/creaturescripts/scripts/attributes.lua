...
function elementalDmg(weaponDescription, elementType, extraAnimation, creature, resistances, primaryDamage, primaryType, secondaryDamage, secondaryType, elementalroll)
	...
	-- Check if the weapon has this elemental damage attribute
	if not dmgmin or not dmgmax then
		-- No elemental damage of this type on this item, return original values unchanged
	...
	if eleDmg ~= 0 then -- only do this if damage is actually applied
		...
		-- Check if this is a wand/rod (pure elemental, no secondary damage component)
		local isWandOrRod = (secondaryType == 0 and secondaryDamage == 0 and primaryType ~= COMBAT_PHYSICALDAMAGE and primaryType ~= 0)
		
		if isWandOrRod then
			-- Wand/rod: add elemental bonus directly to primary damage if same element
			if primaryType == elementType then
				primaryDamage = primaryDamage + eleDmg
			else
				-- Different element, add as secondary
				secondaryDamage = eleDmg
				secondaryType = elementType
			end
		elseif elementalroll then
			-- Previous elemental damage (types were swapped)
			if primaryType == elementType then -- 'secondary' damage is the same, just add to it
				...
				if primaryType == 0 then -- is this a wand? (after swap)
		...
		else -- This is the first elemental roll on this weapon (melee with elemental bonus)
	...
	if attacker then -- Ignore HP changes from healing fountains, terrain elemental fields (neutral sources basically)
		if attacker:isPlayer() then
			if origin == ORIGIN_RANGED or origin == ORIGIN_MELEE then -- Skip if player is using spells
				...
				-- BUT skip swap for wands/rods (pure elemental weapons with no secondary damage)
				-- Wands have: primaryType = elemental, secondaryType = 0, secondaryDamage = 0
				local isWandOrRod = (secondaryType == 0 and secondaryDamage == 0 and primaryType ~= COMBAT_PHYSICALDAMAGE and primaryType ~= 0)
				
				if not isWandOrRod and (secondaryType ~= 0 or primaryType ~= COMBAT_PHYSICALDAMAGE) then
				...
				for i = 1,#checkweaponslots do -- Roll for each slot
					if attacker:getSlotItem(checkweaponslots[i]) ~= nil then
						...
						--]]
							...
							-- Check unified elemental damage (adapts to weapon's current element)
							if slotitemdesc:find "%[Enhanced Elemental Damage" then
								-- Detect what element the weapon currently deals
								local weaponElement = nil
								if elementalroll then
									-- Types were already swapped: primaryType is now the weapon's element
									weaponElement = primaryType
								elseif isWandOrRod then
									weaponElement = primaryType
								else
									-- Check if the weapon itself has a native element via its ItemType
									local weaponItemType = ItemType(slotitem:getId())
									local weaponWpType = weaponItemType:getWeaponType()
									if weaponWpType > 0 and weaponWpType ~= WEAPON_SHIELD then
										-- Check XML-defined element damage
										local elemTypes = {COMBAT_FIREDAMAGE, COMBAT_ICEDAMAGE, COMBAT_ENERGYDAMAGE, COMBAT_EARTHDAMAGE}
										for _, et in ipairs(elemTypes) do
											if weaponItemType:getElementDamage() > 0 and weaponItemType:getElementType() == et then
												weaponElement = et
												break
											end
										end
								...
								-- Default to physical if weapon has no element
								if not weaponElement or weaponElement == COMBAT_PHYSICALDAMAGE or weaponElement == 0 then
									weaponElement = COMBAT_PHYSICALDAMAGE
								end

								-- Apply the bonus damage using the detected element
								local dmgmin, dmgmax = string.match(slotitemdesc, '%[Enhanced Elemental Damage: (%d+)%-(%d+)%]')
								if dmgmin and dmgmax then
									...
									if creature:isMonster() then
										local resistance = creature:getType():getElementList()
										local immunity = creature:getType():getCombatImmunities()
										eleDmg = filterResistance(eleDmg, weaponElement, immunity, resistance)
									elseif creature:isPlayer() then
										eleDmg = eleDmg / 2
										if (resistances[weaponElement] and (resistances[weaponElement].Native + resistances[weaponElement].Custom ~= 0)) then
											local resistancePercent = (100 - (resistances[weaponElement].Custom + resistances[weaponElement].Native))
											eleDmg = (resistancePercent / 100) * eleDmg
										end
									end

									if eleDmg ~= 0 then
										-- Visual effect based on detected element
										local pos = creature:getPosition()
										if weaponElement == COMBAT_FIREDAMAGE then
											pos:sendMagicEffect(CONST_ME_FIREATTACK)
										elseif weaponElement == COMBAT_ENERGYDAMAGE then
											pos:sendMagicEffect(CONST_ME_ENERGYAREA)
										elseif weaponElement == COMBAT_ICEDAMAGE then
											pos:sendMagicEffect(CONST_ME_ICEAREA)
										elseif weaponElement == COMBAT_EARTHDAMAGE then
											pos:sendMagicEffect(CONST_ME_CARNIPHILA)
										end

										-- Apply damage using the same swap logic as before
										if isWandOrRod then
											if primaryType == weaponElement then
												primaryDamage = primaryDamage + eleDmg
											else
												secondaryDamage = eleDmg
												secondaryType = weaponElement
											end
										elseif elementalroll then
											if primaryType == weaponElement then
												primaryDamage = primaryDamage + eleDmg
												...
												if primaryType == 0 then
													if secondaryType == weaponElement then
														secondaryDamage = secondaryDamage + eleDmg
													else
														primaryDamage = eleDmg
														primaryType = weaponElement
													end
												else
													secondaryDamage = secondaryDamage + eleDmg
													secondaryType = weaponElement
										...
										else -- First elemental roll on this weapon
											local originalDamage = primaryDamage
											local originalType = primaryType
											primaryDamage = secondaryDamage + eleDmg
											primaryType = weaponElement
										...
										-- Fire enchant: 33% chance to apply burn DOT
										if weaponElement == COMBAT_FIREDAMAGE and math.random(1,3) == 3 then
											local burnDamage = 20
											if creature:isPlayer() then
											...
											if burnDamage ~= 0 then
												...
												condition:addDamage(10, 2000, -math.floor(burnDamage))
												creature:addCondition(condition)
												creature:getPosition():sendMagicEffect(CONST_ME_FIRE)
											end
										end
										-- Ice enchant: 20% chance to slow
										if weaponElement == COMBAT_ICEDAMAGE and math.random(1,5) == 5 then
