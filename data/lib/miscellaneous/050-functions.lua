...
function checkWallArito(item, toPosition)
	...
	local checkEqual = {
		-- Water pool at 33207,32537,6 checks for scimitar (2419) at 33205,32537,6
		[2016] = {Position(33207, 32537, 6), {2419, -1}, Position(33205, 32537, 6)},
		-- Scimitar at 33205,32537,6 checks for water pool (2016) at 33207,32537,6
		[2419] = {Position(33205, 32537, 6), {2016, 1}, Position(33207, 32537, 6)}
	}
	...
	if(it and it[1] == toPosition and Tile(it[3]):getItemCountById(it[2][1], it[2][2]) > 0)then
		...
		addEvent(
			function()
				...
				-- Remove the scimitar after gate closes
				if(Tile(Position(33205, 32537, 6)):getItemCountById(2419) > 0) then
					Tile(Position(33205, 32537, 6)):getItemById(2419):remove()
				end
			end, 5*60*1000
		)
	end
