...
-- Extract door IDs for compatibility with action scripts
closedQuestDoors = {}
for _, doorData in pairs(questDoor) do
	table.insert(closedQuestDoors, doorData.closedDoor)
end

closedLevelDoors = {}
for _, doorData in pairs(levelDoor) do
	table.insert(closedLevelDoors, doorData.closedDoor)
end

openExtraDoors = {}
for _, doorData in pairs(customDoor) do
	if doorData.openDoor then
		table.insert(openExtraDoors, doorData.openDoor)
	end
end

closedExtraDoors = {}
for _, doorData in pairs(customDoor) do
	if doorData.closedDoor then
		table.insert(closedExtraDoors, doorData.closedDoor)
	end
end

-- Create openDoors, closedDoors, and lockedDoors arrays for key-based doors
openDoors = {}
closedDoors = {}
lockedDoors = {}

-- Extract from key door tables
for _, doorData in pairs(keyUnlockedDoor) do
	if doorData.openDoor then
		table.insert(openDoors, doorData.openDoor)
	end
	if doorData.closedDoor then
		table.insert(closedDoors, doorData.closedDoor)
	end
end

for _, doorData in pairs(keyLockedDoor) do
	if doorData.openDoor then
		table.insert(openDoors, doorData.openDoor)
	end
	if doorData.closedDoor then
		table.insert(lockedDoors, doorData.closedDoor)
	end
end
...
-- Add house doors if they exist (these might need to be defined separately)
-- For now, initialize as empty tables
