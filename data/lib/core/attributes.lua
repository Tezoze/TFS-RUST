...
local stats = { -- Define the attribute and their rolls
	[1] = { -- Attack
		attribute = {
			name = 'Attack',
			rare = {1, 2}, -- Customize roll numbers here
			epic = {2, 3},
			legendary = {4, 5},
			mythic = {6, 8},
		},
	...
	[12] = { -- Elemental Damage (adapts to weapon's enchant element at combat time)
		attribute = {
			name = 'Enhanced Elemental Damage',
			rare = {5, 10},
			epic = {12, 18},
			legendary = {20, 28},
			mythic = {28, 38},
	...
	[25] = { -- Melee Skill (unified - replaces sword/axe/club)
		attribute = {
			name = 'Melee Skill',
			rare = {1, 1},
			epic = {1, 2},
	...
	[26] = { -- Skill Axe (DEPRECATED - kept for backwards compatibility)
		attribute = {
			name = 'Melee Skill',
			rare = {1, 1},
			epic = {1, 2},
	...
	[27] = { -- Skill Club (DEPRECATED - kept for backwards compatibility)
		attribute = {
			name = 'Melee Skill',
			rare = {1, 1},
			epic = {1, 2},
	...
	[41] = { -- 2H Attack (higher rolls than 1H to compensate for no shield slot)
		attribute = {
			name = 'Attack',
			rare = {2, 3},
			epic = {3, 5},
...
-- Returns: rares (int), rollInfo (table of {itemName, tier, stats})
function rollRarity(container, forced, _isRecursive, _rollInfo)
	local rollInfo = _rollInfo or {}
	-- Tiers
	local tiers = {
		[1] = {
			prefix = 'rare',
			chance = {
				[1] = 500, -- 5% chance to roll
		...
		[3] = {
			prefix = 'legendary',
			chance = {
				[1] = 100, -- 1% chance to roll
				[2] = 10000, -- 100% chance for second stat
				[3] = 5000 -- 50% chance for third stat
			}
		},
		...
		[4] = {
			prefix = 'mythic',
			chance = {
				[1] = 3, -- 0.03% chance to roll
				[2] = 10000, -- 100% chance for second stat
				[3] = 10000, -- 100% chance for third stat
				[4] = 5000 -- 50% chance for fourth stat
			}
		},
	}
	local rares = 0
	...
	if it_u:isContainer() then
		-- If this is a forced/scroll roll directly on a container, roll the container itself
		if forced and type(forced) == "string" then
			table.insert(available_stats, stats[40]) -- Extra Slots
		else
			...
			if h > 0 then
				...
				while i <= h do
					...
					local crares, _ = rollRarity(bagitem, manualroll, true, rollInfo)
					rares = rares + crares
					i = i + 1
				end
			end
		...
		if not it_id:isStackable() then
			...
			if wp > 0 then
				...
				-- Distance Items (always 2H - bows/crossbows)
				elseif wp == WEAPON_DISTANCE then
					table.insert(available_stats, stats[41]) -- 2H Attack (higher rolls)
				...
				-- Sword, Clubs and Axes (all use unified Melee Skill now)
				elseif table.contains({WEAPON_SWORD, WEAPON_CLUB, WEAPON_AXE}, wp) then -- Melee Weapon
					
					-- All melee weapons use unified Melee Skill
					table.insert(available_stats, stats[25]) -- Melee Skill
					
					if it_id:getAttack() > 0 then
						if it_id:getSlotPosition() == 2096 then -- Two-handed: higher attack rolls
							table.insert(available_stats, stats[41]) -- 2H Attack
						else
							table.insert(available_stats, stats[1]) -- 1H Attack
						end
					end
					...
					table.insert(available_stats, stats[39]) -- Mana Leech
				end
	...
	if #available_stats > 0 then -- Skips it all if it's empty
		...
		if tier > 0 then -- Item has rolled rare or higher
			local stats_used = {}
			-- 2H weapon bonus: +1 guaranteed stat at Legendary/Mythic
			local tierChances = {}
			for i, v in ipairs(tiers[tier].chance) do
				tierChances[i] = v
			end
			local is2H = false
			local wp2h = it_id:getWeaponType()
			if wp2h == WEAPON_DISTANCE then
				is2H = true -- Bows/crossbows are always 2H
			elseif table.contains({WEAPON_SWORD, WEAPON_CLUB, WEAPON_AXE}, wp2h) then
				if it_id:getSlotPosition() == 2096 then
					is2H = true
				end
			end
			if is2H and tier >= 3 then -- Legendary or Mythic
				table.insert(tierChances, #tierChances + 1, 10000) -- +1 guaranteed stat
			end
			for stat = 1, #tierChances do
				if #available_stats > 0 then
					local roll = math.random(1, 10000)
					if stat <= 3 or tier ~= 4 then -- First 3 stats are guaranteed for mythic, first stat guaranteed for others
						roll = tierChances[stat]
					end
					if roll <= tierChances[stat] then -- Stats are rolled by chance
			...
			if #stats_used > 0 then
				...
				for stat = 1, #stats_used do
					...
					if tiers[tier].prefix == tiers[4].prefix then
						statmin = stats_used[stat].attribute.mythic[1]
						statmax = stats_used[stat].attribute.mythic[2]
					...
					if stats_used[stat].value ~= nil then -- Is the value type defined?
						...
						local minimumDmg = math.max(1, critv - math.random(3, 7)) -- 3-7 less than the maximum damage, minimum of 1
						...
						-- Container (Extra Slots)
						elseif stats_used[stat].value == "Container" then
							local container = Container(it_u.uid)
							if container then
								local baseCapacity = ItemType(it_u:getId()):getCapacity()
								container:setCapacity(baseCapacity + critv)
							end
							table.insert(stat_desc, '[' .. stats_used[stat].attribute.name .. ': +' .. critv .. ']') -- Container value "+4"
						end
				...
				-- Rarity prefix
				local rarityText = tiers[tier].prefix:gsub("^%l", string.upper) -- Capitalize first letter (Rare, Epic, Legendary)
				...
				-- Store roll info for server log notification
				local rolledItemName = ItemType(it_u:getId()):getName()
				table.insert(rollInfo, {
					itemName = rolledItemName,
					tier = tiers[tier].prefix:gsub("^%l", string.upper),
					stats = table.concat(stat_desc, ", ")
				})
			end
		end
	end
	return rares, rollInfo
end

-- Apply condition
function rollCondition(player, item, slot, equip)
	local attributes = {
		 [1] = {"%[" .. stats[25].attribute.name .. ": ", CONDITION_PARAM_SKILL_SWORD}, -- "[Melee Skill: " - uses SKILL_SWORD internally
		 -- [2] and [3] removed - they were duplicates causing triple skill bonus
		 [2] = {"%[" .. stats[28].attribute.name .. ": ", CONDITION_PARAM_SKILL_MELEE}, -- "[Melee Skills: "
		 [3] = {"%[" .. stats[29].attribute.name .. ": ", CONDITION_PARAM_SKILL_DISTANCE},
		 [4] = {"%[" .. stats[30].attribute.name .. ": ", CONDITION_PARAM_SKILL_SHIELD},
		 [5] = {"%[" .. stats[31].attribute.name .. ": ", CONDITION_PARAM_STAT_MAGICPOINTS},
		 [6] = {"%[" .. stats[32].attribute.name .. ": ", CONDITION_PARAM_STAT_MAXHITPOINTS},
		 [7] = {"%[" .. stats[33].attribute.name .. ": ", CONDITION_PARAM_STAT_MAXMANAPOINTS},
		 [8] = {"%[" .. stats[34].attribute.name .. ": ", CONDITION_PARAM_STAT_MAXHITPOINTSPERCENT, percent = true, absolute = true},
		 [9] = {"%[" .. stats[35].attribute.name .. ": ", CONDITION_PARAM_STAT_MAXMANAPOINTSPERCENT, percent = true, absolute = true},
		[10] = {"%[" .. stats[11].attribute.name .. ": ", CONDITION_PARAM_SPECIALSKILL_CRITICALHITAMOUNT, percent = true, absolute = true},
		[11] = {"%[" .. stats[10].attribute.name .. ": ", CONDITION_PARAM_SPECIALSKILL_CRITICALHITCHANCE, percent = true},
		[12] = {"%[" .. stats[36].attribute.name .. ": ", CONDITION_PARAM_SPECIALSKILL_LIFELEECHCHANCE, percent = true},
		[13] = {"%[" .. stats[37].attribute.name .. ": ", CONDITION_PARAM_SPECIALSKILL_LIFELEECHAMOUNT, percent = true},
		[14] = {"%[" .. stats[38].attribute.name .. ": ", CONDITION_PARAM_SPECIALSKILL_MANALEECHCHANCE, percent = true},
		[15] = {"%[" .. stats[39].attribute.name .. ": ", CONDITION_PARAM_SPECIALSKILL_MANALEECHAMOUNT, percent = true},
	}
		...
		if string.match(itemDesc, attributeString) ~= nil then -- "[Attack: +10]"
			...
			if equip then
				if player:getCondition(CONDITION_ATTRIBUTES, CONDITIONID_COMBAT, offset) == nil then
					...
					local condition = player:getCondition(CONDITION_ATTRIBUTES, CONDITIONID_COMBAT, offset)
					if condition then
						condition:setParameter(attributes[k][2], skillBonus)
					end
				end
...
function itemAttributes(player, item, slot, equip)
	...
	if itemDesc:find("Rare") or itemDesc:find("Epic") or itemDesc:find("Legendary") or itemDesc:find("Mythic") then
		...
		if raritySlots[slot] ~= nil then
			...
			if appropriateSlot then -- Item is in the correct slotType
				-- Checks have all passed, run apply/remove attribute
				rollCondition(player, item, slot, equip)
			end
		end
	end
end
	...
	if slotCount > 0 then
		...
		if player:getSlotItem(slot) ~= nil then
			...
			if appropriateSlot then
				local offset = 0
				local attributes = {
					[1] = {"%[Melee Skill", CONDITION_PARAM_SKILL_SWORD},
					[2] = {"%[Melee Skills", CONDITION_PARAM_SKILL_MELEE},
					[3] = {"%[Distance Skill", CONDITION_PARAM_SKILL_DISTANCE},
					[4] = {"%[Shield Skill", CONDITION_PARAM_SKILL_SHIELD},
					[5] = {"%[Magic Level", CONDITION_PARAM_STAT_MAGICPOINTS},
					[6] = {"%[Max Health", CONDITION_PARAM_STAT_MAXHITPOINTS},
					[7] = {"%[Max Mana", CONDITION_PARAM_STAT_MAXMANAPOINTS}
				}
