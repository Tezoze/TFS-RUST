...
- **`MOVEMENTEVENT` — DONE (Step 10).** Corpus `moveuse.cc:2263-2287` is **object-flag + `moveuse.dat`**, not TFS tile `MoveEvent`. Shipped: dress-toggle rings, eternal lit candelabrum **2057→2042** (stays lit; normal lit 2042 is ChangeUse only), armed trap 2579→2578 + poff. No OTB bit — hardcoded OTB ids in [`movement_event.rs`](../crates/tfs-rust-core/src/movement_event.rs). Fires after tile add and after `internal_move_item` (rings on equip). Not “quest items in chests.”
