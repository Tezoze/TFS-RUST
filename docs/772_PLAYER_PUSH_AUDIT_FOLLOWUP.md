...
| **D5** | Hard-block ordering in the kick loop. 772 `MovePossible` walks the tile stack and `return false` **immediately** on a creature hard block (target/master/NPC/unpushable/invisible), so `KickBoxes` is never reached. Rust `break`s the creature loop and then still runs `monster_kick_boxes`. <ref_snippet file="/mnt/storage2/TFS_RUST/crates/tfs-rust-core/src/monster_push.rs" lines="180-233" /> **Fixed** — `creature_hard_block` flag set on each hard-block `break`; `monster_kick_boxes` gated on `!creature_hard_block`. Test: `d5_hard_block_does_not_kick_boxes`. | Medium | ~~Yes~~ Fixed |
| **D6** | `monster_move_possible_planning` rejects `FLOORCHANGE\|TELEPORT` tiles. `TMonster::MovePossible` (`crnonpl.cc:2141-2293`) has no such check. <ref_snippet file="/mnt/storage2/TFS_RUST/crates/tfs-rust-core/src/monster_ai.rs" lines="1818-1826" /> **Fixed** — `monster_move_possible_planning` gains `allow_floorchange_teleport: bool`; push/kick path passes `true` (772 has no such check), walk path keeps `false` (TFS-style stair/teleport avoidance). | Medium | ~~Yes~~ Fixed |
| **D7** | Kick-loop target gate is `attack_target.is_some() \|\| follow_target.is_some()`; 772 checks the single `this->Target` (`crnonpl.cc:2198`). Follow-only monsters chain-push in Rust but not in 772. **Fixed** — gate now `target_attack.is_some()` only. | Low | ~~Yes (edge)~~ Fixed |
...
3. ~~**D5 + D6 + D7**~~ — ✅ landed (hard-block aborts before `KickBoxes`; `FLOORCHANGE|TELEPORT` dropped from push/kick planning path; kick target gate uses `attack_target` only).
