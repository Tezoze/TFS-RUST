...
Creature::~Creature()
{
	// [CRASH FIX] Copy summons list to avoid iterator invalidation during cleanup
	std::vector<Creature*> summonsCopy(summons.begin(), summons.end());
...
void Creature::onThink(uint32_t interval)
{
	...
	// [SHIELD BREAKING SYSTEM] Regenerate blocks every 2 seconds
	// Only applies to players: With shield: 2 blocks, Without shield (2H weapon): 1 block
	blockTicks += interval;
	if (blockTicks >= 2000) {
		const Player* player = getPlayer();
		if (player) {
			blockCount = player->hasShield() ? 2 : 1;
		}
		blockTicks = 0;
	}
	...
	if (followCreature) {
		...
		// CHANGE THIS: 2000 -> 400
		// 400ms is responsive enough for 8.6 stairs/corners without lagging your CPU.
		if (forceUpdateFollowPath || walkUpdateTicks >= 200) {
			walkUpdateTicks = 0;
...
void Creature::onAttacking(uint32_t interval)
{
	...
	// [SAFETY CHECK] Verify attackedCreature is still valid after callbacks
	// The creature could have been removed during onAttacked() events
	if (!attackedCreature) {
		return;
	}
                              ...
                              const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	if (creature == followCreature || (creature == this && followCreature)) {
		if (hasFollowPath) {
			// [FIX START] ---------------------------------------------
			// INSTANT REACTION FIX (ZIPPY VERSION):
			if (creature == followCreature) {
				// Only schedule if we aren't already updating
				if (!isUpdatingPath) {
					// 1. CLEAR THE OLD PATH
					// This is the key to "Zippiness". It forces the monster to abandon
					// its old route immediately and turn towards you.
					listWalkDir.clear();
					...
					// 3. Schedule the new path (0ms delay)
					g_scheduler.addEvent(createSchedulerTask(0, std::bind(&Creature::goToFollowCreature, this)));
				}
			} else {
				// Default behavior: Flag for update on next tick
				isUpdatingPath = true;
			}
			// [FIX END] -----------------------------------------------
		}

		// [CRASH FIX] Safe Visibility Check
		bool targetVisible = false;
		if (followCreature) {
			// Respect server view limits
			targetVisible = Creature::canSee(getPosition(), followCreature->getPosition(), Map::maxViewportX, Map::maxViewportY);
		}

		// Logic: If target moved floors OR is no longer visible, remove them
		if (followCreature && (newPos.z != oldPos.z || !targetVisible)) {
			onCreatureDisappear(followCreature, false);
		}
	}

	if (creature == attackedCreature || (creature == this && attackedCreature)) {
		// [CRASH FIX] Store pointer locally and check for null
		Creature* currentTarget = attackedCreature;
		if (!currentTarget) {
			return;
		}
		
		if (newPos.z != oldPos.z || !canSee(currentTarget->getPosition())) {
			onCreatureDisappear(currentTarget, false);
		} else {
			if (hasExtraSwing()) {
			...
			// [CRASH FIX] Re-check attackedCreature after potential state changes
			if (attackedCreature && newTile && oldTile && newTile->getZone() != oldTile->getZone()) {
...
bool Creature::dropCorpse(Creature* lastHitCreature, Creature* mostDamageCreature, bool lastHitUnjustified, bool mostDamageUnjustified)
{
	if (!lootDrop && !getMonster()) {
		...
		if (corpse) {
			// Don't drop loot for any summons, only leave corpse
			if (!(getMonster() && master)) {
                               ...
                               bool checkDefense /* = false */, bool checkArmor /* = false */, bool /* field = false */, bool /* ignoreResistances = false */)
{
	...
	// [DEBUG] Store original damage for logging
	int32_t originalDamage = damage;
	int32_t shieldReduction = 0;
	int32_t armorReduction = 0;
	bool debugEnabled = getPlayer() != nullptr; // Only log for players
	...
	if (isImmune(combatType)) {
		...
		// Shield blocks the first 2 attacks per turn (2 seconds).
		// Additional attacks bypass the shield (shield breaking).
		if (checkDefense && canUseDefense && attacker) {
			const Player* player = getPlayer();
			...
			if (player && blockCount > 0) {
				...
				// Apply attack mode multipliers to defense value
				// Offensive: -40%, Defensive: +80%
				fightMode_t fightMode = player->getFightMode();
				if (fightMode == FIGHTMODE_ATTACK) {
					defense = (defense * 60) / 100;
				} else if (fightMode == FIGHTMODE_DEFENSE) {
					defense = (defense * 180) / 100;
				}
				
				// Calculate block amount: skill-based roll (50% to 100% of defense)
				int32_t minBlock = defense / 2;
				int32_t maxBlock = defense;
				...
				if (maxBlock > 0) {
					shieldReduction = uniform_random(minBlock, maxBlock);
				}
				
				shieldReduction = std::min(shieldReduction, damage);
		...
		// [ARMOR SYSTEM] Always applies after shield (if damage remains)
		if (checkArmor && damage > 0) {
			int32_t armor = getArmor();

			if (armor > 1) {
				// Standard formula: Min = Armor/2, Max = Armor - 1
				int32_t minArmor = armor / 2;
				int32_t maxArmor = armor - 1;

				if (maxArmor < minArmor) {
					maxArmor = minArmor;
				}

				armorReduction = uniform_random(minArmor, maxArmor);
				
				// [CRITICAL FIX] Clamp reduction to not exceed remaining damage
				armorReduction = std::min(armorReduction, damage);
				damage -= armorReduction;
			} else if (armor > 0) {
				armorReduction = std::min(1, damage);
				damage -= armorReduction;
			}
		...
		if (shieldBlocked && blockType != BLOCK_NONE) {
			onBlockHit();
		}
	}
...
void Creature::onGainExperience(uint64_t gainExp, Creature* target)
{
	...
	for (Creature* spectator : spectators) {
		// [CRASH FIX] Check if spectator is a player before sending message
