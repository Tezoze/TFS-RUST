...
class Game
{
	public:
		...
		/**
		  * Returns the count of online players for an account
		  * \param acc is the account identifier
		  * \returns The number of online players for this account
		  */
		uint32_t getPlayersCountByAccount(uint32_t acc);
		...
		size_t getPlayersOnline() const {
			int32_t spoofCount = g_config.getNumber(ConfigManager::SPOOF_ONLINE_COUNT);
			return spoofCount > 0 ? spoofCount : players.size();
		}
		...
		std::forward_list<Item*> getMarketItemList(uint16_t wareId, uint16_t sufficientCount, const std::map<uint32_t, DepotChest*>& depotChests, Inbox* inbox);
