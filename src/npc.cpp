...
std::map<std::string, std::unique_ptr<NpcType>> Npcs::npcTypes;

// NpcType implementation
NpcType::NpcType(const std::string& initName) : name(initName) {}
...
bool NpcType::loadCallback(LuaScriptInterface* scriptInterface)
{
	...
	switch (info.eventType) {
		...
		case NPCS_EVENT_SAY:
			...
			break;
		case NPCS_EVENT_CLOSECHANNEL:
			info.playerCloseChannelEvent = id;
			break;
		case NPCS_EVENT_BUYITEM:
			info.playerBuyEvent = id;
			break;
		case NPCS_EVENT_SELLITEM:
			info.playerSellEvent = id;
			break;
		case NPCS_EVENT_CHECKITEM:
			info.playerCheckItemEvent = id;
			break;
		default:
			break;
	}
...
NpcType* Npcs::getNpcType(const std::string& name, bool create/* = false*/)
{
	...
	auto npcType = std::make_unique<NpcType>(name);
	NpcType* ptr = npcType.get();
	npcTypes[lowerName] = std::move(npcType);
	return ptr;
}
...
void Npcs::reload()
{
	...
	// Close all shop windows first
	for (const auto& it : npcs) {
		it.second->closeAllShopWindows();
	}

	// Reset the script interface
	delete Npc::scriptInterface;
	Npc::scriptInterface = nullptr;

	// Clear Lua-registered NPC types - they will be re-registered when scripts reload
	npcTypes.clear();

	// Reload NPC scripts from data/npc/ (same approach as monsters)
	if (!g_scripts->loadScripts("npc", false, true)) {
		std::cout << "[Warning - Npcs::reload] Failed to reload NPC scripts from npc." << std::endl;
	}

	// Now reload each NPC - they should find their NpcType from the freshly loaded scripts
	for (const auto& it : npcs) {
		it.second->reload();
	}
}
...
Npc::Npc(const std::string& name) :
	Creature(),
	name(name),
	typeName(name),  // Store original spawn name for NpcType lookup
...
bool Npc::load()
{
	...
	// First check if there's a Lua-defined NpcType (use typeName which is the original spawn name)
	NpcType* npcType = Npcs::getNpcType(typeName, false);
	if (npcType && npcType->fromLua) {
		loaded = loadFromNpcType(npcType);
	} else {
...
bool Npc::loadFromNpcType(NpcType* npcTypePtr)
{
	if (!npcTypePtr) {
		return false;
	}

	// Store pointer for direct callback access
	this->npcType = npcTypePtr;

	name = npcTypePtr->name;
	
	// Load outfit
	defaultOutfit = npcTypePtr->info.outfit;
	currentOutfit = defaultOutfit;
	
	// Load health
	health = npcTypePtr->info.health;
	healthMax = npcTypePtr->info.healthMax;
	
	// Load movement settings
	baseSpeed = npcTypePtr->info.baseSpeed;
	walkTicks = npcTypePtr->info.walkInterval;
	masterRadius = npcTypePtr->info.walkRadius;
	
	// Load flags
	pushable = npcTypePtr->info.pushable;
	floorChange = npcTypePtr->info.floorChange;
	attackable = npcTypePtr->info.attackable;
	ignoreHeight = npcTypePtr->info.ignoreHeight;
	speechBubble = npcTypePtr->info.speechBubble;
	
	// Create event handler for NpcType callbacks
	npcEventHandler = new NpcEventsHandler(npcTypePtr, this);
...
{
	loaded = false;
	npcType = nullptr;  // Clear to prevent dangling pointer during reload
	isIdle = true;
	walkTicks = 1500;
	pushable = true;
	floorChange = false;
...
void Npc::reload()
{
	...
	// Note: We don't call onCreatureAppear during reload because:
	// 1. The NPC is already on the map
	// 2. The Lua callbacks may have stale references after script reload
	// Players who want to interact will trigger proper events naturally
}
...
void Npc::onRemoveCreature(Creature* creature, bool isLogout)
{
	...
	if (creature == this) {
		...
		if (npcType && npcType->info.creatureDisappearEvent != -1) {
			...
			if (interface->reserveScriptEnv()) {
				ScriptEnvironment* env = interface->getScriptEnv();
				env->setScriptId(npcType->info.creatureDisappearEvent, interface);
				env->setNpc(this);

				lua_State* L = interface->getLuaState();
				interface->pushFunction(npcType->info.creatureDisappearEvent);
				LuaScriptInterface::pushUserdata<Npc>(L, this);
				LuaScriptInterface::setMetatable(L, -1, "Npc");
				LuaScriptInterface::pushUserdata<Creature>(L, creature);
				LuaScriptInterface::setCreatureMetatable(L, -1, creature);
                         ...
                         const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	if (creature == this || creature->getPlayer()) {
		...
		if (npcType && npcType->info.creatureMoveEvent != -1) {
			...
			if (interface->reserveScriptEnv()) {
				ScriptEnvironment* env = interface->getScriptEnv();
				env->setScriptId(npcType->info.creatureMoveEvent, interface);
				env->setNpc(this);

				lua_State* L = interface->getLuaState();
				interface->pushFunction(npcType->info.creatureMoveEvent);
				LuaScriptInterface::pushUserdata<Npc>(L, this);
				LuaScriptInterface::setMetatable(L, -1, "Npc");
				LuaScriptInterface::pushUserdata<Creature>(L, creature);
				LuaScriptInterface::setCreatureMetatable(L, -1, creature);
...
void Npc::onCreatureSay(Creature* creature, SpeakClasses type, const std::string& text)
{
	...
	// For Lua-defined NPCs, call the callback directly via g_scripts
	if (npcType && npcType->info.creatureSayEvent != -1) {
		LuaScriptInterface* interface = &g_scripts->getScriptInterface();
		if (!interface->reserveScriptEnv()) {
			std::cout << "[Error - Npc::onCreatureSay] Call stack overflow" << std::endl;
			return;
		}

		ScriptEnvironment* env = interface->getScriptEnv();
		env->setScriptId(npcType->info.creatureSayEvent, interface);
		env->setNpc(this);

		lua_State* L = interface->getLuaState();
		interface->pushFunction(npcType->info.creatureSayEvent);
		LuaScriptInterface::pushUserdata<Npc>(L, this);
		LuaScriptInterface::setMetatable(L, -1, "Npc");
		LuaScriptInterface::pushUserdata<Creature>(L, creature);
		LuaScriptInterface::setCreatureMetatable(L, -1, creature);
	...
	// Fall back to NpcEventsHandler for XML NPCs
	if (npcEventHandler) {
...
{
	// For Lua-defined NPCs, call the callback directly
	if (npcType && npcType->info.playerCloseChannelEvent != -1) {
		LuaScriptInterface* interface = &g_scripts->getScriptInterface();
		if (!interface->reserveScriptEnv()) {
			std::cout << "[Error - Npc::onPlayerCloseChannel] Call stack overflow" << std::endl;
			return;
		}

		ScriptEnvironment* env = interface->getScriptEnv();
		env->setScriptId(npcType->info.playerCloseChannelEvent, interface);
		env->setNpc(this);

		lua_State* L = interface->getLuaState();
		interface->pushFunction(npcType->info.playerCloseChannelEvent);
		LuaScriptInterface::pushUserdata<Npc>(L, this);
		LuaScriptInterface::setMetatable(L, -1, "Npc");
...
void Npc::onThink(uint32_t interval)
{
	...
	if (npcType && npcType->info.thinkEvent != -1) {
		...
		if (interface->reserveScriptEnv()) {
			ScriptEnvironment* env = interface->getScriptEnv();
			env->setScriptId(npcType->info.thinkEvent, interface);
			env->setNpc(this);

			lua_State* L = interface->getLuaState();
			interface->pushFunction(npcType->info.thinkEvent);
			LuaScriptInterface::pushUserdata<Npc>(L, this);
			LuaScriptInterface::setMetatable(L, -1, "Npc");
...
void Npc::onPlayerBuyItem(Player* player, uint16_t itemId, uint8_t count,
                          uint8_t amount, bool ignore/* = false*/, bool inBackpacks/* = false*/)
{
	if (npcEventHandler) {
		npcEventHandler->onPlayerBuyItem(player, itemId, count, amount, ignore, inBackpacks);
	}
	player->sendSaleItemList();
}

void Npc::onPlayerSellItem(Player* player, uint16_t itemId, uint8_t count,
                           uint8_t amount, bool ignoreEquipped/* = false*/)
{
	if (npcEventHandler) {
		npcEventHandler->onPlayerSellItem(player, itemId, count, amount, ignoreEquipped);
	}
	player->sendSaleItemList();
}
...
NpcEventsHandler::NpcEventsHandler(NpcType* npcType, Npc* npc) :
	npc(npc), scriptInterface(npc->getScriptInterface())
{
	if (!npcType) {
		loaded = false;
	...
	// Store the NpcType's script interface for calling NpcType events
	npcTypeScriptInterface = npcType->info.scriptInterface;

	// Copy event IDs from NpcType
	creatureAppearEvent = npcType->info.creatureAppearEvent;
	creatureDisappearEvent = npcType->info.creatureDisappearEvent;
	creatureMoveEvent = npcType->info.creatureMoveEvent;
	creatureSayEvent = npcType->info.creatureSayEvent;
	thinkEvent = npcType->info.thinkEvent;
	playerCloseChannelEvent = npcType->info.playerCloseChannelEvent;
	playerBuyEvent = npcType->info.playerBuyEvent;
	playerSellEvent = npcType->info.playerSellEvent;
	playerEndTradeEvent = -1; // Not used for NpcType

	// Mark as loaded if we have at least one event
	loaded = (creatureAppearEvent != -1 || creatureDisappearEvent != -1 ||
	          creatureMoveEvent != -1 || creatureSayEvent != -1 ||
	          thinkEvent != -1 || playerCloseChannelEvent != -1 ||
	          playerBuyEvent != -1 || playerSellEvent != -1);
}
...
void NpcEventsHandler::onCreatureAppear(Creature* creature)
{
	...
	// Use npcTypeScriptInterface for NpcType events, otherwise use scriptInterface
	LuaScriptInterface* si = npcTypeScriptInterface ? npcTypeScriptInterface : scriptInterface;

	//onCreatureAppear(creature)
	if (!si->reserveScriptEnv()) {
		std::cout << "[Error - NpcScript::onCreatureAppear] Call stack overflow" << std::endl;
		return;
	}

	ScriptEnvironment* env = si->getScriptEnv();
	env->setScriptId(creatureAppearEvent, si);
	env->setNpc(npc);

	lua_State* L = si->getLuaState();
	si->pushFunction(creatureAppearEvent);
	LuaScriptInterface::pushUserdata<Creature>(L, creature);
	LuaScriptInterface::setCreatureMetatable(L, -1, creature);
	si->callFunction(1);
}
...
void NpcEventsHandler::onCreatureDisappear(Creature* creature)
{
	...
	ScriptEnvironment* env = si->getScriptEnv();
	env->setScriptId(creatureDisappearEvent, si);
	env->setNpc(npc);

	lua_State* L = si->getLuaState();
	si->pushFunction(creatureDisappearEvent);
	LuaScriptInterface::pushUserdata<Creature>(L, creature);
	LuaScriptInterface::setCreatureMetatable(L, -1, creature);
	si->callFunction(1);
}
...
void NpcEventsHandler::onCreatureMove(Creature* creature, const Position& oldPos, const Position& newPos)
{
	...
	ScriptEnvironment* env = si->getScriptEnv();
	env->setScriptId(creatureMoveEvent, si);
	env->setNpc(npc);

	lua_State* L = si->getLuaState();
	si->pushFunction(creatureMoveEvent);
	LuaScriptInterface::pushUserdata<Creature>(L, creature);
	LuaScriptInterface::setCreatureMetatable(L, -1, creature);
	LuaScriptInterface::pushPosition(L, oldPos);
	LuaScriptInterface::pushPosition(L, newPos);
...
void NpcEventsHandler::onCreatureSay(Creature* creature, SpeakClasses type, const std::string& text)
{
	...
	ScriptEnvironment* env = si->getScriptEnv();
	env->setScriptId(creatureSayEvent, si);
	env->setNpc(npc);

	lua_State* L = si->getLuaState();
	si->pushFunction(creatureSayEvent);
	LuaScriptInterface::pushUserdata<Creature>(L, creature);
	LuaScriptInterface::setCreatureMetatable(L, -1, creature);
	lua_pushnumber(L, type);
	LuaScriptInterface::pushString(L, text);
...
{
	// Determine if this is a buy or sell operation by comparing callback with stored callbacks
	int32_t onBuy, onSell;
	player->getShopOwner(onBuy, onSell);
	
	bool isBuyOperation = (callback == onBuy);
	bool isSellOperation = (callback == onSell);
	
	// If callback is -1 OR if event handlers are registered, use event handlers instead
	// This handles both cases: no callbacks provided, or dummy callbacks with real event handlers
	if (callback == -1 || (isBuyOperation && playerBuyEvent != -1) || (isSellOperation && playerSellEvent != -1)) {
		if (isBuyOperation || (!isSellOperation && playerBuyEvent != -1)) {
			npc->onPlayerBuyItem(player, itemId, count, amount, ignore, inBackpacks);
		} else {
			npc->onPlayerSellItem(player, itemId, count, amount, ignore);
		}
		return;
	}

	// Call the Lua callback (only if no event handlers are registered)
...
void NpcEventsHandler::onPlayerBuyItem(Player* player, uint16_t itemId,
                              uint8_t count, uint8_t amount, bool ignore, bool inBackpacks)
{
	if (playerBuyEvent == -1) {
		return;
	...
	// Use npcTypeScriptInterface if available, otherwise fall back to scriptInterface
	LuaScriptInterface* si = npcTypeScriptInterface ? npcTypeScriptInterface : scriptInterface;

	//onBuyItem(npc, player, itemid, subType, amount, ignore, inbackpacks)
	if (!si->reserveScriptEnv()) {
		std::cout << "[Error - NpcScript::onPlayerBuyItem] Call stack overflow" << std::endl;
		return;
	}

	ScriptEnvironment* env = si->getScriptEnv();
	env->setScriptId(playerBuyEvent, si);
	env->setNpc(npc);

	lua_State* L = si->getLuaState();
	si->pushFunction(playerBuyEvent);
...
void NpcEventsHandler::onPlayerSellItem(Player* player, uint16_t itemId,
                              uint8_t count, uint8_t amount, bool ignoreEquipped)
{
	if (playerSellEvent == -1) {
		return;
	}
	...
	//onSellItem(npc, player, itemid, subType, amount, ignoreEquipped)
	if (!si->reserveScriptEnv()) {
		std::cout << "[Error - NpcScript::onPlayerSellItem] Call stack overflow" << std::endl;
		return;
	}

	ScriptEnvironment* env = si->getScriptEnv();
	env->setScriptId(playerSellEvent, si);
	env->setNpc(npc);

	lua_State* L = si->getLuaState();
	si->pushFunction(playerSellEvent);
	...
	lua_pushnumber(L, count);
	lua_pushnumber(L, amount);
	LuaScriptInterface::pushBoolean(L, ignoreEquipped);
...
void NpcEventsHandler::onPlayerCloseChannel(Player* player)
{
	...
	ScriptEnvironment* env = si->getScriptEnv();
	env->setScriptId(playerCloseChannelEvent, si);
	env->setNpc(npc);

	lua_State* L = si->getLuaState();
	si->pushFunction(playerCloseChannelEvent);
	LuaScriptInterface::pushUserdata<Player>(L, player);
	LuaScriptInterface::setMetatable(L, -1, "Player");
	si->callFunction(1);
}
...
void NpcEventsHandler::onThink()
{
	...
	ScriptEnvironment* env = si->getScriptEnv();
	env->setScriptId(thinkEvent, si);
	env->setNpc(npc);

	si->pushFunction(thinkEvent);
	si->callFunction(0);
}
