...
void Tile::onUpdateTile(const SpectatorVec& spectators)
{
	...
	for (Creature* spectator : spectators) {
		...
		if (Player* player = spectator->getPlayer()) {
			player->sendUpdateTile(this, cylinderMapPos);
		}
	}
}
...
void Tile::postAddNotification(Thing* thing, const Cylinder* oldParent, int32_t index, cylinderlink_t link /*= LINK_OWNER*/)
{
	...
	// Copy spectators to avoid iterator invalidation during MoveEvent callbacks
	SpectatorVec spectatorsCopy = spectators;
	for (Creature* spectator : spectatorsCopy) {
...
Item* Tile::getUseItem(int32_t index) const
{
	...
	// First, check if there's a container on the tile
	// This is important for rope spots where the ground might be returned instead
	...
	// If there's a container, prioritize it over the ground (especially for rope spots)
	if (containerItem) {
		if (Thing* thing = getThing(index)) {
			Item* item = thing->getItem();
			// If the index points to the ground and we have a container, return the container instead
			if (item == ground) {
				return containerItem;
			}
			// If the index points to the container, return it
		...
		// If index doesn't match but we have a container, return it
		return containerItem;
	}

	// No container found, use normal logic
	if (Thing* thing = getThing(index)) {
		Item* item = thing->getItem();
		if (item) {
			return item;
