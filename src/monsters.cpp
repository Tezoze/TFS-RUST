...
bool Monsters::reload()
{
	...
	// [CRASH FIX] First destroy all old spell data (which may reference the
	// old scriptInterface), THEN reset the scriptInterface for fresh loading.
	// The previous order (reset first, then destroy spells) was safe because
	// spellBlock_t destructors only delete CombatSpell objects they own and
	// don't call back into the scriptInterface. But to be extra safe and
	// clear, we explicitly clear attack/defense spells before resetting.
	for (auto& [name, mType] : monsters) {
...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb, const std::string& description)
{
	...
	if (isScripted) {
		...
		if (tmpName == "melee") {
			...
			// Check if this is an attack or defense spell
			std::string nodeName = node.name();
			bool isAttackSpell = (nodeName == "attack");

			if (isAttackSpell) {
				// Attack spells: Use percentage-based system
				ConditionSpeed* condition = static_cast<ConditionSpeed*>(
					Condition::createCondition(CONDITIONID_COMBAT, conditionType, duration, minSpeedChange)
				);
				combat->addCondition(condition);
			} else {
				// Defense spells: Use formula-based system
...
bool Monsters::deserializeSpell(MonsterSpell* spell, spellBlock_t& sb, const std::string& description)
{
	...
	if (spell->isScripted) {
		...
		if (tmpName == "melee") {
			...
			ConditionSpeed* condition = static_cast<ConditionSpeed*>(Condition::createCondition(CONDITIONID_COMBAT, conditionType, duration, 0));
			float calc_mina = minSpeedChange / 1000.0f;
			float calc_maxa = maxSpeedChange / 1000.0f;
			condition->setFormulaVars(calc_mina, 0, calc_maxa, 0);
			combat->addCondition(condition);
		} else if (tmpName == "outfit") {
			int32_t duration = 10000;
			...
			// Use lazy loading for monster outfit - resolved when spell is cast
			...
			// NEW: If duration is set, use addDamage for fixed damage-per-tick
			// Otherwise use old getDamageCondition system for backward compatibility
			if (spell->duration != 0 && tickInterval > 0) {
				// Fixed damage per tick system
				int32_t numTicks = spell->duration / tickInterval;
				int32_t damagePerTick = minDamage; // Use minDamage as damage per tick
				
				ConditionDamage* condition = static_cast<ConditionDamage*>(
					Condition::createCondition(CONDITIONID_COMBAT, spell->conditionType, 0, 0)
				);
				condition->setParam(CONDITION_PARAM_DELAYED, 1);
				
				// Add fixed damage for each tick
				condition->addDamage(numTicks, tickInterval, -damagePerTick);
				
				combat->addCondition(condition);
			} else {
				// Old system: use generateDamageList countdown pattern
