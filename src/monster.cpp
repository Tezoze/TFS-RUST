...
void Monster::doAttacking(uint32_t interval)
{
	...
	const Position& myPos = getPosition();
	// [CRASH FIX] Copy position by value instead of holding a reference.
	// castSpell() can kill the target, which triggers removeCreature() and frees
	// the Creature object. A const-reference to its Position becomes dangling,
	// causing a SIGSEGV on the next loop iteration when canUseSpell reads it.
	Position targetPos = attackedCreature->getPosition();
	...
	for (const spellBlock_t& spellBlock : mType->info.attackSpells) {
		...
		// Refresh target position each iteration in case the target moved
		...
		if (canUseSpell(myPos, targetPos, spellBlock, interval, inRange, resetTicks)) {
			if (spellBlock.chance >= static_cast<uint32_t>(uniform_random(1, 100))) {
				...
				// [CRASH FIX] After castSpell, Lua callbacks may have removed
				// either the target or this monster itself. Bail out immediately
				// to avoid accessing invalidated state.
...
{
	if (isHostile()) {
		// [SAFETY CHECK] Verify target is valid before accessing position
		if (!target) {
			return false;
		}
...
{
	// [CRASH FIX] Same guard as doAttacking — bail if removed or no spells
	if (isRemoved()) {
		return;
	}
	...
	for (const spellBlock_t& spellBlock : mType->info.defenseSpells) {
		// Per-spell cooldown check: Skip if spell is still on cooldown
		if ((OTSYS_TIME() - spellBlock.lastUsedTime) < static_cast<int64_t>(spellBlock.speed)) {
			continue;
		}
		...
		if ((spellBlock.chance >= static_cast<uint32_t>(uniform_random(1, 100)))) {
			...
			// [CRASH FIX] Bail if this monster was removed during castSpell
			if (isRemoved()) {
				return;
			}

			// Mark spell as used to start its cooldown
			spellBlock.lastUsedTime = OTSYS_TIME();
		}
	}
...
{
	// [SAFETY CHECK] Verify attackedCreature exists before dancing
	if (!attackedCreature) {
		return false;
	}

	bool canDoAttackNow = canUseAttack(creaturePos, attackedCreature);
	// [CRASH FIX] Copy position by value — attackedCreature could be removed
	// during canUseAttack or canWalkTo callbacks, leaving a dangling reference.
...
void Monster::updateLookDirection()
{
	...
	// [SAFETY CHECK] Verify attackedCreature is still valid
	if (attackedCreature) {
		const Position& pos = getPosition();
		// [CRASH FIX] Copy by value — the creature could be freed between
		// this read and the offset calculations below.
