...
bool Item::equals(const Item* otherItem) const
{
	...
	// For stackable items, ignore duration-related attributes in comparison
	// This allows items like empty flasks to stack even if duration data differs
	const uint32_t ignoredAttributes = ITEM_ATTRIBUTE_DURATION | ITEM_ATTRIBUTE_DURATION_TIMESTAMP | ITEM_ATTRIBUTE_DECAYSTATE;
	...
	if (!attributes) {
		...
		// Check if other item only has ignored attributes
		return (otherAttributes->attributeBits & ~ignoredAttributes) == 0;
	} else if (!otherAttributes) {
		// Check if this item only has ignored attributes
		return (attributes->attributeBits & ~ignoredAttributes) == 0;
	}

	// Compare attribute bits, ignoring duration-related attributes
	uint32_t thisBits = attributes->attributeBits & ~ignoredAttributes;
	uint32_t otherBits = otherAttributes->attributeBits & ~ignoredAttributes;
	...
	for (const auto& attribute : attributeList) {
		// Skip duration-related attributes in comparison
		if (attribute.type == ITEM_ATTRIBUTE_DURATION || 
		    attribute.type == ITEM_ATTRIBUTE_DURATION_TIMESTAMP || 
		    attribute.type == ITEM_ATTRIBUTE_DECAYSTATE) {
			continue;
		}
...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_CONTAINERSIZE: {
			uint32_t containerSize;
			if (!propStream.read<uint32_t>(containerSize)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_CONTAINERSIZE, containerSize);
			break;
		}
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	if (hasAttribute(ITEM_ATTRIBUTE_CONTAINERSIZE)) {
		propWriteStream.write<uint8_t>(ATTR_CONTAINERSIZE);
		propWriteStream.write<uint32_t>(getIntAttr(ITEM_ATTRIBUTE_CONTAINERSIZE));
	}
...
void ItemAttributes::setIntAttr(itemAttrTypes type, int64_t value)
{
	...
	if (type == ITEM_ATTRIBUTE_CONTAINERSIZE && value < 1) {
		value = 1;
	}
