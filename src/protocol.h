...
// Packet logging macro
#define LOG_PACKET_FUNCTION(func_name) logPacket(func_name, __FILE__, __LINE__)

// Configuration for packet logging (set to true to enable)
#ifndef PACKET_LOGGING_ENABLED
#define PACKET_LOGGING_ENABLED false
#endif
...
class Protocol : public std::enable_shared_from_this<Protocol>
{
	public:
		...
		void logPacket(const std::string& functionName, const char* file, int line) const;
		void logPacketSend(const OutputMessage_ptr& msg) const;

		void send(OutputMessage_ptr msg) const {
			if (PACKET_LOGGING_ENABLED && msg && !currentFunctionName.empty()) {
				logPacketSend(msg);
			}
	...
	private:
		...
		mutable const char* currentFileName = nullptr;
		mutable int currentLineNumber = 0;
};
