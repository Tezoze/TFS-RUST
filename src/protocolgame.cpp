...
void ProtocolGame::login(const std::string& name, uint32_t accountId, OperatingSystem_t operatingSystem)
{
	...
	if (!foundPlayer || g_config.getBoolean(ConfigManager::ALLOW_CLONES)) {
		...
		int32_t maxPlayersPerAccount = g_config.getNumber(ConfigManager::MAX_PLAYERS_PER_ACCOUNT);
		if (maxPlayersPerAccount > 0 && player->getAccountType() < ACCOUNT_TYPE_GAMEMASTER) {
			uint32_t onlineCount = g_game.getPlayersCountByAccount(player->getAccount());
			if (onlineCount >= static_cast<uint32_t>(maxPlayersPerAccount)) {
				if (maxPlayersPerAccount == 1) {
					disconnectClient("You may only login with one character\nof your account at the same time.");
				} else {
					disconnectClient(fmt::format("You may only login with {:d} characters\nof your account at the same time.", maxPlayersPerAccount));
				}
				return;
			}
		}
		...
		if (std::size_t currentSlot = clientLogin(*player)) {
			...
			LOG_PACKET_FUNCTION("ProtocolGame::login");
			send(output);
			disconnect();
			return;
		}
		...
		// Establish client connection and send SelfAppear message
		...
		// Debug: Log successful fresh login setup
		// std::cout << "[DEBUG] Fresh login setup complete for player " << player->getName() << ", SelfAppear sent" << std::endl;
	} else {
...
void ProtocolGame::connect(uint32_t playerId, OperatingSystem_t operatingSystem)
{
	...
	// Debug: Log successful client setup
	// std::cout << "[DEBUG] Player " << player->getName() << " client setup complete, SelfAppear sent" << std::endl;
}
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	if (g_game.getGameState() == GAME_STATE_STARTUP || g_game.getGameState() == GAME_STATE_INIT) {
...
void ProtocolGame::onConnect()
{
	...
	LOG_PACKET_FUNCTION("ProtocolGame::onConnect");
	send(output);
}
...
void ProtocolGame::disconnectClient(const std::string& message) const
{
	...
	LOG_PACKET_FUNCTION("ProtocolGame::disconnectClient");
	send(output);
	disconnect();
}
...
void ProtocolGame::parsePacket(NetworkMessage& msg)
{
	...
	switch (recvbyte) {
		...
		// Quest log now uses Lua quest system via luascript callbacks
...
void ProtocolGame::sendStats()
{
	...
	// Send unjustified stats separately (Australis OTClient expects opcode 183)
...
{
	NetworkMessage msg;
	msg.addByte(0xB7); // Opcode 183 (GameServerUnjustifiedStats)
	
	uint32_t fragTime = g_config.getNumber(ConfigManager::FRAG_TIME);
	int64_t skullTicks = player->getSkullTicks();
	
	// Based on OTClient expectations:
	// killsDay/killsWeek/killsMonth = percentage (0-100) of progress toward limit
	// killsDayRemaining/killsWeekRemaining/killsMonthRemaining = kills remaining before hitting max (starts at max, decreases)
	// skullTime = total skull time in days
	
	// Determine max kills based on current skull using multiplication from config thresholds
	// FIX: If we are Red Skull OR Black Skull, use the Black Skull limit as the baseline.
	// If we are None or White Skull, use the Red Skull limit.
	uint8_t baseKills = (player->getSkull() >= SKULL_RED) ? g_config.getNumber(ConfigManager::KILLS_TO_BLACK) : g_config.getNumber(ConfigManager::KILLS_TO_RED);
	uint8_t maxDayKills = baseKills;
	uint8_t maxWeekKills = static_cast<uint8_t>(baseKills * 2.5f); // 2.5x for week
	uint8_t maxMonthKills = baseKills * 5; // 5x for month

	uint8_t killsDay = 0;
	uint8_t killsDayRemaining = maxDayKills; // Start with max remaining
	uint8_t killsWeek = 0;
	uint8_t killsWeekRemaining = maxWeekKills; // Start with max remaining
	uint8_t killsMonth = 0;
	uint8_t killsMonthRemaining = maxMonthKills; // Start with max remaining
	uint8_t skullTime = 0;

	if (fragTime > 0 && skullTicks > 0) {
		int64_t currentTime = time(nullptr);
		int64_t oneDayAgo = currentTime - (24 * 60 * 60);
		int64_t oneWeekAgo = currentTime - (7 * 24 * 60 * 60);
		int64_t oneMonthAgo = currentTime - (30 * 24 * 60 * 60);

		// Count kills within each time period from timestamps
		uint8_t killsInDay = 0;
		uint8_t killsInWeek = 0;
		uint8_t killsInMonth = 0;

		for (int64_t timestamp : player->unjustifiedKillTimestamps) {
		...
		// Cap kills at max for each period
		uint8_t actualDayKills = std::min(killsInDay, maxDayKills);
		uint8_t actualWeekKills = std::min(killsInWeek, maxWeekKills);
		uint8_t actualMonthKills = std::min(killsInMonth, maxMonthKills);

		// Calculate percentage (0-100) for progress bars
		killsDay = static_cast<uint8_t>((actualDayKills * 100) / maxDayKills);
		killsWeek = static_cast<uint8_t>((actualWeekKills * 100) / maxWeekKills);
		killsMonth = static_cast<uint8_t>((actualMonthKills * 100) / maxMonthKills);

		// Calculate remaining kills (how many kills left before hitting max)
		killsDayRemaining = maxDayKills - actualDayKills;
		killsWeekRemaining = maxWeekKills - actualWeekKills;
		killsMonthRemaining = maxMonthKills - actualMonthKills;

		// Convert skullTicks (seconds) to days for skullTime
		// fragTime is typically 24*60*60 = 86400 seconds (1 day)
		uint64_t daysRemaining = (skullTicks + fragTime - 1) / fragTime; // Ceiling division to get days
		skullTime = static_cast<uint8_t>(std::min<uint64_t>(daysRemaining, 255));
	}
	
	msg.addByte(killsDay);
	msg.addByte(killsDayRemaining);
	msg.addByte(killsWeek);
	msg.addByte(killsWeekRemaining);
	msg.addByte(killsMonth);
	msg.addByte(killsMonthRemaining);
	msg.addByte(skullTime);
	
	writeToOutputBuffer(msg);
}

void ProtocolGame::sendOTCFeatures()
{
	// Send OTC Features to enable custom features on the client
	// This reads the OTCFeatures table from config.lua and sends them to the client
	// The client expects opcode 0x43 (67) for GameServerFeatures
	
	NetworkMessage msg;
	msg.addByte(0x43); // Opcode 67 (GameServerFeatures)
	
	// List of features to enable (from config.lua OTCFeatures)
	// Feature 68 = GameUnjustifiedPoints
	std::vector<uint8_t> features = {68}; // GameUnjustifiedPoints
	
	msg.add<uint16_t>(features.size()); // Number of features
	
	for (uint8_t feature : features) {
		msg.addByte(feature);  // Feature ID
		msg.addByte(1);         // Enabled (1 = true, 0 = false)
	}
...
{
	// Combat analyzer disabled - function does nothing
	return;
}
...
void ProtocolGame::sendLootStats(const Item* item, const std::string& creatureName)
{
	NetworkMessage msg;
	msg.addByte(0xCF);
...
void ProtocolGame::sendKillTracker(const std::string& monsterName, const Outfit_t& outfit, const std::vector<const Item*>& corpseItems)
{
	NetworkMessage msg;
	msg.addByte(0xD1);
	msg.addString(monsterName);
	AddOutfit(msg, outfit);
	msg.addByte(static_cast<uint8_t>(corpseItems.size()));
...
{
	NetworkMessage msg;
	msg.addByte(0x32); // Extended opcode packet
	msg.addByte(opcode);
...
void ProtocolGame::sendMarketEnter(uint32_t depotId)
{
	...
	for (const auto& [id, depotChest] : depotChests) {
		if (depotChest) {
			containerList.push_front(depotChest);
		}
	}
	if (player->getInbox()) {
		containerList.push_front(player->getInbox());
	}
	...
	do {
		...
		for (Item* item : container->getItemList()) {
			...
			uint16_t wareId = itemType.wareId;
			if (wareId == 0) {
				// Skip items without wareId (XML-only market control)
				continue;
			}
			...
			depotItems[wareId] += Item::countByType(item, -1);
		}
...
{
	// [FIX] Check if player can see either the source or destination position
	// Projectiles should only be visible if at least one endpoint is visible
...
void ProtocolGame::sendAddCreature(const Creature* creature, const Position& pos, int32_t stackpos, bool isLogin)
{
	...
	// Debug: This is the SelfAppear message for the player
	// std::cout << "[DEBUG] Sending SelfAppear (0x17) message for player " << player->getName() << std::endl;
	...
	sendOTCFeatures(); // Enable OTC features like GameUnjustifiedPoints
	sendEnterWorld();
