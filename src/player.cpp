...
int32_t Player::getDefense() const
{
	...
	return (defenseSkill / 3.7 + 2.23) * defenseValue * 0.16 * getDefenseFactor() * vocation->defenseMultiplier;
}
...
{
	if (group->access || creature->isInGhostMode() || (group->id >= 5 && isInPassThroughMode())) {
		return true;
	}
...
DepotLocker* Player::getDepotLocker(uint32_t depotId)
{
	...
	if (it != depotLockerMap.end()) {
		...
		// Update depot box parents when reopening (Canary does this too)
		const TownMap& towns = g_game.map.towns.getTowns();
		for (const auto& townEntry : towns) {
			uint32_t townId = townEntry.first;
			DepotChest* depotBox = getDepotChest(townId, false);
			if (depotBox) {
				// Depot chests are inside the depot container (index 2)
				Container* depotContainer = dynamic_cast<Container*>(it->second->getItemByIndex(2));
				if (depotContainer) {
					depotBox->setParent(depotContainer);
				}
			}
		}
	...
	// Create depot container to hold all depot chests (Canary approach)
	const TownMap& towns = g_game.map.towns.getTowns();
	uint16_t townCount = static_cast<uint16_t>(towns.size());
	
	// Size matches town count exactly so players can't place items in the main depot container
	Container* depotContainer = Item::CreateItemAsContainer(ITEM_DEPOT, townCount);
	if (depotContainer) {
		// Add depot chests to the depot container in reverse order
		for (auto townIt = towns.rbegin(); townIt != towns.rend(); ++townIt) {
			uint32_t townId = townIt->first;
			Town* town = townIt->second;
			
			DepotChest* depotBox = getDepotChest(townId, true);
			if (depotBox) {
				depotBox->setSpecialDescription(fmt::format("Depot of {:s}.", town->getName()));
				depotContainer->internalAddThing(depotBox);
				depotBox->setParent(depotContainer);
			}
		}
		it->second->internalAddThing(depotContainer);
	}
...
void Player::sendUnjustifiedStats()
{
	if (client) {
...
{
	// [CRASH FIX] Check attackedCreature is valid before accessing
	if (!attackedCreature) {
		return;
	}
...
void Player::addUnjustifiedDead(const Player* attacked)
{
	...
	// Track kill timestamp for day/week/month calculations
	int64_t currentTime = time(nullptr);
	unjustifiedKillTimestamps.push_back(currentTime);

	// Remove kills older than 30 days to prevent vector from growing too large
	// Keep at least the number of kills that match skullTicks
	uint32_t fragTime = g_config.getNumber(ConfigManager::FRAG_TIME);
	uint64_t expectedKills = (skullTicks + fragTime - 1) / fragTime;
	int64_t thirtyDaysAgo = currentTime - (30 * 24 * 60 * 60);

	unjustifiedKillTimestamps.erase(
		std::remove_if(unjustifiedKillTimestamps.begin(), unjustifiedKillTimestamps.end(),
			[thirtyDaysAgo](int64_t timestamp) { return timestamp < thirtyDaysAgo; }),
		unjustifiedKillTimestamps.end()
	);

	// 1. Update the skull FIRST
	// This changes the player's state in memory from White -> Red
	if (getSkull() != SKULL_BLACK) {
	...
	// 2. Update the Visual Icon (Optional, depending on your distro)
	// Most 'setSkull' functions auto-send this, but keeping it ensures the icon updates instantly.
	sendStats();

	// 3. Send the Points Data LAST
	// This calculates the math: "Since the player is NOW Red Skull, how many kills to Black?"
...
void Player::checkSkullTicks(int64_t ticks)
{
	...
	// Clean up old kill timestamps (older than 30 days)
	int64_t currentTime = time(nullptr);
	int64_t thirtyDaysAgo = currentTime - (30 * 24 * 60 * 60);
	unjustifiedKillTimestamps.erase(
		std::remove_if(unjustifiedKillTimestamps.begin(), unjustifiedKillTimestamps.end(),
	...
	if ((skull == SKULL_RED || skull == SKULL_BLACK) && skullTicks < 1 && !hasCondition(CONDITION_INFIGHT)) {
		setSkull(SKULL_NONE);
		// Clear kill timestamps when skull is removed
		unjustifiedKillTimestamps.clear();
	}
}
