...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	StabilityManager::getInstance().validatePlayerData(player->getGUID());
	StabilityManager::getInstance().validateAccountData(accno);

	return true;
}

bool IOLoginData::saveItems(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert, PropWriteStream& propWriteStream, const std::map<Container*, int32_t>& openContainers)
{
	...
	for (const auto& it : itemList) {
		...
		// Check if this is an open container and set the auto-open attribute
		if (Container* container = item->getContainer()) {
			auto openIt = openContainers.find(container);
	...
	while (!queue.empty()) {
		...
		for (Item* item : container->getItemList()) {
			...
			if (subContainer) {
				auto openIt = openContainers.find(subContainer);
				if (openIt != openContainers.end()) {
					subContainer->setAutoOpen(openIt->second);
				} else {
...
bool IOLoginData::savePlayer(Player* player)
{
	...
	// Build map of open containers
	std::map<Container*, int32_t> openContainers;
	for (const auto& it : player->openContainers) {
		if (it.second.container) {
			openContainers[it.second.container] = it.first;
		}
	}
