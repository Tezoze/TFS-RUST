...
Game::Game()
{
	...
	offlineTrainingWindow.choices.emplace_back("Melee Fighting and Shielding", SKILL_SWORD);
...
void Game::setGameState(GameState_t newState)
{
	...
	switch (newState) {
		...
		case GAME_STATE_SHUTDOWN: {
			...
			// Queue shutdown task on dispatcher - it will execute after this function returns
			g_dispatcher.addTask(createTask(std::bind(&Game::shutdown, this)));

			// DO NOT call stop() here - let the shutdown task handle proper shutdown sequence
			break;
		}
...
uint32_t Game::getPlayersCountByAccount(uint32_t acc)
{
	uint32_t count = 0;
	for (const auto& it : players) {
		if (it.second->getAccount() == acc) {
...
bool Game::removeCreature(Creature* creature, bool isLogout/* = true*/)
{
	...
	// [CRASH FIX] Copy summons list to avoid iterator invalidation during recursive removal
	std::vector<Creature*> summonsCopy(creature->summons.begin(), creature->summons.end());
	for (Creature* summon : summonsCopy) {
...
Item* Game::transformItem(Item* item, uint16_t newId, int32_t newCount /*= -1*/)
{
		...
		if (newCount == 0 && (item->isStackable() || item->hasAttribute(ITEM_ATTRIBUTE_CHARGES))) {
			if (item->isStackable()) {
				...
				if (newItemId < 0) {
					...
					// Preserve rarity attributes when enchanted items decay back to base form
					if (item->hasAttribute(ITEM_ATTRIBUTE_DESCRIPTION)) {
						newItem->setStrAttr(ITEM_ATTRIBUTE_DESCRIPTION, item->getStrAttr(ITEM_ATTRIBUTE_DESCRIPTION));
					}
					// Preserve rolled stat bonuses (attack, defense, extra defense, armor, hitchance, shootrange)
					static const itemAttrTypes statAttrs[] = {
						ITEM_ATTRIBUTE_ATTACK, ITEM_ATTRIBUTE_DEFENSE,
						ITEM_ATTRIBUTE_EXTRADEFENSE, ITEM_ATTRIBUTE_ARMOR,
						ITEM_ATTRIBUTE_HITCHANCE, ITEM_ATTRIBUTE_SHOOTRANGE
					};
					for (itemAttrTypes attr : statAttrs) {
						if (item->hasAttribute(attr)) {
							newItem->setIntAttr(attr, item->getIntAttr(attr));
						}
					}
	...
	// Preserve rarity attributes when item types differ during transform
	if (item->hasAttribute(ITEM_ATTRIBUTE_DESCRIPTION)) {
		newItem->setStrAttr(ITEM_ATTRIBUTE_DESCRIPTION, item->getStrAttr(ITEM_ATTRIBUTE_DESCRIPTION));
	}
	static const itemAttrTypes statAttrs2[] = {
		ITEM_ATTRIBUTE_ATTACK, ITEM_ATTRIBUTE_DEFENSE,
		ITEM_ATTRIBUTE_EXTRADEFENSE, ITEM_ATTRIBUTE_ARMOR,
		ITEM_ATTRIBUTE_HITCHANCE, ITEM_ATTRIBUTE_SHOOTRANGE
	};
	for (itemAttrTypes attr : statAttrs2) {
		if (item->hasAttribute(attr)) {
			newItem->setIntAttr(attr, item->getIntAttr(attr));
		}
	}
                         ...
                         uint8_t index, uint16_t spriteId)
{
	...
	// For containers on rope spots, allow opening even if sprite ID doesn't match
	// (client might send rope spot sprite ID instead of container sprite ID)
	Tile* tile = map.getTile(pos);
	if (tile && tile->getGround()) {
		uint16_t groundId = tile->getGround()->getID();
		if (groundId == 384 || groundId == 418 || groundId == 8278 || groundId == 8592 || 
		    groundId == 13189 || groundId == 14435 || groundId == 14436 || groundId == 14857 || 
		    groundId == 15635 || groundId == 19518 || groundId == 24621 || groundId == 24622 || 
		    groundId == 24623 || groundId == 24624 || groundId == 26019) {
			if (item && item->getContainer() && !item->isUseable()) {
				// Skip the sprite ID check for containers on rope spots
				goto skip_sprite_check;
			}
		}
	}
	...
	if (!player->canDoAction() && (!item || !item->getContainer())) {
		uint32_t delay = player->getNextActionTime();
...
void Game::playerShowQuestLog(uint32_t playerId)
{
	...
	// Call Lua quest system instead of C++ quest system
	// Use the Lua function Player:sendQuestLog() from data/lib/core/quests.lua
	if (g_scripts) {
		LuaScriptInterface& scriptInterface = g_scripts->getScriptInterface();
		lua_State* L = scriptInterface.getLuaState();
		
		// Get Player metatable and call sendQuestLog
		lua_getglobal(L, "Player");
		if (lua_istable(L, -1)) {
			lua_getfield(L, -1, "sendQuestLog");
			if (lua_isfunction(L, -1)) {
				LuaScriptInterface::pushUserdata<Player>(L, player);
				LuaScriptInterface::setMetatable(L, -1, "Player");
				
				if (lua_pcall(L, 1, 0, 0) != 0) {
					std::cout << "[Error - Game::playerShowQuestLog] " << lua_tostring(L, -1) << std::endl;
					lua_pop(L, 1);
				}
			} else {
				lua_pop(L, 1);
...
void Game::playerShowQuestLine(uint32_t playerId, uint16_t questId)
{
	...
	// Call Lua quest system - let Lua handle everything
	if (g_scripts) {
		LuaScriptInterface& scriptInterface = g_scripts->getScriptInterface();
		lua_State* L = scriptInterface.getLuaState();
		
		// Call Player:sendQuestLineById with quest ID
		lua_getglobal(L, "Player");
		if (lua_istable(L, -1)) {
			lua_getfield(L, -1, "sendQuestLineById");
			if (lua_isfunction(L, -1)) {
				LuaScriptInterface::pushUserdata<Player>(L, player);
				LuaScriptInterface::setMetatable(L, -1, "Player");
				...
				if (lua_pcall(L, 2, 0, 0) != 0) {
					std::cout << "[Error - Game::playerShowQuestLine] " << lua_tostring(L, -1) << std::endl;
					lua_pop(L, 1);
				}
			} else {
				lua_pop(L, 1);
...
bool Game::internalCreatureTurn(Creature* creature, Direction dir)
{
	...
	for (Creature* spectator : spectators) {
		// [CRASH FIX] Check if spectator is a player
		if (Player* player = spectator->getPlayer()) {
			player->sendCreatureTurn(creature);
		}
	}
	return true;
}
...
{
	// Periodic stability monitoring - reduced frequency to prevent spam
	static int stabilityCheckCounter = 0;
	if (++stabilityCheckCounter % 100 == 0) { // Every 100 creature checks (~2.5 seconds)
		StabilityManager::getInstance().periodicHealthCheck();
		StabilityManager::getInstance().monitorResourceUsage();
	}
	...
	while (it != end) {
		...
		if (creature->creatureCheck) {
			if (creature->getHealth() > 0) {
				...
				// [CRASH FIX] onThink() can trigger Lua callbacks that remove
				// this creature. Re-check before continuing.
				if (!creature->isRemoved() && creature->getHealth() > 0) {
...
void Game::changeSpeed(Creature* creature, int32_t varSpeedDelta)
{
	...
	//send to clients only if creature is not being removed
	if (!creature->isRemoved()) {
		SpectatorVec spectators;
...
{
	// [CRASH FIX] Copy by value — Lua onHealthChange callbacks can recursively
	// call combatChangeHealth, and a script could remove the target creature.
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (manaChange > 0) {
		...
		// [CRASH FIX] Copy by value — same pattern as combatChangeHealth.
		const Position targetPos = target->getPosition();
		if (!target->isAttackable()) {
			if (!target->isInGhostMode()) {
				addMagicEffect(targetPos, CONST_ME_POFF);
...
void Game::addDistanceEffect(const Position& fromPos, const Position& toPos, uint8_t effect)
{
	...
	// Use multifloor=false to match Canary behavior - distance effects only visible on same floor
...
void Game::addDistanceEffect(const SpectatorVec& spectators, const Position& fromPos, const Position& toPos, uint8_t effect)
{
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			const Position& spectatorPos = tmpPlayer->getPosition();
			// If spectator is on a different floor, check if they have line of sight to the effect
			if (spectatorPos.z != fromPos.z) {
				// Check if spectator can see either the source or destination of the effect
				if (!g_game.map.isSightClear(spectatorPos, fromPos, false) && 
				    !g_game.map.isSightClear(spectatorPos, toPos, false)) {
					continue;
				}
			}
...
void Game::shutdown()
{
	...
	// Log database pool statistics before shutdown
	Database::getInstance().logPoolStatistics();

	// Shutdown order is important: scheduler first, then database tasks, then dispatcher
...
void Game::updatePlayerHelpers(const Player& player)
{
	...
	for (Creature* spectator : spectators) {
		// [CRASH FIX] Check if spectator is a player
		if (Player* spectatorPlayer = spectator->getPlayer()) {
			spectatorPlayer->sendCreatureHelpers(creatureId, helpers);
		}
	}
}
...
void Game::playerDebugAssert(uint32_t playerId, const std::string& assertLine, const std::string& date, const std::string& description, const std::string& comment)
{
	...
	FileGuard fileGuard(fopen("client_assertions.txt", "a"));
	if (fileGuard) {
...
void Game::playerCreateMarketOffer(uint32_t playerId, uint8_t type, uint16_t spriteId, uint16_t amount, uint32_t price, bool anonymous)
{
	...
	if (type == MARKETACTION_SELL) {
		...
		const auto& depotChests = player->getDepotChests();
		if (depotChests.empty()) {
			return;
		}

		std::forward_list<Item*> itemList = getMarketItemList(it.wareId, amount, depotChests, player->getInbox());
		if (itemList.empty()) {
			return;
		}
...
void Game::playerAcceptMarketOffer(uint32_t playerId, uint32_t timestamp, uint16_t counter, uint16_t amount)
{
	...
	if (offer.type == MARKETACTION_BUY) {
		...
		if (totalPrice > (player->getMoney() + player->bankBalance)) {
			std::cout << "[Market] Purchase failed: Player " << player->getName() << " (ID: " << player->getGUID() << ") doesn't have enough funds. Required: " << totalPrice << ", Has: " << (player->getMoney() + player->bankBalance) << " (cash: " << player->getMoney() << ", bank: " << player->bankBalance << ")" << std::endl;
			return;
		}
		...
		if (it.stackable) {
			...
			while (tmpAmount > 0) {
				uint16_t stackCount = std::min<uint16_t>(100, tmpAmount);
				Item* item = Item::CreateItem(it.id, stackCount);
				ReturnValue ret = internalAddItem(player->getInbox(), item, INDEX_WHEREEVER, FLAG_NOLIMIT);
				if (ret != RETURNVALUE_NOERROR) {
					std::cout << "[Market] Error: Failed to add item " << it.id << " (stackable, count: " << stackCount << ") to buyer " << player->getName() << " inbox. ReturnValue: " << static_cast<int>(ret) << std::endl;
					delete item;
					break;
				}
			...
			for (uint16_t i = 0; i < amount; ++i) {
				...
				ReturnValue ret = internalAddItem(player->getInbox(), item, INDEX_WHEREEVER, FLAG_NOLIMIT);
				if (ret != RETURNVALUE_NOERROR) {
					std::cout << "[Market] Error: Failed to add item " << it.id << " (non-stackable) to buyer " << player->getName() << " inbox. ReturnValue: " << static_cast<int>(ret) << std::endl;
					delete item;
					break;
				}
			}
	...
	offer.amount -= amount;
		...
		if (offer.amount == 0) {
		if (!IOMarket::deleteOffer(offer.id)) {
			std::cout << "[Market] Error: Failed to delete market offer " << offer.id << std::endl;
		}
	} else {
		if (!IOMarket::acceptOffer(offer.id, amount)) {
			std::cout << "[Market] Error: Failed to update market offer " << offer.id << " (reduce amount by " << amount << ")" << std::endl;
		}
	}
	...
	std::cout << "[Market] Purchase successful: Player " << player->getName() << " bought " << amount << "x item " << offer.itemId << " from offer " << offer.id << " for " << totalPrice << " gold" << std::endl;
}
...
std::forward_list<Item*> Game::getMarketItemList(uint16_t wareId, uint16_t sufficientCount, const std::map<uint32_t, DepotChest*>& depotChests, Inbox* inbox)
{
	std::forward_list<Item*> itemList;
	uint16_t count = 0;

	std::list<Container*> containers;
	for (const auto& [id, depotChest] : depotChests) {
		if (depotChest) {
			containers.push_back(depotChest);
		}
	}
	if (inbox) {
		containers.push_back(inbox);
	}
	...
	do {
		...
		for (Item* item : container->getItemList()) {
			...
			if (itemType.wareId == 0 || itemType.wareId != wareId) {
				continue;
			}
...
void Game::playerAnswerModalWindow(uint32_t playerId, uint32_t modalWindowId, uint8_t button, uint8_t choice)
{
	...
	if (modalWindowId == std::numeric_limits<uint32_t>::max()) {
		if (button == offlineTrainingWindow.defaultEnterButton) {
			if (choice == SKILL_SWORD || choice == SKILL_DISTANCE || choice == SKILL_MAGLEVEL) {
				// Validate that the selected skill is compatible with the player's vocation
				const Vocation* vocation = player->getVocation();
				uint16_t vocationId = vocation->getId();

				bool skillAllowed = false;
				std::string errorMsg;

				// Mages: only magic level
				if (vocationId == 1 || vocationId == 2 || vocationId == 5 || vocationId == 6) {
					if (choice == SKILL_MAGLEVEL) {
						skillAllowed = true;
					} else {
						errorMsg = "Mages can only train magic level offline.";
					}
				}
				// Paladins: distance fighting only
				else if (vocationId == 3 || vocationId == 7) {
					if (choice == SKILL_DISTANCE) {
						skillAllowed = true;
					} else {
						errorMsg = "Paladins can only train distance fighting offline.";
					}
				}
				// Knights: melee or distance, but not magic level
				else if (vocationId == 4 || vocationId == 8) {
					if (choice == SKILL_SWORD || choice == SKILL_DISTANCE) {
						skillAllowed = true;
					} else {
						errorMsg = "Knights can only train melee or distance fighting offline.";
					}
				}
