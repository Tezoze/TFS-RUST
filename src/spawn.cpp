...
bool Spawn::findPlayer(const Position& pos)
{
	...
	for (Creature* spectator : spectators) {
		// [CRASH FIX] Check if spectator is a player before accessing
...
void Spawn::checkSpawn()
{
	...
	for (auto& it : spawnMap) {
		...
		// Check if player is blocking the spawn (Canary-style)
		if (findPlayer(sb.pos)) {
			sb.lastSpawn = OTSYS_TIME();  // Reset timer - player is intentionally blocking
			continue;
		}
		...
		// Try to spawn - timer is set in spawnMonster() on success
		spawnMonster(spawnId, sb);
		
		// REMOVED: RATE_SPAWN limit - all ready monsters spawn immediately
	}
...
{
	// Use GCD for optimal check interval (Canary-style)
	interval = std::gcd(interval, sb.interval);
