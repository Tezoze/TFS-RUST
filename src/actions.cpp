...
ReturnValue Actions::internalUseItem(Player* player, const Position& pos, uint8_t index, Item* item, bool isHotkey)
{
	...
	if (Container* container = item->getContainer()) {
		...
		uint32_t corpseOwner = openContainer->getCorpseOwner();
...
{
	// Only apply action delay for non-container items (potions, runes, etc.)
	// Containers (including corpses) should not set or be affected by the action delay
