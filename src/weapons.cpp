...
//players - melee damage formula: floor(0.085 × skill × attack × d) + floor(level / 5)
int32_t Weapons::getMaxWeaponDamage(uint32_t level, int32_t attackSkill, int32_t attackValue, float attackFactor)
{
	// d = 1/attackFactor (Full Attack: 1, Balanced: 0.75, Defense: 0.5)
	double d = 1.0 / attackFactor;
	return static_cast<int32_t>(std::floor(0.085 * attackSkill * attackValue * d) + std::floor(level / 5.0));
}
...
bool Weapon::useFist(Player* player, Creature* target)
{
	...
	// Use triangular distribution for player melee (averages two rolls like real Tibia)
	damage.primary.value = -triangular_random(0, maxDamage);
...
int32_t WeaponMelee::getWeaponDamage(const Player* player, const Creature*, const Item* item, bool maxDamage /*= false*/) const
{
	...
	// Knights: min damage = 15% of max damage for more consistent output
	// Use triangular distribution for player melee (averages two rolls like real Tibia)
	int32_t minValue = static_cast<int32_t>(std::floor(maxValue * 0.15));
	return -triangular_random(minValue, maxValue);
}
...
bool WeaponDistance::useWeapon(Player* player, Item* item, Creature* target) const
{
	...
	if (it.weaponType == WEAPON_AMMO) {
		...
		if (mainWeapon) {
			...
			// Basic weapons not configured in weapons.xml should be usable by all players
			// But still check range
			const Position& playerPos = player->getPosition();
			const Position& targetPos = target->getPosition();
			if (playerPos.z != targetPos.z || std::max<uint32_t>(Position::getDistanceX(playerPos, targetPos), Position::getDistanceY(playerPos, targetPos)) > mainWeaponItem->getShootRange()) {
				damageModifier = 0;
			} else {
				damageModifier = 100;
			}
...
int32_t WeaponDistance::getWeaponDamage(const Player* player, const Creature* target, const Item* item, bool maxDamage /*= false*/) const
{
	...
	// Distance formula: max = floor(0.09 × skill × attack × d) + floor(level / 5)
	// d = 1/attackFactor (Full Attack: 1, Balanced: 0.75, Defense: 0.5)
	double d = 1.0 / attackFactor;
	int32_t maxValue = static_cast<int32_t>((std::floor(0.09 * attackSkill * attackValue * d) + std::floor(player->getLevel() / 5.0)) * player->getVocation()->distDamageMultiplier);
	
	if (maxDamage) {
		return -maxValue;
	}

	// Paladins: min damage = 13% of max damage for more consistent output
	int32_t minValue = static_cast<int32_t>(std::floor(maxValue * 0.13));
