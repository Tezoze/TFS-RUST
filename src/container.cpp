...
void Container::onUpdateContainerItem(uint32_t index, Item* oldItem, Item* newItem)
{
	...
	for (Creature* spectator : spectators) {
		if (Player* player = spectator->getPlayer()) {
			player->sendUpdateContainerItem(this, index, newItem);
		}
	}
...
void Container::onRemoveContainerItem(uint32_t index, Item* item)
{
	...
	for (Creature* spectator : spectators) {
		if (Player* player = spectator->getPlayer()) {
			player->sendRemoveContainerItem(this, index);
		}
	}
...
void Container::addThing(int32_t index, Thing* thing)
{
	...
	//send change to client
	if (getParent() && (getParent() != VirtualCylinder::virtualCylinder.get())) {
...
{
	// Safety check for null or invalid thing pointer
	if (!thing) {
		return /*RETURNVALUE_NOTPOSSIBLE*/;
	}
	...
	if (index < 0 || static_cast<size_t>(index) >= itemlist.size()) {
		return /*RETURNVALUE_NOTPOSSIBLE*/;
	}
