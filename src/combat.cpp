	...
	if (params.combatType != COMBAT_NONE) {
		...
		for (Tile* tile : tiles) {
			...
			if (CreatureVector* creatures = tile->getCreatures()) {
				...
				// [CRASH FIX] Copy the creature list before iterating.
				// onTargetCombat calls into Lua which could teleport/remove creatures,
				// modifying the tile's creature vector and invalidating iterators.
...
void Combat::doAreaCombat(Creature* caster, const Position& position, const AreaCombat* area, CombatDamage& damage, const CombatParams& params)
{
	...
	for (Creature* creature : toDamageCreatures) {
		// [CRASH FIX] A previous iteration's damage could trigger a Lua callback
		// (e.g. onHealthChange) that removes this creature. Skip if already gone.
		if (creature->isRemoved()) {
			continue;
		}
