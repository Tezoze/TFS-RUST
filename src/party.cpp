...
{
	uint64_t totalShareExperience = experience;
	g_events->eventPartyOnShareExperience(this, totalShareExperience);

	// Split the total shared experience among all party members
	size_t memberCount = memberList.size() + 1; // +1 for leader
	uint64_t baseShare = totalShareExperience / memberCount;
	uint64_t remainder = totalShareExperience % memberCount;

	// Distribute base share to all members first
	size_t extraDistributed = 0;
	for (Player* member : memberList) {
		uint64_t extra = (extraDistributed < remainder) ? 1 : 0;
		member->onGainSharedExperience(baseShare + extra, source);
		extraDistributed++;
	}

	// Give remaining extra to leader if any left
	uint64_t leaderExtra = (extraDistributed < remainder) ? 1 : 0;
	leader->onGainSharedExperience(baseShare + leaderExtra, source);
}
