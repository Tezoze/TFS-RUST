...
Condition* Condition::createCondition(ConditionId_t id, ConditionType_t type, int32_t ticks, int32_t param/* = 0*/, bool buff/* = false*/, uint32_t subId/* = 0*/, bool aggressive/* = false */)
{
	switch (type) {
		...
		case CONDITION_PARALYZE: {
			ConditionSpeed* condition = new ConditionSpeed(id, type, ticks, buff, subId, param, aggressive);
			// [FIX] Force formula variables to 0 for percentage-based attacks (param != 0)
			if (param != 0) {
				condition->setFormulaVars(0, 0, 0, 0);
			}
			return condition;
		}
...
bool ConditionAttributes::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	if (attr == CONDITIONATTR_SKILLS) {
        if (currentSkill > SKILL_LAST) {
            // Read and discard extra attributes to prevent crashes from malformed data
            int32_t dummy;
            return propStream.read<int32_t>(dummy);
        }
		return propStream.read<int32_t>(skills[currentSkill++]);
	} else if (attr == CONDITIONATTR_SPECIALSKILLS) {
        if (currentSpecialSkill > SPECIALSKILL_LAST) {
            // Read and discard extra attributes to prevent crashes from malformed data
            int32_t dummy;
            return propStream.read<int32_t>(dummy);
        }
...
{
    // Capture the base class result
    bool ret = Condition::setParam(param, value);

    if (param == CONDITION_PARAM_SPEED) {
        speedDelta = value;

        // Mark as percentage-based by zeroing formulas
        mina = 0;
        minb = 0;
        maxa = 0;
        maxb = 0;
    ...
    // Return true if the base class handled it (e.g., TICKS), otherwise false
    return ret;
}
...
int32_t ConditionSpeed::getParam(ConditionParam_t param)
{
    switch (param) {
        case CONDITION_PARAM_SPEED:
            return speedDelta;  // Always return the value
...
bool ConditionSpeed::startCondition(Creature* creature)
{
    ...
    // Standard formula-based rune (e.g. Haste/Strong Haste)
    if (speedDelta == 0) {
        int32_t baseSpeed = creature->getBaseSpeed();
        if (baseSpeed <= 0) baseSpeed = 128; // Safety

        // Safety Latch: If Max is 0 but Min isn't, user likely wanted a fixed percentage
        if (mina != 0 && maxa == 0) {
            maxa = mina;
        }

        auto minmax = getFormulaValues(baseSpeed, mina, minb, maxa, maxb);
    ...
    // Check if this is percentage-based (-700 from monster)
    bool isPercentage = (conditionType == CONDITION_PARALYZE &&
                        mina == 0 && maxa == 0 && minb == 0 && maxb == 0);

    if (isPercentage) {
        // [CRITICAL FIX] Use BaseSpeed, not getSpeed().
        // We must calculate the slow relative to the naked creature,
        // because conflicting buffs (Haste) are about to be removed.
        int32_t currentSpeed = creature->getBaseSpeed();
        if (currentSpeed <= 0) currentSpeed = 128;

        int32_t percentage = speedDelta; // e.g., -700

        // Calculate target: 200 * (1000 - 700) / 1000 = 60
        int32_t targetSpeed = (currentSpeed * (1000 + percentage)) / 1000;

        // Store flat change: 60 - 200 = -140
...
{
    // [CRITICAL FIX]
    // If the creature is dead or being removed, DO NOT try to update the game state.
    // Interacting with a dying object causes "pure virtual method called" crashes.
    if (creature->getHealth() <= 0 || creature->isRemoved()) {
        return;
    }

    // Only update speed if the creature is still alive and valid
    g_game.changeSpeed(creature, -speedDelta);
}
...
void ConditionSpeed::addCondition(Creature* creature, const Condition* condition)
{
    ...
    // [FIX] Force update if it's the same type (refresh duration)
    // Removed the "ticks == -1" check that blocked refreshes

    const ConditionSpeed* newCondition = static_cast<const ConditionSpeed*>(condition);
    int32_t baseSpeed = creature->getBaseSpeed();
    if (baseSpeed <= 0) baseSpeed = 128;

    // 1. Resolve Current Power (Flat value)
    int32_t currentPower = speedDelta;

    // 2. Resolve Incoming Power (Convert to Flat value)
    int32_t incomingPower;

    // Check if incoming is percentage-based
    bool isIncomingPercentage = (newCondition->mina == 0 && newCondition->maxa == 0 &&
                                 newCondition->minb == 0 && newCondition->maxb == 0 &&
                                 newCondition->speedDelta != 0);

    if (isIncomingPercentage) {
        // Calculate flat change against BASE speed
        int32_t percentage = newCondition->speedDelta;
        int32_t targetSpeed = (baseSpeed * (1000 + percentage)) / 1000;
        incomingPower = targetSpeed - baseSpeed;
    } else {
        // Formula based
        double avgA = (newCondition->mina + newCondition->maxa) / 2.0;
        double avgB = (newCondition->minb + newCondition->maxb) / 2.0;
        incomingPower = static_cast<int32_t>(baseSpeed * avgA + avgB);
    }

    // 3. Compare Strength
    bool incomingIsStronger;
    bool incomingIsEqual = (incomingPower == currentPower);

    if (conditionType == CONDITION_PARALYZE) {
        // For paralyze (negative), lower is stronger (e.g. -140 is stronger than -50)
        incomingIsStronger = (incomingPower < currentPower);
    } else {
        // For haste (positive), higher is stronger
        incomingIsStronger = (incomingPower > currentPower);
    }
    ...
    int32_t adjustment = incomingPower - currentPower;

    speedDelta = incomingPower;
    mina = newCondition->mina;
    maxa = newCondition->maxa;
    minb = newCondition->minb;
    maxb = newCondition->maxb;

    if (adjustment != 0) {
        g_game.changeSpeed(creature, adjustment);
    }
}
...
{
	this->lazyMonsterOutfit = monsterName;
}
...
bool ConditionOutfit::startCondition(Creature* creature)
{
	...
	// Lazy resolve monster outfit if needed
	if (outfit.lookType == 0 && outfit.lookTypeEx == 0 && !lazyMonsterOutfit.empty()) {
		MonsterType* mType = g_monsters.getMonsterType(lazyMonsterOutfit);
		if (mType) {
			outfit = mType->info.outfit;
		} else {
			std::cout << "[Warning - ConditionOutfit::startCondition] Monster not found: " << lazyMonsterOutfit << std::endl;
			return false;
		}
	}
...
void ConditionOutfit::addCondition(Creature* creature, const Condition* condition)
{
	if (updateCondition(condition)) {
		...
		lazyMonsterOutfit = conditionOutfit.lazyMonsterOutfit;
