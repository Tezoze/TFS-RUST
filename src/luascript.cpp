...
void ScriptEnvironment::resetEnv()
{
	...
	while (it != pair.second) {
		Item* item = it->second;
		if (item && item->getParent() == VirtualCylinder::virtualCylinder.get()) {
...
void LuaScriptInterface::reportError(const char* function, const std::string& error_desc, lua_State* L /*= nullptr*/, bool stack_trace /*= false*/)
{
	...
	errorStream << std::endl << "Lua Script Error: ";

	if (scriptInterface) {
		errorStream << '[' << scriptInterface->getInterfaceName() << "] " << std::endl;

		if (timerEvent) {
			errorStream << "in a timer event called from: " << std::endl;
		}

		if (callbackId) {
			errorStream << "in callback: " << scriptInterface->getFileById(callbackId) << std::endl;
		}

		errorStream << scriptInterface->getFileById(scriptId) << std::endl;
	}
	...
	if (L && stack_trace) {
		errorStream << getStackTrace(L, error_desc) << std::endl;
	} else {
		errorStream << error_desc << std::endl;
	}

	// Output to console
	std::cout << errorStream.str();

	// Write to LUA error log file with date-based filename
	try {
		auto now = std::chrono::system_clock::now();
		auto time_t_now = std::chrono::system_clock::to_time_t(now);
		
		// Create dated filename: lua_errors_YYYY-MM-DD.log
		std::ostringstream filenameStream;
		filenameStream << "data/logs/LUA/lua_errors_";
		filenameStream << std::put_time(std::localtime(&time_t_now), "%Y-%m-%d");
		filenameStream << ".log";
		std::string logFilePath = filenameStream.str();
		...
		if (currentTime - lastCleanup >= 86400) { // 86400 seconds = 1 day
			lastCleanup = currentTime;
			
			try {
				std::filesystem::path luaLogDir = "data/logs/LUA";
				if (std::filesystem::exists(luaLogDir) && std::filesystem::is_directory(luaLogDir)) {
					// Calculate cutoff date (7 days ago)
					auto cutoffDate = std::chrono::system_clock::to_time_t(now - std::chrono::hours(24 * 7));
					
					for (const auto& entry : std::filesystem::directory_iterator(luaLogDir)) {
						if (entry.is_regular_file()) {
							std::string filename = entry.path().filename().string();
							// Only process lua_errors_YYYY-MM-DD.log files
							if (filename.find("lua_errors_") == 0 && filename.find(".log") != std::string::npos) {
								// Extract date from filename: lua_errors_YYYY-MM-DD.log
								size_t dateStart = 11; // length of "lua_errors_"
								size_t dateEnd = filename.find(".log");
								if (dateEnd != std::string::npos && dateEnd - dateStart == 10) {
									std::string dateStr = filename.substr(dateStart, 10); // YYYY-MM-DD
									
									// Parse date string
									std::tm fileTm = {};
									if (sscanf(dateStr.c_str(), "%d-%d-%d", &fileTm.tm_year, &fileTm.tm_mon, &fileTm.tm_mday) == 3) {
										fileTm.tm_year -= 1900; // tm_year is years since 1900
										fileTm.tm_mon -= 1;     // tm_mon is 0-11
										fileTm.tm_hour = 0;
										fileTm.tm_min = 0;
										fileTm.tm_sec = 0;
										...
										if (fileTime < cutoffDate) {
											...
											std::cout << "[Lua] Removed old log file: " << filename << std::endl;
										}
									}
								}
							}
				...
				// Silently ignore cleanup errors to avoid log spam
			}
		}
	} catch (const std::exception& e) {
		// Fallback to console if file logging fails
		std::cerr << "[Lua] Failed to write error log: " << e.what() << std::endl;
	}
}
...
void LuaScriptInterface::pushCylinder(lua_State* L, Cylinder* cylinder)
{
	...
	} else if (cylinder == VirtualCylinder::virtualCylinder.get()) {
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnum(ITEM_ATTRIBUTE_CONTAINERSIZE)
	...
	registerEnum(NPCS_EVENT_CLOSECHANNEL)
	registerEnum(NPCS_EVENT_BUYITEM)
	registerEnum(NPCS_EVENT_SELLITEM)
	...
	registerEnumIn("configKeys", ConfigManager::MYSQL_CONNECTION_POOL_ENABLED)
	registerEnumIn("configKeys", ConfigManager::MYSQL_CONNECTION_POOL_SIZE)
	registerEnumIn("configKeys", ConfigManager::MYSQL_CONNECTION_MAX_POOL_SIZE)
	registerEnumIn("configKeys", ConfigManager::MYSQL_CONNECTION_MIN_POOL_SIZE)
	registerEnumIn("configKeys", ConfigManager::MYSQL_CONNECTION_TIMEOUT)
	registerEnumIn("configKeys", ConfigManager::MYSQL_CONNECTION_ACQUIRE_TIMEOUT)
	...
	registerMethod("Player", "setPassThroughMode", LuaScriptInterface::luaPlayerSetPassThroughMode);
	...
	registerMethod("NpcType", "currency", LuaScriptInterface::luaNpcTypeCurrency);
	...
	registerMethod("NpcType", "getShopItems", LuaScriptInterface::luaNpcTypeGetShopItems);
	...
	registerMethod("NpcType", "onCloseChannel", LuaScriptInterface::luaNpcTypeEventOnCallback);
	registerMethod("NpcType", "onBuyItem", LuaScriptInterface::luaNpcTypeEventOnCallback);
	registerMethod("NpcType", "onSellItem", LuaScriptInterface::luaNpcTypeEventOnCallback);
	registerMethod("NpcType", "onCheckItem", LuaScriptInterface::luaNpcTypeEventOnCallback);
...
const luaL_Reg LuaScriptInterface::luaDatabaseTable[] = {
	...
	{"getPoolSize", LuaScriptInterface::luaDatabaseGetPoolSize},
	{"getActiveConnections", LuaScriptInterface::luaDatabaseGetActiveConnections},
	{"getAvailableConnections", LuaScriptInterface::luaDatabaseGetAvailableConnections},
	{"getActiveTransactionCount", LuaScriptInterface::luaDatabaseGetActiveTransactionCount},
	{"getBufferedProtocolsCount", LuaScriptInterface::luaGetBufferedProtocolsCount},
	{"addProtocolToAutosend", LuaScriptInterface::luaAddProtocolToAutosend},
	{"removeProtocolFromAutosend", LuaScriptInterface::luaRemoveProtocolFromAutosend},
	{nullptr, nullptr}
};
...
int LuaScriptInterface::luaDatabaseGetPoolSize(lua_State* L)
{
	lua_pushnumber(L, Database::getInstance().getPoolSize());
	return 1;
}

int LuaScriptInterface::luaDatabaseGetActiveConnections(lua_State* L)
{
	lua_pushnumber(L, Database::getInstance().getActiveConnections());
	return 1;
}

int LuaScriptInterface::luaDatabaseGetAvailableConnections(lua_State* L)
{
	lua_pushnumber(L, Database::getInstance().getAvailableConnections());
	return 1;
}

int LuaScriptInterface::luaDatabaseGetActiveTransactionCount(lua_State* L)
{
	lua_pushnumber(L, Database::getInstance().getActiveTransactionCount());
	return 1;
}

int LuaScriptInterface::luaGetBufferedProtocolsCount(lua_State* L)
{
	lua_pushnumber(L, OutputMessagePool::getInstance().getBufferedProtocols().size());
	return 1;
}

int LuaScriptInterface::luaAddProtocolToAutosend(lua_State* L)
{
	// For testing purposes - this is a mock function that doesn't actually add a protocol
	// but simulates the behavior for testing the OutputMessagePool logic
	lua_pushboolean(L, true);
	return 1;
}

int LuaScriptInterface::luaRemoveProtocolFromAutosend(lua_State* L)
{
	// For testing purposes - mock function for testing disconnect simulation
	lua_pushboolean(L, true);
	return 1;
}
...
int LuaScriptInterface::luaGameCreateItem(lua_State* L)
{
	...
	if (lua_gettop(L) >= 3) {
		...
		item->setParent(VirtualCylinder::virtualCylinder.get());
	}
...
int LuaScriptInterface::luaGameCreateContainer(lua_State* L)
{
	...
	if (lua_gettop(L) >= 3) {
		...
		container->setParent(VirtualCylinder::virtualCylinder.get());
	}
...
int LuaScriptInterface::luaTileAddItemEx(lua_State* L)
{
	...
	if (item->getParent() != VirtualCylinder::virtualCylinder.get()) {
...
int LuaScriptInterface::luaItemClone(lua_State* L)
{
	...
	clone->setParent(VirtualCylinder::virtualCylinder.get());
...
int LuaScriptInterface::luaItemSplit(lua_State* L)
{
	...
	*itemPtr = newItem;
	...
	splitItem->setParent(VirtualCylinder::virtualCylinder.get());
...
int LuaScriptInterface::luaContainerSetCapacity(lua_State* L)
{
	...
	uint32_t newSize = getNumber<uint32_t>(L, 2);
	if (newSize < 1) {
		newSize = 1;
	}

	container->setIntAttr(ITEM_ATTRIBUTE_CONTAINERSIZE, newSize);
	pushBoolean(L, true);
	return 1;
}
...
int LuaScriptInterface::luaPlayerUpdateKillTracker(lua_State* L)
{
	...
	Monster* monster = getUserdata<Monster>(L, 2);
	if (!monster) {
		lua_pushnil(L);
		return 1;
	}
	...
	// Get monster outfit
	Outfit_t outfit = monster->getDefaultOutfit();
	
	// Collect all items from the corpse
	std::vector<const Item*> corpseItems;
	for (ContainerIterator it = corpse->iterator(); it.hasNext(); it.advance()) {
		corpseItems.push_back(*it);
	}

	// Send kill tracker with monster name, outfit, and corpse items
	player->sendKillTracker(monster->getName(), outfit, corpseItems);
	
	// Also send individual loot stats for each item
	for (const Item* item : corpseItems) {
		player->sendLootStats(item, monster->getName());
	}
...
int LuaScriptInterface::luaPlayerSetPassThroughMode(lua_State* L)
{
	// player:setPassThroughMode(enabled)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	bool enabled = getBoolean(L, 2);
	if (player->isInPassThroughMode() == enabled) {
		pushBoolean(L, true);
		return 1;
	}

	player->switchPassThroughMode();
	pushBoolean(L, true);
	return 1;
}
...
int LuaScriptInterface::luaNpcIsInTalkRange(lua_State* L)
{
	...
	if (position.z != npcPos.z) {
		pushBoolean(L, false);
		return 1;
	}

	// Check if position is within range (Chebyshev distance)
	int32_t dx = std::abs(static_cast<int32_t>(position.x) - static_cast<int32_t>(npcPos.x));
	int32_t dy = std::abs(static_cast<int32_t>(position.y) - static_cast<int32_t>(npcPos.y));
	pushBoolean(L, static_cast<uint32_t>(std::max(dx, dy)) <= range);
	return 1;
}
...
{
	// get: npcType:walkRadius() set: npcType:walkRadius(radius)
	NpcType* npcType = getUserdata<NpcType>(L, 1);
	if (npcType) {
		if (lua_gettop(L) == 1) {
...
{
	// get: npcType:speechBubble() set: npcType:speechBubble(bubble)
	NpcType* npcType = getUserdata<NpcType>(L, 1);
	if (npcType) {
		if (lua_gettop(L) == 1) {
...
int LuaScriptInterface::luaNpcTypeEventOnCallback(lua_State* L)
{
	...
	NpcType* npcType = getUserdata<NpcType>(L, 1);
	if (npcType) {
		// Push the callback function to top of stack (it's at position 2)
		// getEvent() expects the function at position -1 (top of stack)
		lua_pushvalue(L, 2);
...
int LuaScriptInterface::luaNpcTypeCurrency(lua_State* L)
{
	// get: npcType:currency() set: npcType:currency(currencyId)
	NpcType* npcType = getUserdata<NpcType>(L, 1);
	if (!npcType) {
		lua_pushnil(L);
		return 1;
	...
	if (lua_gettop(L) == 1) {
		lua_pushnumber(L, npcType->info.currency);
	} else {
		npcType->info.currency = getNumber<uint16_t>(L, 2);
		pushBoolean(L, true);
	}
	return 1;
}
...
{
	// get: npcType:yellSpeedTicks() set: npcType:yellSpeedTicks(ticks)
	NpcType* npcType = getUserdata<NpcType>(L, 1);
	if (npcType) {
		if (lua_gettop(L) == 1) {
...
{
	// npcType:addShopItem({id = itemId, buy = buyPrice, sell = sellPrice, name = "name", subType = 1})
	NpcType* npcType = getUserdata<NpcType>(L, 1);
	if (!npcType) {
		lua_pushnil(L);
		return 1;
	...
	ShopBlock shopBlock;
	shopBlock.itemId = getField<uint16_t>(L, 2, "id");
	shopBlock.buyPrice = getField<uint32_t>(L, 2, "buy");
	shopBlock.sellPrice = getField<uint32_t>(L, 2, "sell");
	shopBlock.subType = getField<int32_t>(L, 2, "subType");
	shopBlock.name = getFieldString(L, 2, "name");

	npcType->info.shopItemVector.push_back(shopBlock);

	// Set speech bubble to trade if we have shop items
	if (npcType->info.speechBubble == SPEECHBUBBLE_NONE || npcType->info.speechBubble == SPEECHBUBBLE_NORMAL) {
		npcType->info.speechBubble = SPEECHBUBBLE_TRADE;
	}
...
int LuaScriptInterface::luaNpcTypeGetShopItems(lua_State* L)
{
	// npcType:getShopItems()
	NpcType* npcType = getUserdata<NpcType>(L, 1);
	if (!npcType) {
		lua_pushnil(L);
		return 1;
	...
	int index = 0;
	lua_createtable(L, npcType->info.shopItemVector.size(), 0);
	for (const auto& shopBlock : npcType->info.shopItemVector) {
		lua_createtable(L, 0, 5);
		setField(L, "id", shopBlock.itemId);
		setField(L, "buy", shopBlock.buyPrice);
		setField(L, "sell", shopBlock.sellPrice);
		setField(L, "subType", shopBlock.subType);
		setField(L, "name", shopBlock.name);
		lua_rawseti(L, -2, ++index);
	}
	return 1;
}
...
int LuaScriptInterface::luaMonsterSpellSetOutfitMonster(lua_State* L)
{
	...
	// Store the monster name - it will be resolved later during deserializeSpell
	// when all monsters have been loaded
	MonsterSpell* spell = getUserdata<MonsterSpell>(L, 1);
	if (spell) {
