...
class Player final : public Creature, public Cylinder
{
	public:
		...
		const std::map<uint32_t, DepotChest*>& getDepotChests() const { return depotChests; }
	...
	void sendTextMessage(const TextMessage& message) const {
		...
		void sendLootStats(const Item* item, const std::string& creatureName) const {
			if (client) {
				client->sendLootStats(item, creatureName);
			}
		}
		void sendKillTracker(const std::string& monsterName, const Outfit_t& outfit, const std::vector<const Item*>& corpseItems) const {
			if (client) {
				client->sendKillTracker(monsterName, outfit, corpseItems);
			}
		}
	...
	private:
		...
		std::vector<int64_t> unjustifiedKillTimestamps; // Timestamps of unjustified kills for day/week/month tracking
