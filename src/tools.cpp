...
void printXMLError(const std::string& where, const std::string& fileName, const pugi::xml_parse_result& result)
{
	...
	FileGuard fileGuard(fopen(fileName.c_str(), "rb"));
	if (!fileGuard) {
		return;
	}
...
{
	// Changed to uniform distribution to match real Tibia behavior
	// Every value from min to max has equal probability
	return uniform_random(minNumber, maxNumber);
}

int32_t triangular_random(int32_t minNumber, int32_t maxNumber)
{
	// Triangular distribution: averages two uniform rolls
	// This matches tibia-game-master's player melee damage calculation:
	// int RandomFactor = ((rand() % 100) + (rand() % 100)) / 2;
	// Creates a slight bell curve where middle values are more likely
	if (minNumber == maxNumber) {
		return minNumber;
	} else if (minNumber > maxNumber) {
		std::swap(minNumber, maxNumber);
	...
	int32_t roll1 = uniform_random(minNumber, maxNumber);
	int32_t roll2 = uniform_random(minNumber, maxNumber);
...
itemAttrTypes stringToItemAttribute(const std::string& str)
{
	...
	} else if (str == "containersize") {
		return ITEM_ATTRIBUTE_CONTAINERSIZE;
	}
