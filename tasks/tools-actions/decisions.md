...
1. **Finish `registerClass` uniformly** — Gap 7a/7b ✅ done (2026-08-10) for the userdata classes; **Gap 7c outstanding** for the revscript ctor globals (re-audit 2026-08-13). Less code than what existed before.
2. **Fail-fast load phases** — Gap 5a. Converts the contract's worst property (silent `nil`) into a boot error. Unblocked for `data/lib/core`; **still blocked on 7c** for `data/scripts/lib`.
...
6. **`register_class` is the single owner of class globals** — not a hardcoded bootstrap list, not per-module `globals.set`. ⚠️ **Decided, not yet true in the tree:** 7 revscript ctor globals still use `globals.set` ([Gap 7c](gap7-class-globals.md#gap-7c--revscript-constructor-globals-still-bypass-register_class)). Idempotent and order-independent, so init sequencing stops being load-bearing. Mirrors `luascript.cpp` `registerClass`. Resolved 2026-08-10.
...
2. **Verification gap** — `required_data_globals_present_after_lib_load` passed while 9 of 17 core lib files failed to load. Any future "X is done" claim in this doc should be backed by a probe that enumerates load results, not just an allowlist assertion. `lib_core_files_load_with_zero_errors` now asserts this for `data/lib/core` — **but there is no equivalent for `data/scripts/lib`, which is exactly where the 3 failures found on 2026-08-13 were hiding.** Add one as part of Gap 7c.
3. **`createFunctions` policy** — `data/scripts/lib/create_functions.lua` needs `createFunctions` from `data/lib/compat/compat.lua:1408`, which resolved decision #3 above says not to load. Pick one: load compat, port `createFunctions` into `data/lib/core/`, or exclude that file from the fatal lib set. Needed before Gap 5a can be fatal.
4. **`data/actions/**` legacy tree** — delete it or wire it? Currently unreferenced dead content duplicating the tool scripts.
