...
**Date:** 2026-08-09 · updated 2026-08-10 (Gaps 5-6, TVP load model, Gap 7a+7b) · **re-audited 2026-08-13**
...
| Gap 5a — lib stage fatal | **blocked on Gap 7c** — [gaps-load.md](gaps-load.md) |
...
| **Gap 7c — revscript ctor globals** | ⬅ **next**; 3 of 5 `data/scripts/lib/**` files fail today — [gap7-class-globals.md](gap7-class-globals.md) |
...
**Reordered 2026-08-10** after the Gap 7 probe; **re-checked 2026-08-13.** Gap 7 moved to the front as a hard prerequisite. 7a+7b landed 2026-08-10, but the re-audit found 7a incomplete — **the next step is Gap 7c**, then 5a.
...
3c. **Gap 7c — revscript ctor globals** ⬅ **next** — `Action`/`TalkAction`/`MoveEvent`/`Channel`/`Condition`/`Variant`/`MonsterType` through `register_class`; add `CreatureEvent`/`GlobalEvent`; settle `createFunctions` (compat vs port vs exclude); add the table-driven `all_class_globals_are_tables` test. Clears the 3 remaining `data/scripts/lib` load failures.
4. **Gap 5a — Phase 2 fatal** — aggregate lib-stage failures into one error and make them boot-blocking; keep content-stage warn-and-continue. **Blocked on 7c** (3 files still fail — re-audit 2026-08-13). After this, every later gap surfaces at boot instead of at use-time.
...
Dependency summary (updated 2026-08-13): 7a+7b ✅ done → **7c (next)** → 5a → 3 (re-audit ✅ done, implement pending); 3 depends on 4 for the fishing path; 6 is easiest after 3 makes the paths reachable; 1 is independent; 11 is last and optional — and note 11 must **replace** the `data/lib/core` scan, not coexist with it, since `core.lua` currently double-loads 15 core files.
...
Existing guards in `crates/tfs-rust-lua/src/actions.rs::tests` (all passing — 75 lib tests as of 2026-08-13):
...
- `lib_core_files_load_with_zero_errors` — the Gap 7a guard; `data/lib/core` must load clean. **No equivalent exists for `data/scripts/lib`** — that is where the 2026-08-13 failures were hiding (add `scripts_lib_files_load_with_zero_failures` in Gap 7c).

Target-architecture tests (add as the corresponding step lands):

| Test | Asserts | Step |
|---|---|---|
| `all_class_globals_are_tables` | every `register_class` name is a `table`, and callable where it has a ctor — table-driven so new classes are covered automatically | 3c — **does not exist yet**; the 7a tests are the three `register_class_*` unit tests, which do not enumerate real globals. Would fail today (Gap 7c) |
| `scripts_lib_files_load_with_zero_failures` | the `data/scripts/lib/**` stage loads clean (currently 3 of 5 fail) | 3c |
