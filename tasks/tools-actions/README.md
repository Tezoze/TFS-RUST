...
| Gap 3 — missing Lua API methods | ✅ done 2026-08-14 — [gaps-lua-api.md](gaps-lua-api.md) |
...
**Reordered 2026-08-10** after the Gap 7 probe; **re-checked 2026-08-13.** Gap 7 moved to the front as a hard prerequisite. 7a+7b landed 2026-08-10; **7c landed 2026-08-13;** **5a landed 2026-08-13.** Next is `new_for_test()`.

1. **Gap 2** ✅ done — load `functions.lua` + `data/lib/core/*.lua` before actions in `run_server.rs`.
2. **Gap 5** ✅ done — recursive scans + Gap 5a fatal lib stage.
...
5. **`new_for_test()`** — still pending — route the **8** hand-assembled test VMs in `userdata/combat.rs` (`Lua::new()` at 1082/1122/1198/1226/1316/1446/1565/1650) through the real init path, so tests stop validating a VM that isn't shipped.
...
- `lib_stage_failures_are_fatal_and_aggregated` — Gap 5a policy; broken files → `LibStageFailures` listing every file; `core.lua`/`lib.lua` skipped.
