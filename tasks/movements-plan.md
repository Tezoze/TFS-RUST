...
| `:tileItem` / `:uid` / `:position` | **`:tileItem` remaps Add/Remove → ITEMTILE** at register. `:uid` / `:position` skipped |
| StepIn/StepOut fire after walk packets, mutation scope, all tile items | **Done** — uid skipped → aid → itemid |
| AddItem/RemoveItem cylinder fire | **Done** — `executeAddRemItem(moveitem, tileitem, pos)` after tile change; no actor; mutation scope |
...
| StepIn + AddItem | 46 | **M1 walk done** / **M2 drop done** |
| AddItem only | 11 | **M2 done** (scarab coins, altars) |
| StepIn + StepOut | 13 | **M1 done** |
| StepOut only | 1 | **M1 done** (`closing_doors.lua`) |
| RemoveItem (± AddItem) | 3 | **M2 done** (`demon_scroll`, crate puzzle, sandstone) |
