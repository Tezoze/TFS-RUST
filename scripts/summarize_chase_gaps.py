...
def build_report(
    ...
    for evt, key_fn in key_fns.items():
...
def print_report(report: Dict[str, Any]) -> None:
    ...
    print("\nCombat trace (E0–E3)")
    for evt in ("combat_state", "attack_enqueue", "melee_hit"):
...
def main() -> int:
    ...
    ref_events = normalize_todo_go_events(
        filter_max_tick(filter_monster(load_events(args.ref), args.monster), args.max_tick)
    )
    rust_events = normalize_todo_go_events(
        filter_max_tick(filter_monster(load_events(args.rust), args.monster), args.max_tick)
    )
    ...
    report = build_report(ref_events, rust_events, args.monster)
