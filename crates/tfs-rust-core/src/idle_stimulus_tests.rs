...
fn test_772_classify_melee_vs_dist() {
    ...
    let dist_monster =
        insert_monster_with_config(&mut world, "Rat", mpos, 200, dist_idle_monster_config(4));
...
fn test_772_dist_chase_step_budget_from_target_distance() {
    ...
    let outcome =
        world.monster_idle_chase_repath(dist_monster, Some("idle_drain"), max_steps, must_reach);
    assert_eq!(outcome, MonsterIdleChaseRepathOutcome::PathQueued);
    assert_eq!(
        world
            .creatures
    ...
    assert_eq!((max_steps, must_reach), (3, false));
    let outcome =
        world.monster_idle_chase_repath(band3_monster, Some("idle_drain"), max_steps, must_reach);
    assert_eq!(outcome, MonsterIdleChaseRepathOutcome::PathQueued);
    assert_eq!(
        world
            .creatures
...
fn test_e3_attacking_close_chase_arms_wakeup_after_idle() {
    ...
    let base = world.creatures.get(monster).unwrap().base();
    assert!(base.todo.has_go(), "close-chase Go must be enqueued");
    assert!(
...
fn test_phase1_player_single_step_walk_via_todo() {
    ...
    assert!(
        base.todo.has_go(),
        "player ToDo must have Go after move request"
    );
...
fn test_phase1_player_stop_auto_walk_clears_todo() {
    ...
    assert!(
        !base.todo.todo_stop,
        "todo_stop must be cleared after stop completes"
    );
    // `SendSnapback` — `0xB5` (`encode_cancel_walk`).
    let pkts = world
        .pending_outgoing
        .get(&conn)
        .expect("must enqueue snapback");
    assert!(
...
fn test_audit2_stop_from_standstill_sends_immediate_snapback() {
    ...
    assert!(
        !base.todo.todo_stop,
        "todo_stop must not be set when no walk is in progress"
    );
    // Immediate `SendSnapback` — `0xB5` (`cract.cc:1005-1006`).
    let pkts = world
        .pending_outgoing
        .get(&conn)
        .expect("must enqueue immediate snapback");
    assert!(
        pkts.iter().any(|b| !b.is_empty() && b[0] == 0xB5),
...
fn test_audit2_cancel_attack_sends_snapback_when_walk_pending() {
    ...
    assert!(
        !base.todo.has_go(),
        "cancel must clear pending Go (ToDoClear)"
    );
...
fn test_attack_override_clears_pending_walk_and_snapbacks() {
    ...
    assert!(
        base.todo.has_attack(),
        "attack must be enqueued after the clear"
    );
...
fn test_phase1_player_attack_sets_target_and_enqueues() {
    ...
    assert!(
        base.todo.has_attack(),
        "player ToDo must have Attack enqueued"
    );
}
...
fn test_player_walk_while_attacking_re_arms_attack() {
    ...
    assert_eq!(
        base.attack_target,
        Some(target),
        "attack_target must still be set"
    );
    assert!(
        base.todo.has_attack() || base.next_wakeup.is_some(),
...
fn test_phase1_player_follow_sets_follow_and_close_chase() {
    ...
    assert_eq!(
        base.follow_target,
        Some(target),
        "follow must set follow_target"
    );
    assert_eq!(
        base.chase_mode,
        ChaseMode::Close,
        "follow must set CLOSE chase"
    );
}
...
fn test_phase1_player_cancel_clears_target_and_sends_clear_target() {
    ...
    let pkts = world
        .pending_outgoing
        .get(&conn)
        .expect("must enqueue clear-target");
    assert!(
...
fn test_phase1_player_chase_arms_go_when_target_far() {
    ...
    assert!(
        !base.walk_queue.is_empty(),
        "chase must populate walk_queue"
    );
...
fn player_walk_action(
    world: &GameWorld,
    player: CreatureId,
) -> Option<crate::creature::PlayerWalkAction> {
...
fn test_audit4_unpushed_auto_walk_completes_normally() {
    ...
    assert!(
        base.walk_queue.is_empty(),
        "walk_queue must drain on completion"
    );
    assert!(
        base.walk_destinations.is_empty(),
...
fn test_arrow_override_does_not_break_subsequent_auto_walk() {
    ...
    assert!(
        !base.todo.has_go(),
        "todo Go must be drained after completion"
    );
}
...
fn test_rejected_step_does_not_strand_subsequent_auto_walk() {
    ...
    let _blocker = insert_monster(&mut world, "Rat", Position::new(102, 100, 7), 200);
    ...
    assert!(
        !base.force_update_follow_path,
        "flag must remain clear for players"
    );
}
...
fn two_object_use_within_multiuse_gate_defers() {
    ...
    assert_eq!(
        world.creatures.get(player).unwrap().base().todo.queue.len(),
        1,
...
fn otclient_772_west_onto_south_stairs_uses_teleport_path() {
    ...
    for _ in 0..10 {
        if world.creatures.get(player).map(|k| k.position()) == Some(Position::new(100, 101, 7)) {
            break;
        }
        let earliest = world
            .creatures
    ...
    let has_floor_change_opcode = packets
        .iter()
        .any(|p| p.iter().any(|&b| b == 0xBE || b == 0xBF));
    assert!(
...
fn real_772_client_west_onto_south_stairs_uses_notify_go() {
    let (mut world, player, conn) = setup_south_stair_world(Position::new(101, 100, 8), 0);
    let now = std::time::Instant::now();
    world.player_move_request(conn, player, Direction::West, now);
...
fn otclient_772_up_from_surface_skips_remove() {
    ...
    world
        .map
        .unregister_creature_at(Position::new(101, 100, 8), player);
    world
        .map
        .register_creature_at(Position::new(101, 100, 7), player);

    let now = std::time::Instant::now();
    world.player_move_request(conn, player, Direction::West, now);

    for _ in 0..10 {
        if world.creatures.get(player).map(|k| k.position()) == Some(Position::new(100, 99, 6)) {
            break;
        }
        let earliest = world
            .creatures
