...
fn parse_otcv8_string_probe(s: &[u8]) -> Result<(u16, &[u8])> {
    ...
    // 0xFFFF or oversized length = 0xFF RSA padding, not a real string probe.
    // Treat as "no OTCv8 probe" instead of erroring on credentials that decrypt fine.
    if len == 0xFFFF || s.len() < 2 + len {
        return Ok((0, s));
    }
    let chunk = &s[2..2 + len];
    let tail = &s[2 + len..];
