//! MoveEvent registry — revscript StepIn/StepOut/AddItem (and optional Equip).
//!
//! Field DoT and equip abilities are native (`is_magic_field` / `ItemAbilities`),
//! not XML `function=` bindings. C++ reference: `src/movement.cpp` `MoveEvents`,
//! `MoveEvent::fireEquip` / `executeStep`, `executeAddRemItem`,
//! `MoveEvents::registerLuaEvent`, `MoveEvents::getEvent(Item*)`.
...
/// Apply revscript defs into an existing registry.
