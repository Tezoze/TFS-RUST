---@meta
-- Generated by `cargo run -p tfs-rust-lua --bin emit-lua-defs`. Do not edit.
-- VM hardening pillar 5 — tasks/tools-actions/vm-hardening.md

ACCOUNT_TYPE_COMMUNITYMANAGER = 5
ACCOUNT_TYPE_GAMEMASTER = 4
ACCOUNT_TYPE_GOD = 6
ACCOUNT_TYPE_NORMAL = 1
ACCOUNT_TYPE_SENIORTUTOR = 3
ACCOUNT_TYPE_TUTOR = 2
APPLY_SKILL_MULTIPLIER = true
CALLBACK_PARAM_LEVELMAGICVALUE = 0
CALLBACK_PARAM_SKILLVALUE = 1
CALLBACK_PARAM_TARGETCREATURE = 3
CALLBACK_PARAM_TARGETTILE = 2
COMBAT_EARTHDAMAGE = 4
COMBAT_ENERGYDAMAGE = 2
COMBAT_FIREDAMAGE = 8
COMBAT_FORMULA_DAMAGE = 3
COMBAT_FORMULA_LEVELMAGIC = 1
COMBAT_FORMULA_SKILL = 2
COMBAT_FORMULA_UNDEFINED = 0
COMBAT_HEALING = 128
COMBAT_LIFEDRAIN = 32
COMBAT_MANADRAIN = 64
COMBAT_NONE = 0
COMBAT_PARAM_AGGRESSIVE = 7
COMBAT_PARAM_BLOCKARMOR = 4
COMBAT_PARAM_BLOCKSHIELD = 3
COMBAT_PARAM_CREATEITEM = 6
COMBAT_PARAM_DISPEL = 8
COMBAT_PARAM_DISTANCEEFFECT = 2
COMBAT_PARAM_EFFECT = 1
COMBAT_PARAM_FORCEONTARGETEVENT = 11
COMBAT_PARAM_NODAMAGE = 10
COMBAT_PARAM_TARGETCASTERORTOPMOST = 5
COMBAT_PARAM_TYPE = 0
COMBAT_PARAM_USECHARGES = 9
COMBAT_PHYSICALDAMAGE = 1
COMBAT_UNDEFINEDDAMAGE = 16
CONDITIONID_AMMO = 10
CONDITIONID_ARMOR = 4
CONDITIONID_BACKPACK = 3
CONDITIONID_COMBAT = 0
CONDITIONID_DEFAULT = -1
CONDITIONID_FEET = 8
CONDITIONID_HEAD = 1
CONDITIONID_LEFT = 6
CONDITIONID_LEGS = 7
CONDITIONID_NECKLACE = 2
CONDITIONID_RIGHT = 5
CONDITIONID_RING = 9
CONDITION_ATTRIBUTES = 131072
CONDITION_BLEEDING = 8
CONDITION_CHANNELMUTEDTICKS = 32768
CONDITION_DRUNK = 2048
CONDITION_ENERGY = 4
CONDITION_EXHAUST_WEAPON = 4096
CONDITION_FIRE = 2
CONDITION_HASTE = 16
CONDITION_INFIGHT = 1024
CONDITION_INVISIBLE = 128
CONDITION_LIGHT = 256
CONDITION_MANASHIELD = 512
CONDITION_MUTED = 16384
CONDITION_NONE = 0
CONDITION_OUTFIT = 64
CONDITION_PARALYZE = 32
CONDITION_PARAM_BUFF_SPELL = 44
CONDITION_PARAM_COUNT = 58
CONDITION_PARAM_CYCLE = 56
CONDITION_PARAM_DELAYED = 8
CONDITION_PARAM_DRUNKENNESS = 55
CONDITION_PARAM_FORCEUPDATE = 18
CONDITION_PARAM_HEALTHGAIN = 4
CONDITION_PARAM_HEALTHTICKS = 5
CONDITION_PARAM_LIGHT_COLOR = 11
CONDITION_PARAM_LIGHT_LEVEL = 10
CONDITION_PARAM_MANAGAIN = 6
CONDITION_PARAM_MANATICKS = 7
CONDITION_PARAM_MAXVALUE = 15
CONDITION_PARAM_MAX_COUNT = 59
CONDITION_PARAM_MINVALUE = 14
CONDITION_PARAM_OWNER = 1
CONDITION_PARAM_OWNERGUID = 60
CONDITION_PARAM_PERIODICDAMAGE = 35
CONDITION_PARAM_SKILL_AXE = 23
CONDITION_PARAM_SKILL_AXEPERCENT = 40
CONDITION_PARAM_SKILL_CLUB = 21
CONDITION_PARAM_SKILL_CLUBPERCENT = 38
CONDITION_PARAM_SKILL_DISTANCE = 24
CONDITION_PARAM_SKILL_DISTANCEPERCENT = 41
CONDITION_PARAM_SKILL_FISHING = 26
CONDITION_PARAM_SKILL_FISHINGPERCENT = 43
CONDITION_PARAM_SKILL_FIST = 20
CONDITION_PARAM_SKILL_FISTPERCENT = 37
CONDITION_PARAM_SKILL_MELEE = 19
CONDITION_PARAM_SKILL_MELEEPERCENT = 36
CONDITION_PARAM_SKILL_SHIELD = 25
CONDITION_PARAM_SKILL_SHIELDPERCENT = 42
CONDITION_PARAM_SKILL_SWORD = 22
CONDITION_PARAM_SKILL_SWORDPERCENT = 39
CONDITION_PARAM_SOULGAIN = 12
CONDITION_PARAM_SOULTICKS = 13
CONDITION_PARAM_SPEED = 9
CONDITION_PARAM_STARTVALUE = 16
CONDITION_PARAM_STAT_MAGICPOINTS = 30
CONDITION_PARAM_STAT_MAGICPOINTSPERCENT = 34
CONDITION_PARAM_STAT_MAXHITPOINTS = 27
CONDITION_PARAM_STAT_MAXHITPOINTSPERCENT = 31
CONDITION_PARAM_STAT_MAXMANAPOINTS = 28
CONDITION_PARAM_STAT_MAXMANAPOINTSPERCENT = 32
CONDITION_PARAM_SUBID = 45
CONDITION_PARAM_TICKINTERVAL = 17
CONDITION_PARAM_TICKS = 2
CONDITION_POISON = 1
CONDITION_SOUL = 8192
CONDITION_YELLTICKS = 65536
CONST_ANI_ARROW = 3
CONST_ANI_BOLT = 2
CONST_ANI_BURSTARROW = 7
CONST_ANI_DEATH = 11
CONST_ANI_ENERGY = 5
CONST_ANI_FIRE = 4
CONST_ANI_LARGEROCK = 12
CONST_ANI_NONE = 0
CONST_ANI_POISON = 15
CONST_ANI_POISONARROW = 6
CONST_ANI_POWERBOLT = 14
CONST_ANI_SMALLSTONE = 10
CONST_ANI_SNOWBALL = 13
CONST_ANI_SPEAR = 1
CONST_ANI_THROWINGKNIFE = 9
CONST_ANI_THROWINGSTAR = 8
CONST_ME_BLOCKHIT = 4
CONST_ME_DRAWBLOOD = 1
CONST_ME_ENERGYHIT = 12
CONST_ME_EXPLOSIONAREA = 5
CONST_ME_EXPLOSIONHIT = 6
CONST_ME_FIREAREA = 7
CONST_ME_GREEN_RINGS = 9
CONST_ME_HITAREA = 10
CONST_ME_HITBYFIRE = 16
CONST_ME_HITBYPOISON = 17
CONST_ME_LOSEENERGY = 2
CONST_ME_MAGIC_BLUE = 13
CONST_ME_MAGIC_GREEN = 15
CONST_ME_MAGIC_RED = 14
CONST_ME_MORTAREA = 18
CONST_ME_NONE = 0
CONST_ME_POFF = 3
CONST_ME_POISONAREA = 21
CONST_ME_SOUND_BLUE = 24
CONST_ME_SOUND_GREEN = 19
CONST_ME_SOUND_PURPLE = 23
CONST_ME_SOUND_RED = 20
CONST_ME_SOUND_WHITE = 25
CONST_ME_SOUND_YELLOW = 22
CONST_ME_TELEPORT = 11
CONST_ME_YELLOW_RINGS = 8
CONST_PROP_BLOCKPATH = 3
CONST_PROP_BLOCKPROJECTILE = 2
CONST_PROP_BLOCKSOLID = 0
CONST_PROP_HASHEIGHT = 1
CONST_PROP_IMMOVABLEBLOCKPATH = 8
CONST_PROP_IMMOVABLEBLOCKSOLID = 7
CONST_PROP_IMMOVABLENOFIELDBLOCKPATH = 9
CONST_PROP_ISHORIZONTAL = 5
CONST_PROP_ISVERTICAL = 4
CONST_PROP_MOVEABLE = 6
CONST_PROP_NOFIELDBLOCKPATH = 10
CONST_PROP_SPECIALFIELDBLOCKPATH = 12
CONST_PROP_SUPPORTHANGABLE = 11
CONST_SLOT_AMMO = 10
CONST_SLOT_ARMOR = 4
CONST_SLOT_BACKPACK = 3
CONST_SLOT_FEET = 8
CONST_SLOT_HEAD = 1
CONST_SLOT_LEFT = 6
CONST_SLOT_LEGS = 7
CONST_SLOT_NECKLACE = 2
CONST_SLOT_RIGHT = 5
CONST_SLOT_RING = 9
DIRECTION_EAST = 1
DIRECTION_NORTH = 0
DIRECTION_NORTHEAST = 7
DIRECTION_NORTHWEST = 6
DIRECTION_SOUTH = 2
DIRECTION_SOUTHEAST = 5
DIRECTION_SOUTHWEST = 4
DIRECTION_WEST = 3
FLAG_IGNOREBLOCKCREATURE = 4
FLAG_IGNOREBLOCKITEM = 2
FLUID_BEER = 3
FLUID_BLOOD = 5
FLUID_LEMONADE = 12
FLUID_LIFEFLUID = 11
FLUID_MANAFLUID = 10
FLUID_MILK = 9
FLUID_MUD = 4
FLUID_NONE = 0
FLUID_OIL = 7
FLUID_SLIME = 6
FLUID_URINE = 8
FLUID_WATER = 1
FLUID_WINE = 2
GAME_STATE_CLOSED = 3
GAME_STATE_CLOSING = 5
GAME_STATE_INIT = 1
GAME_STATE_MAINTAIN = 6
GAME_STATE_NORMAL = 2
GAME_STATE_SHUTDOWN = 4
GAME_STATE_STARTUP = 0
GUEST_LIST = 256
ITEM_ATTRIBUTE_ACTIONID = 1
ITEM_ATTRIBUTE_CHARGES = 1048576
ITEM_ATTRIBUTE_DESCRIPTION = 4
ITEM_ATTRIBUTE_DOORLEVEL = "doorlevel"
ITEM_ATTRIBUTE_DOORQUESTNUMBER = "doorquestnumber"
ITEM_ATTRIBUTE_DOORQUESTVALUE = "doorquestvalue"
ITEM_ATTRIBUTE_DURATION = 131072
ITEM_ATTRIBUTE_KEYHOLENUMBER = "keyholenumber"
ITEM_ATTRIBUTE_KEYNUMBER = "keynumber"
ITEM_ATTRIBUTE_NONE = 0
ITEM_ATTRIBUTE_TEXT = 8
ITEM_ATTRIBUTE_UNIQUEID = 2
ITEM_CRYSTAL_COIN = 2160
ITEM_ENERGYFIELD_NOPVP = 1504
ITEM_ENERGYFIELD_PERSISTENT = 1495
ITEM_ENERGYFIELD_PVP = 1491
ITEM_FIREFIELD_NOPVP = 1500
ITEM_FIREFIELD_PERSISTENT_FULL = 1492
ITEM_FIREFIELD_PERSISTENT_MEDIUM = 1493
ITEM_FIREFIELD_PERSISTENT_SMALL = 1494
ITEM_FIREFIELD_PVP_FULL = 1487
ITEM_FIREFIELD_PVP_MEDIUM = 1488
ITEM_FIREFIELD_PVP_SMALL = 1489
ITEM_GOLD_COIN = 2148
ITEM_GROUP_AMMUNITION = 4
ITEM_GROUP_ARMOR = 5
ITEM_GROUP_CHARGES = 6
ITEM_GROUP_CONTAINER = 2
ITEM_GROUP_DEPRECATED = 14
ITEM_GROUP_DOOR = 13
ITEM_GROUP_FLUID = 12
ITEM_GROUP_GROUND = 1
ITEM_GROUP_KEY = 10
ITEM_GROUP_MAGICFIELD = 8
ITEM_GROUP_NONE = 0
ITEM_GROUP_SPLASH = 11
ITEM_GROUP_TELEPORT = 7
ITEM_GROUP_WEAPON = 3
ITEM_GROUP_WRITEABLE = 9
ITEM_MAGICWALL = 1497
ITEM_MAGICWALL_NOPVP = 20669
ITEM_MAGICWALL_PERSISTENT = 1498
ITEM_PLATINUM_COIN = 2152
ITEM_POISONFIELD_NOPVP = 1503
ITEM_POISONFIELD_PERSISTENT = 1496
ITEM_POISONFIELD_PVP = 1490
ITEM_TYPE_BED = 8
ITEM_TYPE_CONTAINER = 4
ITEM_TYPE_DEPOT = 1
ITEM_TYPE_DOOR = 5
ITEM_TYPE_KEY = 9
ITEM_TYPE_MAGICFIELD = 6
ITEM_TYPE_MAILBOX = 2
ITEM_TYPE_NONE = 0
ITEM_TYPE_RUNE = 10
ITEM_TYPE_TELEPORT = 7
ITEM_TYPE_TRASHHOLDER = 3
ITEM_WILDGROWTH = 1499
ITEM_WILDGROWTH_NOPVP = 20670
ITEM_WILDGROWTH_PERSISTENT = 2721
MESSAGE_EVENT_ADVANCE = 19
MESSAGE_EVENT_DEFAULT = 20
MESSAGE_INFO_DESCR = 22
MESSAGE_STATUS_CONSOLE_BLUE = 24
MESSAGE_STATUS_CONSOLE_LBLUE = 4
MESSAGE_STATUS_CONSOLE_ORANGE = 17
MESSAGE_STATUS_CONSOLE_RED = 25
MESSAGE_STATUS_CONSOLE_YELLOW = 1
MESSAGE_STATUS_DEFAULT = 21
MESSAGE_STATUS_SMALL = 23
MESSAGE_STATUS_WARNING = 18
PLAYERSEX_FEMALE = 0
PLAYERSEX_MALE = 1
PlayerFlag_CanConvinceAll = 16
PlayerFlag_CanSummonAll = 32
PlayerFlag_CanTalkRedChannel = 4194304
PlayerFlag_CanTalkRedPrivate = 2097152
PlayerFlag_CannotAttackMonster = 4
PlayerFlag_CannotAttackPlayer = 2
PlayerFlag_CannotBeAttacked = 8
PlayerFlag_CannotUseCombat = 1
PlayerFlag_HasInfiniteCapacity = 1048576
PlayerFlag_HasInfiniteMana = 1024
PlayerFlag_IsAlwaysPremium = 34359738368
PlayerFlag_TalkOrangeHelpChannel = 8388608
RELOAD_TYPE_ACTIONS = 1
RELOAD_TYPE_ALL = 0
RELOAD_TYPE_CHAT = 2
RELOAD_TYPE_CONFIG = 3
RELOAD_TYPE_CREATURESCRIPTS = 4
RELOAD_TYPE_EVENTS = 5
RELOAD_TYPE_GLOBAL = 6
RELOAD_TYPE_GLOBALEVENTS = 7
RELOAD_TYPE_ITEMS = 8
RELOAD_TYPE_MONSTERS = 9
RELOAD_TYPE_MOVEMENTS = 10
RELOAD_TYPE_NPCS = 11
RELOAD_TYPE_RAIDS = 12
RELOAD_TYPE_SCRIPTS = 13
RELOAD_TYPE_SPELLS = 14
RELOAD_TYPE_TALKACTIONS = 15
RELOAD_TYPE_WEAPONS = 16
RETURNVALUE_ACTIONNOTPERMITTEDINANOPVPZONE = 54
RETURNVALUE_ACTIONNOTPERMITTEDINPROTECTIONZONE = 40
RETURNVALUE_ANOTHERRAIDISALREADYEXECUTING = 62
RETURNVALUE_BOTHHANDSNEEDTOBEFREE = 11
RETURNVALUE_CANNOTBEDRESSED = 14
RETURNVALUE_CANNOTPICKUP = 22
RETURNVALUE_CANNOTTHROW = 5
RETURNVALUE_CANNOTUSETHISOBJECT = 26
RETURNVALUE_CANONLYUSEONESHIELD = 58
RETURNVALUE_CANONLYUSEONEWEAPON = 12
RETURNVALUE_CANONLYUSETHISRUNEONCREATURES = 39
RETURNVALUE_CONTAINERNOTENOUGHROOM = 20
RETURNVALUE_CREATUREBLOCK = 8
RETURNVALUE_CREATUREDOESNOTEXIST = 25
RETURNVALUE_CREATUREISNOTREACHABLE = 46
RETURNVALUE_DEPOTISFULL = 24
RETURNVALUE_DESTINATIONOUTOFREACH = 7
RETURNVALUE_DIRECTPLAYERSHOOT = 31
RETURNVALUE_DROPTWOHANDEDITEM = 10
RETURNVALUE_FIRSTGODOWNSTAIRS = 18
RETURNVALUE_FIRSTGOUPSTAIRS = 19
RETURNVALUE_ITEMCANNOTBEMOVEDTHERE = 69
RETURNVALUE_NAMEISTOOAMBIGUOUS = 57
RETURNVALUE_NEEDEXCHANGE = 13
RETURNVALUE_NOERROR = 0
RETURNVALUE_NOPARTYMEMBERSINRANGE = 59
RETURNVALUE_NOSUCHRAIDEXISTS = 61
RETURNVALUE_NOTENOUGHCAPACITY = 21
RETURNVALUE_NOTENOUGHLEVEL = 32
RETURNVALUE_NOTENOUGHMAGICLEVEL = 33
RETURNVALUE_NOTENOUGHMANA = 34
RETURNVALUE_NOTENOUGHROOM = 2
RETURNVALUE_NOTENOUGHSOUL = 35
RETURNVALUE_NOTMOVEABLE = 9
RETURNVALUE_NOTPOSSIBLE = 1
RETURNVALUE_PLAYERISNOTINVITED = 4
RETURNVALUE_PLAYERISNOTREACHABLE = 38
RETURNVALUE_PLAYERISPZLOCKED = 3
RETURNVALUE_PLAYERISPZLOCKEDENTERPVPZONE = 53
RETURNVALUE_PLAYERISPZLOCKEDLEAVEPVPZONE = 52
RETURNVALUE_PLAYERWITHTHISNAMEISNOTONLINE = 27
RETURNVALUE_PUTTHISOBJECTINBOTHHANDS = 16
RETURNVALUE_PUTTHISOBJECTINYOURHAND = 15
RETURNVALUE_THEREISNOWAY = 6
RETURNVALUE_THISISIMPOSSIBLE = 23
RETURNVALUE_THISPLAYERISALREADYTRADING = 29
RETURNVALUE_TOOFARAWAY = 17
RETURNVALUE_TRADEPLAYERALREADYOWNSAHOUSE = 65
RETURNVALUE_TRADEPLAYERFARAWAY = 63
RETURNVALUE_TRADEPLAYERHIGHESTBIDDER = 66
RETURNVALUE_TURNSECUREMODETOATTACKUNMARKEDPLAYERS = 47
RETURNVALUE_YOUAREALREADYTRADING = 28
RETURNVALUE_YOUAREEXHAUSTED = 36
RETURNVALUE_YOUARENOTTHEOWNER = 60
RETURNVALUE_YOUCANNOTLOGOUTHERE = 55
RETURNVALUE_YOUCANNOTTRADETHISHOUSE = 67
RETURNVALUE_YOUCANNOTUSEOBJECTSTHATFAST = 37
RETURNVALUE_YOUCANONLYUSEITONCREATURES = 45
RETURNVALUE_YOUDONTHAVEREQUIREDPROFESSION = 68
RETURNVALUE_YOUDONTOWNTHISHOUSE = 64
RETURNVALUE_YOUMAYNOTATTACKAPERSONINPROTECTIONZONE = 42
RETURNVALUE_YOUMAYNOTATTACKAPERSONWHILEINPROTECTIONZONE = 43
RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE = 44
RETURNVALUE_YOUMAYNOTATTACKTHISPLAYER = 41
RETURNVALUE_YOUMAYNOTLOGOUTDURINGAFIGHT = 30
RETURNVALUE_YOUNEEDAMAGICITEMTOCASTSPELL = 56
RETURNVALUE_YOUNEEDAWEAPONTOUSETHISSPELL = 51
RETURNVALUE_YOUNEEDPREMIUMACCOUNT = 48
RETURNVALUE_YOUNEEDTOLEARNTHISSPELL = 49
RETURNVALUE_YOURVOCATIONCANNOTUSETHISSPELL = 50
SKILL_AXE = 3
SKILL_CLUB = 1
SKILL_DISTANCE = 4
SKILL_FISHING = 6
SKILL_FIST = 0
SKILL_LEVEL = 8
SKILL_MAGLEVEL = 7
SKILL_SHIELD = 5
SKILL_SWORD = 2
SKULL_GREEN = 2
SKULL_NONE = 0
SKULL_RED = 4
SKULL_WHITE = 3
SKULL_YELLOW = 1
SPELL_INSTANT = 1
SPELL_RUNE = 2
SUBOWNER_LIST = 257
TALKTYPE_CHANNEL_O = 12
TALKTYPE_CHANNEL_R1 = 10
TALKTYPE_CHANNEL_R2 = 14
TALKTYPE_CHANNEL_Y = 5
TALKTYPE_MONSTER_SAY = 17
TALKTYPE_SAY = 1
TILESTATE_BED = 32768
TILESTATE_BLOCKPATH = 262144
TILESTATE_BLOCKSOLID = 131072
TILESTATE_DEPOT = 65536
TILESTATE_FLOORCHANGE = 127
TILESTATE_FLOORCHANGE_DOWN = 1
TILESTATE_FLOORCHANGE_EAST = 8
TILESTATE_FLOORCHANGE_NORTH = 2
TILESTATE_FLOORCHANGE_SOUTH = 4
TILESTATE_FLOORCHANGE_WEST = 16
TILESTATE_IMMOVABLEBLOCKSOLID = 524288
TILESTATE_MAGICFIELD = 4096
TILESTATE_MAILBOX = 8192
TILESTATE_NOLOGOUT = 512
TILESTATE_NONE = 0
TILESTATE_NOPVPZONE = 256
TILESTATE_PROTECTIONZONE = 128
TILESTATE_PVPZONE = 1024
TILESTATE_REFRESH = 0
TILESTATE_TELEPORT = 2048
TILESTATE_TRASHHOLDER = 16384
VARIANT_NUMBER = 1
VARIANT_POSITION = 2
VARIANT_STRING = 4
VARIANT_TARGETPOSITION = 3
VOCATION_NONE = 0
WEAPON_AMMO = 7
WEAPON_AXE = 3
WEAPON_CLUB = 2
WEAPON_DISTANCE = 5
WEAPON_NONE = 0
WEAPON_SHIELD = 4
WEAPON_SWORD = 1
WEAPON_WAND = 6
WORLD_TYPE_NO_PVP = 0
WORLD_TYPE_PVP = 1
WORLD_TYPE_PVP_ENFORCED = 2

configKeys = {
    FREE_PREMIUM = 7,
    GUILHALLS_ONLYFOR_LEADERS = 49,
    HOUSES_ONLY_PREMIUM = 50,
    HOUSE_PRICE = 8,
    KILLS_DAY_BANISHMENT = 33,
    KILLS_DAY_RED_SKULL = 30,
    KILLS_MONTH_BANISHMENT = 35,
    KILLS_MONTH_RED_SKULL = 32,
    KILLS_WEEK_BANISHMENT = 34,
    KILLS_WEEK_RED_SKULL = 31,
    RATE_EXPERIENCE = 3,
    RATE_LOOT = 5,
    RATE_MAGIC = 6,
    RATE_SKILL = 4,
    USE_HOUSE_AREA_PRICES = 51,
}
