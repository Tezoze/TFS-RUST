...
Always pass the indexed project name. For `/mnt/storage2/TFS_RUST` it is:

```
project: "mnt-storage2-TFS_RUST"
```

Do **not** use `TFS_RUST`, `mnt-storage2-TFS-RUST`, or other guesses from the folder basename.
When unsure, call `list_projects` first (or read `.codebase-memory/artifact.json` → `project`).
...
6. Wrong `project` yields `project not found or not indexed` with `available_projects` — use that list exactly; never guess `TFS_RUST` from the repo folder name.
