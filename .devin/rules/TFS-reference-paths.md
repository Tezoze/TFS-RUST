...
description: Local 772 C++ reference trees — primary parity target; agent access and graph lookup
...
| Path | Era | Use for |
|------|-----|---------|
| `reference/cipsoft-772/tibia-game-master/src/` | **772 mechanics (primary)** | AI, chase, combat **outcomes** |
| `reference/classic-772/tibia-game-master/src/` | 772 mechanics | Same layout (preferred name when present) |
| `reference/tvp-772/gameserver/src/` | **772 wire (primary)** | Opcodes, packets, login |
| `src/` (repo root) | 1098 + TFS **domain shape** cites | 1098 behavior; TFS APIs/modules the data pack expects |

**Primary goal:** 772 decompile **outcomes** + gameserver **wire**, on a **TFS-style domain** (`data/` / Lua), written in **idiomatic Rust**. Repo-root `src/` guides domain shape and 1098 behavior — not C++ style to copy, and not the default 772 mechanics numbers.
