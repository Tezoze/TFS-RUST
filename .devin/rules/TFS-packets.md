...
**Versioning:** wire layout differs by `clientVersion` (772 vs 1098). **Primary wire target: 772** (`gameserver/src/` only). Follow `@.cursor/rules/TFS-wire-codec.mdc`; route encoders through `ProtocolCodec`, not ad-hoc `*_1098` helpers. **1098** cites repo-root `src/`; never use decompile or repo-root `src/` for 772 wire.
