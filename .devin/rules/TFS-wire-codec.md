...
# Wire Codec (772 primary / 1098 supported)

Complements `@.cursor/rules/TFS-packets.mdc`. **Primary wire target: 772** (`gameserver/src/` only).
...
4. **Impls** — implement **`Codec772` first** (gameserver layout); keep/update `Codec1098` when the shared path must still serve 1098. Branch lives in codec only.
5. **Opcode** — version-keyed table in `protocol_opcodes.rs`. Never inline hex at call sites.
6. **C++ ref** — **772: `gameserver/src/` only** (never repo-root `src/`, never `tibia-game-master`); 1098: repo-root `protocolgame.cpp` / `networkmessage.cpp`.
7. **Test** — golden bytes in `tests/protocol_compat.rs`, one module per version (772 required for new 772 wire).
