...
**In one line:** decompile **numbers and behavior**, TFS **shape** (data pack), Rust **idioms** (code).
...
- **Rust:** best modern idioms — see `@.cursor/rules/TFS-rust-idioms.mdc`
...
**Write the best idiomatic Rust** behind that TFS-shaped domain. Prefer zero-cost abstractions, enums + pattern matching, and clean ownership over mirroring `.cpp` structure. See `@.cursor/rules/TFS-rust-idioms.mdc`.
...
Ultra-lightweight one-offs: `mkdir`, `touch`, `chmod`, `cd`, heredocs, or commands RTK has no proxy for (e.g. `code-review-graph`, `python3 -c`, MCP invocations).
